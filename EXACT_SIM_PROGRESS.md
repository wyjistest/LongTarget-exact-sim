...
- host-merge representative corpus 现在还有一层脚本化决策护栏：
  ...
  - 针对同一份 16-case frozen corpus，`context_apply` 现在又按代码天然边界继续拆成 `lookup / mutate / finalize`，并把 `attempted / modified / noop / lookup_hit / lookup_miss` 计数落进 `replay_context_apply_split.aggregate.tsv`：
    - 16-case 直接求和后，`store_other_merge_context_apply=3.0316s`，其中 `lookup=2.2130s (73.00%)`、`mutate=0.8186s (27.00%)`、`finalize≈0.000001s (~0%)`；计数侧对应 `attempted=47.73M`、`modified=47.55M (99.61%)`、`noop=0.186M (0.39%)`、`lookup_hit=2.27M (4.76%)`、`lookup_miss=45.46M (95.24%)`。
    - 这说明 `context_apply` 内部的下一刀不该先打 `finalize`、snapshot 或 no-op fast-path，而应优先打 `lookup` / slot-materialization 路径；`mutate` 仍有次级占比，但不是第一主战场。
    - 同时，新的 split 是通过 per-summary 计时器做的更深 microbenchmark，绝对秒数会抬高 replay aggregate 的顶层 `store_other_merge` 口径；因此 `analysis_context_apply_split/summary.json` 只用于验证 coverage gate 仍然是 `ready` 且 `dominant_phase=store_other_merge`，不用于替代之前 `analysis_other_merge_split` 的顶层 phase-share 基线。
  - 随后把 `ensureSimCandidateIndexForRun()` 的 miss-path 做成一轮低风险优化：单次 probe 复用、直接写入已探测 slot，以及 miss 分支上直接初始化 candidate，而不是再走一次通用更新逻辑；同时把 `slot_created_count` 与归一化 lookup 指标接进 `replay_lookup_miss_optimized.aggregate.tsv`：
    - 用同一份 16-case frozen corpus 回放后，`store_other_merge_context_apply_lookup` 从 `2.2130s` 降到 `2.0423s`，相对改善约 `7.71%`；`store_other_merge_context_apply` 从 `3.0316s` 降到 `2.8294s`，相对改善约 `6.67%`；顶层 `store_other_merge` 从 `5.8446s` 降到 `5.6611s`，相对改善约 `3.14%`；`full_host_merge` 也从 `8.7041s` 小幅降到 `8.6403s`。
    - 新增归一化指标显示，当前 16-case corpus 上 `lookup_ns_per_attempt≈42.79 ns`，其中 `lookup_hit_ns_per_event≈25.13 ns`、`lookup_miss_ns_per_event≈43.67 ns`；hit path 明显更便宜，但热点仍然来自 miss path 的极高频次。
    - 更关键的是，计数侧新增的 `slot_created_count=800` 与 `lookup_miss=45.46M` 并不一一对应，且 `800 = 16 cases × 50 slots` 正好等于每个 case 把 candidate set 填满一次所需的新增槽位数。这说明当前 miss 主体并不是“反复创建新 slot”，而是 candidate set 饱和后的 miss/evict/reuse 路径；因此下一刀不该回去打 hit-path 或 no-op，而应继续沿 `lookup` 内部的满表 miss / eviction 路径做更细 split 或优化。
    - 用同一份 corpus 重新跑 `analysis_lookup_miss_optimized/summary.json` 后，coverage gate 仍是 `ready`（`covered_logical_event_share=0.8544`、`covered_store_materialized_share=0.8988`），`dominant_phase` 也仍然是 `store_other_merge`。由于这次 replay 继续引入更深的 per-summary instrumentation，新的 phase-share 只用于确认“主瓶颈没有移走”，不用于和更浅层的 `analysis_other_merge_split` 做绝对秒数对比。
  - 随后继续把同一条 `lookup miss` 路径拆到“candidate set 未满”与“candidate set 已满后的 eviction/reuse”这一级：runtime benchmark stderr、replay result 与 aggregate TSV 现在额外落出 `lookup_probe_steps_total/max`、`lookup_miss_open_slot_*`、`lookup_miss_candidate_set_full_probe_*`、`lookup_miss_eviction_select_*`、`lookup_miss_reuse_writeback_*`：
    - 在同一份 16-case frozen corpus 的 `replay_lookup_miss_split.aggregate.tsv` 上直接求和，`lookup_miss=4.6939s`，其中 `open_slot=0.000057s (~0.001%)`、`candidate_set_full_probe=1.4011s (29.85%)`、`eviction_select=0.6999s (14.91%)`、`reuse_writeback=2.5928s (55.24%)`。
    - 计数侧与这组时间分布完全一致：`lookup_miss_open_slot_count=800`，`lookup_miss_candidate_set_full_count=45,460,323`，且 `eviction_selected_count=reused_slot_count=45,460,323`；也就是说，当前 steady-state miss 基本全部发生在 50-slot candidate set 填满之后，而 pre-capacity open-slot miss 只占极小尾巴。
    - `lookup_probe_steps_total=171,521,467`、`lookup_probe_steps_avg≈3.59`、`lookup_probe_steps_max=895` 说明 open-addressing probe 确实存在长尾，但在当前 16-case corpus 上，它仍然不是 miss-path 的最大时间块；第一优先级已经从“继续猜 probe 还是 eviction”收紧为“先把 reused-slot writeback 再拆开或优化”。
    - 用同一份 corpus 重新跑 `analysis_lookup_miss_split/summary.json` 后，coverage gate 仍是 `ready`（`covered_logical_event_share=0.8544`、`covered_store_materialized_share=0.8988`），`dominant_phase=store_other_merge` 也没有变化。和之前的更深 split 一样，这份 summary 只用于确认主瓶颈仍在 `store_other_merge`，真正用于决定下一刀的是 miss-path 内部四段的相对占比，而不是顶层绝对秒数。
  - 继续把同一条 steady-state `lookup_miss_reuse_writeback` 路径按代码天然边界再拆成 `victim_reset / key_rebind / candidate_copy / aux_bookkeeping`，并把 `victim_reset_count`、`candidate_copy_count`、`aux_bookkeeping_count`、`payload_bytes_total`、`aux_updates_total` 一起接进 runtime benchmark stderr、replay result 与 aggregate TSV：
    - 在同一份 16-case frozen corpus 的 `replay_reuse_writeback_split.aggregate.tsv` 上直接求和，`reuse_writeback=6.882546s`；其中 `victim_reset=0.754307s (10.96%)`、`key_rebind=0.700933s (10.18%)`、`candidate_copy=0.771843s (11.21%)`、`aux_bookkeeping=4.655461s (67.64%)`。由于这一轮继续向 `reuse_writeback` 内部加入更细的 per-summary 计时器，这组绝对秒数只用于看内部占比，不和上一轮 `replay_lookup_miss_split.aggregate.tsv` 的顶层绝对秒数横比。
    - 计数侧进一步说明主矛盾并不在“偶发大对象复制”，而在 steady-state 常态路径本身：`victim_reset_count=candidate_copy_count=aux_bookkeeping_count=45,460,323`，`payload_bytes_total=3,273,143,256`（等于每次重用写回一个 `72B` 的 `SimCandidate`），`aux_updates_total=45,914,681`，也就是每次 steady-state reuse 平均约 `1.01` 次 auxiliary update。
    - 用同一份 corpus 重新跑 `analysis_reuse_writeback_split/summary.json` 后，coverage gate 仍然是 `ready`（`covered_logical_event_share=0.8544`、`covered_store_materialized_share=0.8988`），`dominant_phase` 仍然保持 `store_other_merge`。脚本级 `next_action` 还是泛化的 `revisit_store_other_merge`，但从 `reuse_writeback` 内部占比看，下一刀已经足够明确：不是回去打 probe/hit/update/no-op，而是先把 `aux_bookkeeping` 这个 `67.64%` 的 residual 再拆开。
