...
DOC="$ROOT/docs/fasim_gasal2_phase7_post_consumer_gpu_scoreinfo_certificate_engine_first1_shadow_scaffold.md"
PREV="$ROOT/docs/fasim_gasal2_phase7_post_consumer_gpu_scoreinfo_certificate_engine_spec_or_path_a_acceptance.md"
...
if missing:
    raise SystemExit(
        "missing post-consumer certificate-engine first1 shadow phrase(s):\n"
        + "\n".join(missing)
    )
...
for forbidden in [
    ...
    if forbidden in text:
        raise SystemExit(f"first1 shadow scaffold doc contains forbidden phrase: {forbidden}")

if "next_valid_gate = phase7_post_consumer_gpu_scoreinfo_certificate_engine_first1_shadow_scaffold" not in prev_text:
    raise SystemExit("previous spec doc no longer points to first1 shadow scaffold")

link = "docs/fasim_gasal2_phase7_post_consumer_gpu_scoreinfo_certificate_engine_first1_shadow_scaffold.md"
for label, content in [
    ...
    if link not in content:
        raise SystemExit(f"{label} does not link first1 shadow scaffold checkpoint")
    if "phase7_post_consumer_gpu_scoreinfo_certificate_engine_first1_shadow_consumer_no_go" not in content:
        raise SystemExit(f"{label} does not record the consumer/no-go checkpoint after first1 shadow")
    if "current_gate = phase7_gpu_owned_scoreinfo_consumer_design_spec_or_path_a_acceptance" not in content:
        raise SystemExit(f"{label} cursor was not advanced to the GPU-owned scoreInfo consumer spec gate")
...
for phrase in [
    ...
    "phase7_post_consumer_gpu_scoreinfo_certificate_engine_first1_shadow_runtime_reduction_enabled",
    "phase7_post_consumer_gpu_scoreinfo_certificate_engine_first1_shadow_runtime_work_drop_enabled",
    "phase7_post_consumer_gpu_scoreinfo_certificate_engine_first1_shadow_gate_first1_shadow_pass",
...
for target in [
    ...
    "check-fasim-gasal2-roadmap-phase7-post-consumer-gpu-scoreinfo-certificate-engine-first1-shadow-scaffold:",
]:
...
if "check-fasim-gasal2-roadmap-phase7-post-consumer-gpu-scoreinfo-certificate-engine-first1-shadow-scaffold" not in current_state_line:
    raise SystemExit("current-state aggregate does not include first1 shadow scaffold target")

print("phase7_post_consumer_gpu_scoreinfo_certificate_engine_first1_shadow_scaffold=fail_closed_shadow")
print("phase7_post_consumer_gpu_scoreinfo_certificate_engine_first1_shadow_scaffold_status=fail_closed_no_runtime_reduction")
print("phase7_post_consumer_gpu_scoreinfo_certificate_engine_first1_shadow_runtime_reduction_enabled=0")
print("phase7_post_consumer_gpu_scoreinfo_certificate_engine_first1_shadow_runtime_work_drop_enabled=0")
print("phase7_post_consumer_gpu_scoreinfo_certificate_engine_first1_shadow_gate_first1_shadow_pass=0")
print("next_valid_gate=phase7_post_consumer_gpu_scoreinfo_certificate_engine_first1_shadow_consumer_or_no_go")
