...
WORK="${WORK:-"$ROOT/.tmp/check_fasim_tfosorted_column_archive_probe"}"
...
archive_file="$(find "$WORK/run" -maxdepth 1 -type f -name '*.column-archive.tfoa' | sort | head -n 1)"
if [[ -z "$archive_file" || ! -s "$archive_file" ]]; then
  echo "missing column archive probe output" >&2
  exit 1
fi
...
dictionary_payloads="$(awk -F= '$1 == "dictionary_payloads" {print $2}' "$WORK/run/restore.log")"
if [[ -z "$dictionary_payloads" || "$dictionary_payloads" -lt 2 ]]; then
  echo "column archive did not report two dictionary payloads" >&2
  cat "$WORK/run/restore.log" >&2
  exit 1
fi
restored_rows="$(awk -F= '$1 == "rows" {print $2}' "$WORK/run/restore.log")"
