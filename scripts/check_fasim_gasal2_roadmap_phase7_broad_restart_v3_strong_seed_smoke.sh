...
DOC="$ROOT/docs/fasim_gasal2_phase7_broad_restart_v3_strong_seed_smoke.md"
PREV="$ROOT/docs/fasim_gasal2_phase7_broad_restart_v3_next_source_smoke.md"
...
for phrase in required:
    if phrase not in flat:
        raise SystemExit(f"missing required strong-seed smoke phrase: {phrase}")
...
for phrase in [
    ...
    if phrase not in prev_flat:
        raise SystemExit(f"missing previous next-source smoke prerequisite phrase: {phrase}")

for target in [
    "check-fasim-gasal2-phase7-v3-strong-seed-runtime-smoke:",
    "check-fasim-gasal2-roadmap-phase7-broad-restart-v3-strong-seed-smoke:",
]:
...
if claimed_broad:
    raise SystemExit("workload matrix must not claim broad_replacement from strong-seed smoke")

print("phase7_broad_restart_v3_strong_seed_smoke=task_coverage_clean_attempt_coverage_missing")
print("phase7_broad_restart_v3_strong_seed_status=runtime_smoke_no_go_attempt_coverage")
print("phase7_broad_restart_v3_gate_v3_1_pass=0")
print("phase7_broad_restart_v3_gate_v3_2_pass=0")
print("phase7_broad_restart_v3_next_gate=attempt_coverage_seed_certificate_or_different_exact_scoreinfo_source")
