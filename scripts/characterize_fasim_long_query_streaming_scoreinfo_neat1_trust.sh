...
WORK="${WORK:-"$ROOT/.tmp/characterize_fasim_long_query_streaming_scoreinfo_neat1_trust"}"
BUILD_BIN="${BUILD_BIN:-1}"
NEAT1_RECORD_LIMITS="${NEAT1_RECORD_LIMITS:-4 16}"
...
run_limit() {
  ...
  local run_work="$WORK/neat1_first${limit}"
  local child_work="$run_work/run"
  mkdir -p "$run_work"
  echo "running NEAT1 first${limit} realpath trust prototype" >&2
  ...
  if [[ ! -s "$stdout" || ! -s "$stderr" || ! -s "$baseline_stdout" || ! -s "$candidate_stdout" ]]; then
    echo "missing output for NEAT1 first${limit}" >&2
    exit 1
  fi
  ...
  for name in \
    ...
    baseline_wall_seconds candidate_wall_seconds decision; do
    if [[ -z "${!name}" ]]; then
      echo "missing metric $name for NEAT1 first${limit}" >&2
      exit 1
    fi
  done

  if [[ "$realpath_trust" != "1" || "$realpath_digest_authority" != "external_digest_gate" ]]; then
    echo "expected trust external digest gate for NEAT1 first${limit}, got trust=$realpath_trust authority=$realpath_digest_authority" >&2
    exit 1
  fi
  if [[ "$realpath_used" != "$tasks" || "$gpu_minscore_used" != "$tasks" ]]; then
    echo "expected realpath/GPU minScore used to equal tasks for NEAT1 first${limit}, got realpath=$realpath_used gpu_minscore=$gpu_minscore_used tasks=$tasks" >&2
    exit 1
  fi
  if [[ "$realpath_fallbacks" != "0" || "$gpu_minscore_fallbacks" != "0" ]]; then
    echo "expected no trust fallback for NEAT1 first${limit}, got realpath=$realpath_fallbacks gpu_minscore=$gpu_minscore_fallbacks" >&2
    exit 1
  fi
  if [[ "$gpu_scoreinfo_groups" -le 0 || "$cpu_scoreinfo_groups" != "0" ]]; then
    echo "expected GPU scoreInfo only for NEAT1 first${limit}, got gpu=$gpu_scoreinfo_groups cpu=$cpu_scoreinfo_groups" >&2
    exit 1
  fi
  if [[ "$cpu_prealign_seconds" != "0" || "$compare_seconds" != "0" ]]; then
    echo "expected no CPU preAlign/compare validation for NEAT1 first${limit}, got cpu=$cpu_prealign_seconds compare=$compare_seconds" >&2
    exit 1
  fi
  if [[ "$decision" != "streaming_scoreinfo_shadow_active" ]]; then
    echo "unexpected NEAT1 trust decision for first${limit}: $decision" >&2
    exit 1
  fi
