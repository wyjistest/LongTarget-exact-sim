...
WORK="${WORK:-"$ROOT/.tmp/check_fasim_gasal2_phase7_v4_legacy_byte_scoreinfo_shadow_runtime_smoke"}"
...
run_case "$WORK/candidate" \
  FASIM_GASAL2_PHASE7_V4_LEGACY_BYTE_SCOREINFO_SHADOW=1
...
if digest_for_dir(baseline_dir) != digest_for_dir(candidate_dir):
    raise SystemExit("v4 legacy-byte scoreInfo shadow changed lite output digest")
...
if value(base, "requested") != "0" or value(base, "active") != "0":
    raise SystemExit("v4 legacy-byte scoreInfo shadow must be default-off")
if value(candidate, "requested") != "1":
    raise SystemExit("v4 legacy-byte scoreInfo shadow was not requested")
if value(candidate, "active") != "1":
    raise SystemExit("v4 legacy-byte scoreInfo shadow did not run")
...
if tasks <= 0:
    raise SystemExit("v4 shadow must see at least one task")
if cpu_rows <= 0 or host_rows <= 0:
    raise SystemExit("v4 shadow must compare non-empty scoreInfo rows")
...
if source != "host_column_score_reconstruction":
    raise SystemExit(f"unexpected v4 host-contract source: {source}")

print("phase7_v4_legacy_byte_scoreinfo_shadow_runtime_smoke=host_contract_clean_needs_gpu")
print("phase7_v4_legacy_byte_scoreinfo_shadow_active=1")
print("phase7_v4_legacy_byte_scoreinfo_shadow_scoreinfo_rows_equal=1")
print("phase7_v4_legacy_byte_scoreinfo_shadow_scoreinfo_order_equal=1")
print("phase7_v4_legacy_byte_scoreinfo_shadow_scoreinfo_attempt_windows_equal=1")
print("phase7_v4_legacy_byte_scoreinfo_shadow_host_contract_pass=1")
print("phase7_v4_legacy_byte_scoreinfo_shadow_gpu_scoreinfo_rows=0")
print("phase7_v4_legacy_byte_scoreinfo_shadow_gate_v4_1_pass=0")
