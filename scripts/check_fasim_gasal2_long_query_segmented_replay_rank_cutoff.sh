...
WORK="${WORK:-"$ROOT/.tmp/check_fasim_gasal2_long_query_segmented_replay_rank_cutoff"}"
...
bash "$ROOT/scripts/characterize_fasim_gasal2_long_query_segmented_shadow.sh" \
  ...
  CPU_TRACEBACK_MAX_RANK="$MAX_RANK" \
  >"$WORK.stdout.log"

summary="$WORK/summary.tsv"
if [[ ! -s "$summary" ]]; then
  echo "missing rank-cutoff summary: $summary" >&2
  exit 1
fi

python3 - "$summary" "$MAX_RANK" <<'PY'
import csv
import sys
...
row = rows[0]
max_rank = sys.argv[2]
if row.get("cpu_traceback_max_rank") != max_rank:
    raise SystemExit(f"expected cpu_traceback_max_rank={max_rank}: {row}")
if row.get("cpu_traceback_no_last") != "1":
    raise SystemExit(f"expected no-last replay: {row}")
