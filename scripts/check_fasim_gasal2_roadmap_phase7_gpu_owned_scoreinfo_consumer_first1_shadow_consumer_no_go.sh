...
DOC="$ROOT/docs/fasim_gasal2_phase7_gpu_owned_scoreinfo_consumer_first1_shadow_consumer_no_go.md"
PREV="$ROOT/docs/fasim_gasal2_phase7_gpu_owned_scoreinfo_consumer_first1_shadow_scaffold.md"
...
if missing:
    raise SystemExit(
        "missing GPU-owned scoreInfo consumer no-go phrase(s):\n"
        + "\n".join(missing)
    )

for forbidden in [
    "accepted_gpu_owned_scoreinfo_consumer = 1",
    "accepted_pre_drop_frontier_certificate = 1",
    ...
    if forbidden in text:
        raise SystemExit(
            f"GPU-owned scoreInfo consumer no-go doc contains forbidden phrase: {forbidden}"
        )

if "next_valid_gate = phase7_gpu_owned_scoreinfo_consumer_first1_shadow_consumer_or_no_go" not in prev_text:
    raise SystemExit("previous GPU-owned shadow scaffold does not point to consumer/no-go")

link = "docs/fasim_gasal2_phase7_gpu_owned_scoreinfo_consumer_first1_shadow_consumer_no_go.md"
next_gate = "path_a_scope_acceptance_or_different_gpu_execution_design_after_gpu_owned_consumer_no_go"
next_pr = "fasim_path_a_scope_acceptance_or_different_gpu_execution_design_after_gpu_owned_consumer_no_go"
advanced_gate = "phase7_gasal2_full_align_verifier_design_spec_or_path_a_acceptance"
advanced_pr = "fasim_gasal2_full_align_verifier_design_spec_or_path_a_acceptance"
later_gate = "phase7_gasal2_full_align_verifier_first1_fail_closed_shadow_scaffold"
later_pr = "fasim_gasal2_full_align_verifier_first1_fail_closed_shadow_scaffold"
for name, content in [
    ...
    if link not in content:
        raise SystemExit(f"{name} does not link GPU-owned scoreInfo consumer no-go checkpoint")
    if (
        ...
        raise SystemExit(
            f"{name} cursor was not advanced to the post-GPU-owned fork gate or a later accepted gate"
        )
    if (
        ...
        raise SystemExit(
            f"{name} next PR was not advanced to the post-GPU-owned fork or a later accepted PR"
        )
...
for phrase in [
    ...
    "path_b_gpu_owned_scoreinfo_consumer_family_stopped",
...
target = "check-fasim-gasal2-roadmap-phase7-gpu-owned-scoreinfo-consumer-first1-shadow-consumer-no-go:"
...
target_name = "check-fasim-gasal2-roadmap-phase7-gpu-owned-scoreinfo-consumer-first1-shadow-consumer-no-go"
if target_name not in current_state_line:
    raise SystemExit("current-state aggregate does not include GPU-owned scoreInfo consumer no-go target")

print("phase7_gpu_owned_scoreinfo_consumer_first1_shadow_consumer_no_go=recorded")
print("phase7_gpu_owned_scoreinfo_consumer_first1_shadow_consumer_status=no_go_no_valid_pre_drop_certificate")
print("path_b_gpu_owned_scoreinfo_consumer_family_stopped=1")
print("next_valid_gate=path_a_scope_acceptance_or_different_gpu_execution_design_after_gpu_owned_consumer_no_go")
