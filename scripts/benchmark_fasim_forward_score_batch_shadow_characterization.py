...
RAW_FIELDS = [
    ...
    "batch_shadow_enabled",
    "batch_requests",
    "batch_cells",
    "batch_cpu_reference_seconds",
    "batch_pack_seconds",
    "batch_h2d_seconds",
    "batch_kernel_seconds",
    "batch_d2h_seconds",
    "batch_unpack_seconds",
    "batch_total_seconds",
    "batch_processed_requests",
    "batch_processed_fraction",
    "batch_total_vs_observed_cpu_reference",
    "batch_processed_cpu_reference_est_seconds",
    "batch_total_vs_processed_cpu_reference_est",
    "batch_score_mismatches",
    "batch_endpoint_mismatches",
    "batch_unsupported_requests",
    "report_path",
]
...
def row_from_report(
    *,
    ...
    cpu_reference = _num(
        telemetry.get("fasim_forward_score_batch_cpu_reference_seconds")
    )
    batch_total = _num(telemetry.get("fasim_forward_score_batch_total_seconds"))
    batch_requests = _int(telemetry.get("fasim_forward_score_batch_requests"))
    unsupported = _int(telemetry.get("fasim_forward_score_batch_unsupported_requests"))
    processed_requests = max(0, batch_requests - unsupported)
    processed_fraction = _safe_ratio(float(processed_requests), float(batch_requests))
    ...
    return {
        ...
        "batch_shadow_enabled": _int(
            telemetry.get("fasim_forward_score_batch_shadow_enabled")
        ),
        "batch_requests": batch_requests,
        "batch_cells": _int(telemetry.get("fasim_forward_score_batch_cells")),
        "batch_cpu_reference_seconds": cpu_reference,
        "batch_pack_seconds": _num(telemetry.get("fasim_forward_score_batch_pack_seconds")),
        "batch_h2d_seconds": _num(telemetry.get("fasim_forward_score_batch_h2d_seconds")),
        "batch_kernel_seconds": _num(
            telemetry.get("fasim_forward_score_batch_kernel_seconds")
        ),
        "batch_d2h_seconds": _num(telemetry.get("fasim_forward_score_batch_d2h_seconds")),
        "batch_unpack_seconds": _num(
            telemetry.get("fasim_forward_score_batch_unpack_seconds")
        ),
        "batch_total_seconds": batch_total,
        "batch_processed_requests": processed_requests,
        "batch_processed_fraction": processed_fraction,
        "batch_total_vs_observed_cpu_reference": _safe_ratio(batch_total, cpu_reference),
        "batch_processed_cpu_reference_est_seconds": processed_cpu_reference_est,
        "batch_total_vs_processed_cpu_reference_est": _safe_ratio(
            batch_total,
            processed_cpu_reference_est,
        ),
        "batch_score_mismatches": _int(
            telemetry.get("fasim_forward_score_batch_score_mismatches")
        ),
        "batch_endpoint_mismatches": _int(
            telemetry.get("fasim_forward_score_batch_endpoint_mismatches")
        ),
        "batch_unsupported_requests": unsupported,
...
def _run_one(
    *,
    ...
    cmd = [
        ...
        "--env",
        f"FASIM_ALIGN_FORWARD_SCORE_GPU_BATCH_SHADOW_MAX_REQUESTS={max_requests}",
    ]
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Characterize current-base batched forward score GPU shadow.",
    )
...
def main() -> int:
    ...
    (work_dir / "summary.json").write_text(
        json.dumps(
            {
                "schema_version": 1,
                "mode": "fasim_forward_score_batch_shadow_characterization",
                "active_env": {
                    "FASIM_ENABLE_PREALIGN_CUDA": "1",
