...
DOC="$ROOT/docs/fasim_gasal2_phase7_gasal2_full_align_verifier_first1_shadow_scaffold.md"
PREV="$ROOT/docs/fasim_gasal2_phase7_gasal2_full_align_verifier_design_spec_or_path_a_acceptance.md"
...
if missing:
    raise SystemExit(
        "missing full-align verifier first1 shadow scaffold phrase(s):\n"
        + "\n".join(missing)
    )
...
for forbidden in [
    ...
    if forbidden in text:
        raise SystemExit(f"full-align verifier scaffold doc contains forbidden phrase: {forbidden}")

if "next_valid_gate = phase7_gasal2_full_align_verifier_first1_fail_closed_shadow_scaffold" not in prev_text:
    raise SystemExit("previous spec doc no longer points to full-align verifier first1 shadow scaffold")

link = "docs/fasim_gasal2_phase7_gasal2_full_align_verifier_first1_shadow_scaffold.md"
for label, path in [
    ("roadmap", roadmap),
    ...
    if link not in content:
        raise SystemExit(f"{label} does not link full-align verifier first1 shadow scaffold")
    if "phase7_gasal2_full_align_verifier_first1_shadow_consumer_or_no_go" not in content:
...
for phrase in [
    ...
    "phase7_full_align_verifier_runtime_reduction_enabled",
    "phase7_full_align_verifier_runtime_work_drop_enabled",
    "phase7_full_align_verifier_gate_first1_shadow_pass",
...
for target in [
    ...
    "check-fasim-gasal2-roadmap-phase7-gasal2-full-align-verifier-first1-shadow-scaffold:",
]:
...
if "check-fasim-gasal2-roadmap-phase7-gasal2-full-align-verifier-first1-shadow-scaffold" not in current_state_line:
    raise SystemExit("current-state aggregate does not include full-align verifier scaffold target")

print("phase7_gasal2_full_align_verifier_first1_shadow_scaffold=fail_closed_shadow")
print("phase7_full_align_verifier_first1_shadow_scaffold_status=fail_closed_no_runtime_reduction")
...
print("next_valid_gate=phase7_gasal2_full_align_verifier_first1_shadow_consumer_or_no_go")
