...
DOC="$ROOT/docs/fasim_gasal2_path_a_scope_acceptance_or_different_gpu_execution_design_after_gpu_upper_bound_reject_certificate_no_go.md"
PREV="$ROOT/docs/fasim_gasal2_phase7_gpu_upper_bound_reject_certificate_first1_shadow_consumer_no_go.md"
...
required = [
    "# Fasim GASAL2 Path A Scope Acceptance Or Different GPU Execution Design After GPU Upper-Bound Reject Certificate No-Go",
    "path_a_scope_acceptance_or_different_gpu_execution_design_after_gpu_upper_bound_reject_certificate_no_go = recorded",
    "previous_checkpoint = docs/fasim_gasal2_phase7_gpu_upper_bound_reject_certificate_first1_shadow_consumer_no_go.md",
    "previous_gate = path_a_scope_acceptance_or_different_gpu_execution_design_after_gpu_upper_bound_reject_certificate_no_go",
    ...
    "gate_first1_pass = 0",
    "stopped_family = gpu_upper_bound_reject_certificate_engine",
    "do_not_continue_upper_bound_reject_shadow_as_runtime = 1",
    "do_not_relabel_zero_reject_accounting_as_work_drop = 1",
    "do_not_run_first64_from_failed_upper_bound_consumer = 1",
    ...
    "no promotion of the stopped upper-bound reject family",
...
if missing:
    raise SystemExit(
        "missing post-upper-bound-reject fork phrase(s):\n" + "\n".join(missing)
    )
...
for forbidden in [
    ...
    if forbidden in text:
        raise SystemExit(f"post-upper-bound-reject fork doc contains forbidden phrase: {forbidden}")

old_gate = "path_a_scope_acceptance_or_different_gpu_execution_design_after_gpu_upper_bound_reject_certificate_no_go"
old_pr = "fasim_path_a_scope_acceptance_or_different_gpu_execution_design_after_gpu_upper_bound_reject_certificate_no_go"
next_gate = "new_gpu_execution_design_family_spec_or_path_a_acceptance_after_gpu_upper_bound_reject_certificate_no_go"
next_pr = "fasim_new_gpu_execution_design_family_spec_or_path_a_acceptance_after_gpu_upper_bound_reject_certificate_no_go"
advanced_gate = "phase7_gpu_exact_work_unit_compaction_first1_shadow_scaffold"
advanced_pr = "fasim_gpu_exact_work_unit_compaction_first1_shadow_scaffold"
advanced_doc = "docs/fasim_gasal2_phase7_gpu_exact_work_unit_compaction_design_spec_or_path_a_acceptance.md"

if f"next_valid_gate = {old_gate}" not in prev_text:
    raise SystemExit("upper-bound reject consumer no-go does not point to this fork")
if f"current_next_pr = {old_pr}" not in prev_text:
    raise SystemExit("upper-bound reject consumer no-go does not name this fork PR")

link = "docs/fasim_gasal2_path_a_scope_acceptance_or_different_gpu_execution_design_after_gpu_upper_bound_reject_certificate_no_go.md"
...
for label, path in [
    ...
    if link not in content:
        raise SystemExit(f"{label} does not link post-upper-bound-reject fork checkpoint")
    if gate_doc not in content and f"current_gate_document = {advanced_doc}" not in content:
        raise SystemExit(f"{label} does not keep a post-upper-bound checkpoint as current gate document")
    if (
        ...
        raise SystemExit(f"{label} cursor was not advanced to post-upper-bound next design/spec gate")
    if f"current_next_pr = {next_pr}" not in content and f"current_next_pr = {advanced_pr}" not in content:
        raise SystemExit(f"{label} next PR was not advanced to post-upper-bound next design/spec gate")
...
target = "check-fasim-gasal2-roadmap-path-a-scope-acceptance-or-different-gpu-execution-design-after-gpu-upper-bound-reject-certificate-no-go:"
if target not in makefile_text:
    raise SystemExit(f"missing Makefile target: {target}")

target_name = "check-fasim-gasal2-roadmap-path-a-scope-acceptance-or-different-gpu-execution-design-after-gpu-upper-bound-reject-certificate-no-go"
if f"check-fasim-gasal2-roadmap-current-state: {target_name}" not in makefile_text:
    raise SystemExit("current-state aggregate does not include post-upper-bound-reject fork target")

print("path_a_scope_acceptance_or_different_gpu_execution_design_after_gpu_upper_bound_reject_certificate_no_go=recorded")
print("path_b_different_gpu_execution_design_after_gpu_upper_bound_reject_certificate_no_go_status=required_not_defined")
print("path_b_new_design_family_after_gpu_upper_bound_reject_certificate_no_go=undefined")
print("next_valid_gate=new_gpu_execution_design_family_spec_or_path_a_acceptance_after_gpu_upper_bound_reject_certificate_no_go")
