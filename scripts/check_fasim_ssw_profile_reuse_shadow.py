...
def main() -> int:
    ...
    parser.add_argument(
        "--work-dir",
        default=str(ROOT / ".tmp" / "fasim_ssw_profile_reuse_shadow"),
    )
    ...
    workload = WorkloadSpec(
        ...
        "small hg38 soft-mask fixture with SSW profile reuse coverage",
        dna_path=dna_path,
    ...
    shadow = run_once(
        ...
        mode=ModeSpec(
            ...
            "cuda",
            {
                ...
                "FASIM_SSW_PROFILE_REUSE_SHADOW": "1",
                "FASIM_SSW_PROFILE_REUSE_SHADOW_MAX_COMPARE": "128",
            },
        ),
        bin_path=cuda_bin,
        work_dir=work_dir / workload.label / "ssw_profile_reuse_shadow",
        require_profile=True,
    )
    require_digest_match(table, shadow, "ssw_profile_reuse_shadow")
    ...
    if metric_int(shadow.metrics, "fasim_ssw_profile_reuse_shadow_enabled") != 1:
        raise RuntimeError("SSW profile reuse shadow was not enabled")
    if metric_int(shadow.metrics, "fasim_ssw_profile_build_calls") <= 0:
        raise RuntimeError("SSW profile build calls were not recorded")
    if metric_int(shadow.metrics, "fasim_ssw_profile_unique_keys") <= 0:
        raise RuntimeError("SSW profile unique keys were not recorded")
    if metric_int(shadow.metrics, "fasim_ssw_profile_reused_possible_calls") <= 0:
        raise RuntimeError("SSW profile reuse opportunity was not observed")
    if metric_float(shadow.metrics, "fasim_ssw_profile_build_seconds") <= 0.0:
        raise RuntimeError("SSW profile build seconds were not recorded")
    if metric_float(shadow.metrics, "fasim_ssw_profile_est_reuse_saved_seconds") <= 0.0:
        raise RuntimeError("SSW profile reuse estimated savings were not recorded")
    if metric_int(shadow.metrics, "fasim_ssw_profile_shadow_compared") <= 0:
        raise RuntimeError("SSW profile reuse shadow did not compare sampled calls")
    if metric_int(shadow.metrics, "fasim_ssw_profile_shadow_score_mismatches") != 0:
        raise RuntimeError("SSW profile reuse shadow score mismatched")
    if metric_int(shadow.metrics, "fasim_ssw_profile_shadow_endpoint_mismatches") != 0:
        raise RuntimeError("SSW profile reuse shadow endpoint mismatched")
    if metric_int(shadow.metrics, "fasim_ssw_profile_shadow_cigar_mismatches") != 0:
        raise RuntimeError("SSW profile reuse shadow CIGAR mismatched")
    if metric_int(shadow.metrics, "fasim_ssw_profile_shadow_output_digest_mismatches") != 0:
        raise RuntimeError("SSW profile reuse shadow output digest mismatched")

    print("Fasim SSW profile reuse shadow checks passed")
    return 0
