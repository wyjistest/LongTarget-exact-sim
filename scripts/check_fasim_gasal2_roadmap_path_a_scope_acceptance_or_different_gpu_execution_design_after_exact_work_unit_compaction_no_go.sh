...
DOC="$ROOT/docs/fasim_gasal2_path_a_scope_acceptance_or_different_gpu_execution_design_after_exact_work_unit_compaction_no_go.md"
PREV="$ROOT/docs/fasim_gasal2_phase7_gpu_exact_work_unit_compaction_first1_shadow_consumer_no_go.md"
...
if missing:
    raise SystemExit(
        "missing post-exact-work-unit fork phrase(s):\n" + "\n".join(missing)
    )
...
for forbidden in [
    ...
    "not_gpu_exact_work_unit_compaction_continuation = 0",
    ...
    if forbidden in text:
        raise SystemExit(f"post-exact-work-unit fork doc contains forbidden phrase: {forbidden}")

old_gate = "path_a_scope_acceptance_or_different_gpu_execution_design_after_exact_work_unit_compaction_no_go"
old_pr = "fasim_path_a_scope_acceptance_or_different_gpu_execution_design_after_exact_work_unit_compaction_no_go"
next_gate = "new_gpu_execution_design_family_spec_or_path_a_acceptance_after_exact_work_unit_compaction_no_go"
next_pr = "fasim_new_gpu_execution_design_family_spec_or_path_a_acceptance_after_exact_work_unit_compaction_no_go"

if f"next_valid_gate = {old_gate}" not in prev_text:
    raise SystemExit("exact work-unit consumer no-go does not point to this fork")
if f"current_next_pr = {old_pr}" not in prev_text:
    raise SystemExit("exact work-unit consumer no-go does not name this fork PR")

link = "docs/fasim_gasal2_path_a_scope_acceptance_or_different_gpu_execution_design_after_exact_work_unit_compaction_no_go.md"
for label, content in [
    ("roadmap", roadmap_text),
    ("action roadmap", action_text),
]:
    if link not in content:
        raise SystemExit(f"{label} does not link post-exact-work-unit fork checkpoint")
    if f"current_execution_gate = {next_gate}" not in content:
        raise SystemExit(f"{label} cursor was not advanced to post-exact-work-unit next design/spec gate")
    if f"current_next_pr = {next_pr}" not in content:
        raise SystemExit(f"{label} next PR was not advanced to post-exact-work-unit next design/spec gate")
...
target = "check-fasim-gasal2-roadmap-path-a-scope-acceptance-or-different-gpu-execution-design-after-exact-work-unit-compaction-no-go:"
if target not in makefile_text:
    raise SystemExit(f"missing Makefile target: {target}")

target_name = "check-fasim-gasal2-roadmap-path-a-scope-acceptance-or-different-gpu-execution-design-after-exact-work-unit-compaction-no-go"
if f"check-fasim-gasal2-roadmap-current-state: {target_name}" not in makefile_text:
    raise SystemExit("current-state aggregate does not include post-exact-work-unit fork target")

print("path_a_scope_acceptance_or_different_gpu_execution_design_after_exact_work_unit_compaction_no_go=recorded")
print("path_b_different_gpu_execution_design_after_exact_work_unit_compaction_no_go_status=required_not_defined")
print("path_b_new_design_family_after_exact_work_unit_compaction_no_go=undefined")
print("next_valid_gate=new_gpu_execution_design_family_spec_or_path_a_acceptance_after_exact_work_unit_compaction_no_go")
