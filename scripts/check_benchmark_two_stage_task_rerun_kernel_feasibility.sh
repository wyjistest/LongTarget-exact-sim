...
WORK="$ROOT/.tmp/check_benchmark_two_stage_task_rerun_kernel_feasibility"
rm -rf "$WORK"
INPUT="$WORK/input"
OUTPUT="$WORK/output"
mkdir -p "$INPUT"
...
for command in report["commands"]:
    ...
    output_matches = [
        item
        for item in command
        if "LONGTARGET_TWO_STAGE_TASK_RERUN_TASK_OUTPUT_TSV=" in item
    ]
    ...
    output_path = Path(output_matches[0].split("=", 1)[1]).resolve()
    ...
    assert str(output_path).startswith(str(expected_output_root)), output_path
PY
