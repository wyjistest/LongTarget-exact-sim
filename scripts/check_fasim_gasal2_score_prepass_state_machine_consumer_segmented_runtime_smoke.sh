...
WORK="${WORK:-"$ROOT/.tmp/check_fasim_gasal2_score_prepass_state_machine_consumer_segmented_runtime_smoke"}"
...
if [[ "$REPLAY_PROBE_MAX_TASKS" -le 0 ]]; then
  echo "segmented runtime smoke requires REPLAY_PROBE_MAX_TASKS > 0 to bound cost" >&2
  exit 1
fi
