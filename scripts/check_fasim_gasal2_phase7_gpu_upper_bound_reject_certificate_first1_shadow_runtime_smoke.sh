...
WORK="${WORK:-"$ROOT/.tmp/check_fasim_gasal2_phase7_gpu_upper_bound_reject_certificate_first1_shadow_runtime_smoke"}"
...
run_case "$WORK/candidate" \
  FASIM_GASAL2_PHASE7_GPU_UPPER_BOUND_REJECT_FIRST1_SHADOW=1
...
prefix = "benchmark.fasim_gasal2_phase7_gpu_upper_bound_reject_"
...
if digest_for_file(baseline_output) != digest_for_file(candidate_output):
    raise SystemExit("upper-bound reject shadow changed lite output digest")
if baseline_output.read_bytes() != candidate_output.read_bytes():
    raise SystemExit("upper-bound reject shadow changed lite output bytes")
...
require_positive_int(candidate, "upper_bound_descriptors")
require_positive_int(candidate, "upper_bound_certificates")
require_nonnegative_int(candidate, "reject_candidates_shadow")
require_nonnegative_int(candidate, "would_reject_scoreinfo_groups")
require_nonnegative_int(candidate, "would_reject_align_attempts")
require_eq(candidate, "certificate_false_negatives", "0")
require_eq(candidate, "baseline_rows_in_rejected_groups", "0")
require_eq(candidate, "baseline_rows_in_rejected_attempts", "0")
require_eq(candidate, "unsupported_descriptors", "0")
...
print("phase7_gpu_upper_bound_reject_certificate_first1_shadow_runtime_smoke=pass")
print("phase7_gpu_upper_bound_reject_certificate_requested=1")
print("phase7_gpu_upper_bound_reject_certificate_active=1")
print("phase7_gpu_upper_bound_reject_certificate_descriptors_gt_zero=1")
print("phase7_gpu_upper_bound_reject_certificate_certificates_gt_zero=1")
print("phase7_gpu_upper_bound_reject_certificate_runtime_reduction_enabled=0")
print("phase7_gpu_upper_bound_reject_certificate_runtime_work_drop_enabled=0")
