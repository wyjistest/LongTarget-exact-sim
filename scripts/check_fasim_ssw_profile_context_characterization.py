...
def main() -> int:
    ...
    parser.add_argument(
        "--work-dir",
        default=str(ROOT / ".tmp" / "fasim_ssw_profile_context_characterization_check"),
    )
    parser.add_argument(
        "--output",
        default=str(ROOT / ".tmp" / "fasim_ssw_profile_context_characterization_check.md"),
    )
    ...
    cmd = [
        sys.executable,
        str(ROOT / "scripts" / "benchmark_fasim_ssw_profile_context_characterization.py"),
        "--cuda-bin",
        str(cuda_bin),
        "--synthetic-entries",
        "1,8",
    ...
    if proc.returncode != 0:
        raise RuntimeError(
            "Fasim SSW ProfileContext characterization check failed "
            f"with exit {proc.returncode}"
        )
    print("Fasim SSW ProfileContext characterization checks passed")
    return 0
