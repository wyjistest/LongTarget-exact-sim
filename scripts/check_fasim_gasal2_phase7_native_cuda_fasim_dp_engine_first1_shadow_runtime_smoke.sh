...
WORK="${WORK:-"$ROOT/.tmp/check_fasim_gasal2_phase7_native_cuda_fasim_dp_engine_first1_shadow_runtime_smoke"}"
...
run_case "$WORK/candidate" \
  FASIM_GASAL2_PHASE7_NATIVE_CUDA_FASIM_DP_ENGINE_FIRST1_SHADOW=1
...
prefix = "benchmark.fasim_gasal2_phase7_native_cuda_fasim_dp_engine_"
...
if digest_for_file(baseline_output) != digest_for_file(candidate_output):
    raise SystemExit("native CUDA Fasim DP shadow changed lite output digest")
if baseline_output.read_bytes() != candidate_output.read_bytes():
    raise SystemExit("native CUDA Fasim DP shadow changed lite output bytes")
...
require_eq(base, "scoreinfo_prealign_reduced", "0")
require_eq(base, "align_side_reduced", "0")
require_eq(base, "gpu_score_authority", "0")
require_eq(base, "gpu_endpoint_authority", "0")
require_eq(base, "gpu_cigar_traceback_output_authority", "0")
...
tiles = require_positive_int(candidate, "native_scoreinfo_tiles")
require_eq(candidate, "forward_endpoint_witnesses", "0")
require_eq(candidate, "reverse_start_witnesses", "0")
require_eq(candidate, "traceback_cigar_witnesses", "0")
require_eq(candidate, "certificates", "0")
require_eq(candidate, "certificate_false_negatives", "0")
require_same_int(candidate, "missing_required_attempts", "native_scoreinfo_tiles")
require_eq(candidate, "scoreinfo_byte_mismatches", "0")
require_eq(candidate, "endpoint_mismatches", "0")
require_eq(candidate, "reverse_start_mismatches", "0")
...
require_same_int(candidate, "cpu_align_fallbacks", "native_scoreinfo_tiles")
...
if tiles == 0:
    raise SystemExit("native_scoreinfo_tiles unexpectedly zero")

print("phase7_native_cuda_fasim_dp_engine_first1_shadow_runtime_smoke=fail_closed")
print("phase7_native_cuda_fasim_dp_engine_requested=1")
print("phase7_native_cuda_fasim_dp_engine_active=1")
print("phase7_native_cuda_fasim_dp_engine_native_scoreinfo_tiles_gt_zero=1")
print("phase7_native_cuda_fasim_dp_engine_certificates=0")
