...
def row_from_runner_report(
    *,
    ...
    row = {
        ...
        "topk": _int(telemetry.get("fasim_prealign_cuda_topk")),
        "max_tasks": _int(telemetry.get("fasim_prealign_cuda_max_tasks")),
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Characterize preAlign CUDA fallback reason telemetry."
    )
    parser.add_argument(
        "--workload",
        action="append",
    ...
    output = {
        "schema_version": 1,
        "mode": "fasim_prealign_cuda_fallback_taxonomy",
