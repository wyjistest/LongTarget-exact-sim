    ...
    shadow = run_once(
        ...
        mode=ModeSpec(
            ...
            "cuda",
            {
                ...
                "FASIM_ALIGNER_ALIGN_BATCH_SHADOW_MAX_REQUESTS": "256",
            },
        ),
        bin_path=cuda_bin,
        work_dir=work_dir / workload.label / "batch_shadow",
        require_profile=True,
    )
    require_digest_match(table, shadow, "batch_shadow")
    require_metrics(shadow.metrics, REQUIRED_KEYS)

    if metric_int(shadow.metrics, "fasim_align_batch_shadow_enabled") != 1:
        raise RuntimeError("align batch shadow was not enabled")
    if metric_int(shadow.metrics, "fasim_align_batch_shadow_supported") != 1:
        raise RuntimeError("align batch shadow CUDA score/ref-end backend was not supported")
    if metric_int(shadow.metrics, "fasim_align_batch_shadow_requests_total") <= 0:
        raise RuntimeError("align batch shadow did not observe any requests")
    if metric_int(shadow.metrics, "fasim_align_batch_shadow_requests_compared") <= 0:
        raise RuntimeError("align batch shadow did not compare any requests")
    if metric_int(shadow.metrics, "fasim_align_batch_shadow_h2d_bytes") <= 0:
        raise RuntimeError("align batch shadow did not transfer request bytes to GPU")
    if metric_int(shadow.metrics, "fasim_align_batch_shadow_d2h_bytes") <= 0:
        raise RuntimeError("align batch shadow did not transfer result bytes from GPU")
    if metric_float(shadow.metrics, "fasim_align_batch_shadow_kernel_seconds") <= 0.0:
        raise RuntimeError("align batch shadow did not report GPU kernel time")
    if metric_int(shadow.metrics, "fasim_align_batch_shadow_has_score_contract") != 1:
        raise RuntimeError("align batch shadow must report score contract coverage")
    if metric_int(shadow.metrics, "fasim_align_batch_shadow_has_coordinate_contract") != 1:
        raise RuntimeError("align batch shadow must report coordinate contract coverage")
    if metric_int(shadow.metrics, "fasim_align_batch_shadow_has_cigar_contract") != 0:
        raise RuntimeError("initial align batch shadow should not claim CIGAR coverage")
    if metric_int(shadow.metrics, "fasim_align_batch_shadow_total_mismatches") != 0:
        raise RuntimeError("align batch shadow score/coordinate contract mismatched")

    print("Fasim aligner.Align batch shadow checks passed")
    return 0
