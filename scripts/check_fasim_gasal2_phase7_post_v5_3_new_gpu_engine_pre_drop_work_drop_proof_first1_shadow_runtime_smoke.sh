...
WORK="${WORK:-"$ROOT/.tmp/check_fasim_gasal2_phase7_post_v5_3_new_gpu_engine_pre_drop_work_drop_proof_first1_shadow_runtime_smoke"}"
...
run_case "$WORK/candidate" \
  FASIM_GASAL2_PHASE7_POST_V5_3_NEW_GPU_ENGINE_PRE_DROP_WORK_DROP_PROOF_FIRST1_SHADOW=1
...
prefix = (
    ...
    "pre_drop_work_drop_proof_first1_shadow_"
)
...
if digest_for_file(baseline_output) != digest_for_file(candidate_output):
    raise SystemExit("pre-drop proof shadow changed lite output digest")
if baseline_output.read_bytes() != candidate_output.read_bytes():
    raise SystemExit("pre-drop proof shadow changed lite output bytes")
...
require_eq(base, "gate_first1_proof_pass", "0")
...
require_eq(candidate, "uses_pre_drop_output_inert_proof", "0")
...
require_eq(candidate, "proof_must_not_use_top5_only_contract", "1")
require_eq(candidate, "proof_must_cover_complete_row_set", "1")
...
require_eq(candidate, "candidate_proof_false_negatives", "0")
require_eq(candidate, "candidate_proof_missing_required_attempts", "0")
...
require_eq(candidate, "gate_first1_proof_pass", "0")
require_eq(candidate, "gate_first1_pass", "0")

print("phase7_post_v5_3_new_gpu_engine_pre_drop_work_drop_proof_first1_shadow_runtime_smoke=fail_closed")
print("phase7_post_v5_3_new_gpu_engine_pre_drop_work_drop_proof_first1_shadow_requested=1")
print("phase7_post_v5_3_new_gpu_engine_pre_drop_work_drop_proof_first1_shadow_active=1")
