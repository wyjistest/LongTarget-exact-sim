...
WORK="$ROOT/.tmp/fasim_sim_recovery_learned_detector_large_corpus_expansion_check"
...
REPORT="$WORK/large_corpus_expansion_report.md"
LOG="$WORK/large_corpus_expansion.log"
DOC_REPORT="$ROOT/docs/fasim_sim_recovery_learned_detector_large_corpus_expansion.md"
mkdir -p "$WORK"
...
EXTRA_LIMIT="${FASIM_SIM_RECOVERY_LEARNED_DETECTOR_LARGE_CORPUS_EXTRA_MARMOSET_LIMIT:-36}"
...
PYTHONDONTWRITEBYTECODE=1 python3 "$ROOT/scripts/benchmark_fasim_sim_recovery_real_corpus_characterization.py" \
  ...
  --report-title "Fasim SIM-Close Learned Detector Large Corpus Expansion Dataset" \
  --base-branch fasim-sim-recovery-learned-detector-expanded-hard-negative-model-shadow \
...
PYTHONDONTWRITEBYTECODE=1 python3 "$ROOT/scripts/benchmark_fasim_sim_recovery_learned_detector_large_corpus_expansion.py" \
  --dataset "$NEGATIVE_DATASET" \
  --source-dataset "$SOURCE_DATASET" \
...
grep -q '^benchmark\.fasim_sim_recovery_learned_detector_large_corpus_expansion\.total\.enabled=1$' "$LOG"
grep -q "^benchmark\\.fasim_sim_recovery_learned_detector_large_corpus_expansion\\.total\\.extra_marmoset_limit=${EXTRA_LIMIT}$" "$LOG"
grep -Eq '^benchmark\.fasim_sim_recovery_learned_detector_large_corpus_expansion\.total\.rows=[1-9][0-9]*$' "$LOG"
grep -Eq '^benchmark\.fasim_sim_recovery_learned_detector_large_corpus_expansion\.total\.positive_rows=[0-9]+$' "$LOG"
grep -Eq '^benchmark\.fasim_sim_recovery_learned_detector_large_corpus_expansion\.total\.negative_rows=[1-9][0-9]*$' "$LOG"
grep -Eq '^benchmark\.fasim_sim_recovery_learned_detector_large_corpus_expansion\.total\.workload_count=[1-9][0-9]*$' "$LOG"
grep -Eq '^benchmark\.fasim_sim_recovery_learned_detector_large_corpus_expansion\.total\.validate_supported_workload_count=[0-9]+$' "$LOG"
grep -Eq '^benchmark\.fasim_sim_recovery_learned_detector_large_corpus_expansion\.total\.hard_negative_source_count=[1-9][0-9]*$' "$LOG"
grep -Eq '^benchmark\.fasim_sim_recovery_learned_detector_large_corpus_expansion\.total\.growth_vs_expanded_hard_negative_rows=-?[0-9]+$' "$LOG"
grep -q '^benchmark\.fasim_sim_recovery_learned_detector_large_corpus_expansion\.total\.production_model=0$' "$LOG"
grep -q '^benchmark\.fasim_sim_recovery_learned_detector_large_corpus_expansion\.total\.sim_labels_runtime_inputs=0$' "$LOG"
grep -q '^benchmark\.fasim_sim_recovery_learned_detector_large_corpus_expansion\.total\.runtime_behavior_changed=0$' "$LOG"
grep -q '^benchmark\.fasim_sim_recovery_learned_detector_large_corpus_expansion\.total\.model_training_added=0$' "$LOG"
grep -q '^benchmark\.fasim_sim_recovery_learned_detector_large_corpus_expansion\.total\.deep_learning_dependency=0$' "$LOG"
grep -q '^benchmark\.fasim_sim_recovery_learned_detector_large_corpus_expansion\.total\.recommended_default_sim_close=0$' "$LOG"

if [[ -d "$MARMOSET_ROOT" && "$EXTRA_LIMIT" -ge 20 ]]; then
  grep -Eq '^benchmark\.fasim_sim_recovery_learned_detector_large_corpus_expansion\.total\.selected_extra_case_count=([2-9][0-9]|[1-9][0-9]{2,})$' "$LOG"
  grep -Eq '^benchmark\.fasim_sim_recovery_learned_detector_large_corpus_expansion\.total\.rows=([6-9][0-9]|[1-9][0-9]{2,})$' "$LOG"
  grep -Eq '^benchmark\.fasim_sim_recovery_learned_detector_large_corpus_expansion\.total\.growth_vs_expanded_hard_negative_rows=[1-9][0-9]*$' "$LOG"
fi

grep -q '## Large Corpus Expansion' "$REPORT"
grep -q '## Corpus Growth' "$REPORT"
...
grep -q '## Large Corpus Expansion' "$DOC_REPORT"
