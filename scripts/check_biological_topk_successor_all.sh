...
if [[ "$MODE" == "--precommit" ]]; then
  phase="${BIOLOGICAL_TOPK_SUCCESSOR_PHASE:?set BIOLOGICAL_TOPK_SUCCESSOR_PHASE for --precommit}"
  exec python3 "$ROOT/scripts/check_biological_topk_successor_phase.py" \
    --phase "$phase" --mode precommit
fi

mapfile -t phases < <(
  python3 - "$ROOT/paper/biological_topk_successor/PROGRAM_STATE.json" <<'PY'
import json
import sys
...
for phase in "${phases[@]}"; do
  python3 "$ROOT/scripts/check_biological_topk_successor_phase.py" \
    --phase "$phase" --mode postcommit
done

echo "biological Top-K successor aggregate checks OK"
