...
from benchmark_fasim_ssw_profile_cache import CACHE_KEYS  # noqa: E402
...
def delta_seconds(results: Dict[str, List[RunResult]]) -> float:
    return mode_metric(results, "auto", "fasim_total_seconds") - mode_metric(
        results, "auto_cache", "fasim_total_seconds"
    )


def cache_hit_rate(results: Dict[str, List[RunResult]], mode: str = "auto_cache") -> str:
    return percent(
...
def cache_clean(results: Dict[str, List[RunResult]], mode: str) -> bool:
    return all(
...
def align_calls(results: Dict[str, List[RunResult]], mode: str = "auto_cache") -> int:
    return mode_count(results, mode, "fasim_aligner_align_calls")


def require_cache_metrics(all_results: Dict[str, Dict[str, List[RunResult]]]) -> None:
...
def check_results(all_results: Dict[str, Dict[str, List[RunResult]]]) -> None:
    require_cache_metrics(all_results)
    active_workloads = 0
    for workload, results in all_results.items():
        for mode in ["auto", "auto_cache", "auto_cache_validate"]:
            if not digest_matches_reference(results, mode):
                raise RuntimeError(f"{workload}/{mode}: digest does not match table_only")
            if stable_records(results[mode]) != stable_records(results["table_only"]):
                raise RuntimeError(f"{workload}/{mode}: record count does not match table_only")
        for mode in ["auto_cache", "auto_cache_validate"]:
            ...
            if mode_count(results, mode, "fasim_ssw_profile_cache_calls") <= 0:
                raise RuntimeError(f"{workload}/{mode}: SSW profile cache saw no calls")
            if mode_count(results, mode, "fasim_ssw_profile_cache_hits") <= 0:
                raise RuntimeError(f"{workload}/{mode}: SSW profile cache saw no hits")
            if not cache_clean(results, mode):
                raise RuntimeError(f"{workload}/{mode}: cache validation metrics are not clean")
    if active_workloads <= 0:
        raise RuntimeError("no characterized workload exercised SSW profile cache calls")
...
def render_report(
    *,
    ...
    lines: List[str] = []
    lines.append("# Fasim SSW Profile Cache Characterization")
    lines.append("")
    lines.append(
        "This report repeatedly characterizes the default-off real SSW query-profile "
        "cache from `FASIM_SSW_PROFILE_CACHE=1`. It adds no optimization logic and "
        "does not change defaults. Validation mode uses `FASIM_SSW_PROFILE_CACHE_VALIDATE=1` "
        "to rebuild the legacy profile and fall back on any score, endpoint, or CIGAR mismatch."
    )
    lines.append("")
    ...
    append_table(
        ...
        [
            [
                workload.label,
                fmt_int(stable_records(all_results[workload.label]["table_only"])),
                fmt_seconds(mode_metric(all_results[workload.label], "table_only", "fasim_total_seconds")),
                fmt_seconds(mode_metric(all_results[workload.label], "auto", "fasim_total_seconds")),
                fmt_seconds(mode_metric(all_results[workload.label], "auto_cache", "fasim_total_seconds")),
                fmt_seconds(delta_seconds(all_results[workload.label])),
                fmt_speedup(
                    speedup(
                        mode_metric(all_results[workload.label], "table_only", "fasim_total_seconds"),
                        mode_metric(all_results[workload.label], "auto_cache", "fasim_total_seconds"),
                    )
                ),
                fmt_speedup(
                    speedup(
                        mode_metric(all_results[workload.label], "auto", "fasim_total_seconds"),
                        mode_metric(all_results[workload.label], "auto_cache", "fasim_total_seconds"),
                    )
                ),
                "yes"
    ...
    append_table(
        ...
        [
            [
                workload.label,
                fmt_int(mode_count(all_results[workload.label], "auto_cache", "fasim_ssw_profile_cache_calls")),
                fmt_int(align_calls(all_results[workload.label])),
                fmt_int(mode_count(all_results[workload.label], "auto_cache", "fasim_ssw_profile_cache_hits")),
                fmt_int(mode_count(all_results[workload.label], "auto_cache", "fasim_ssw_profile_cache_misses")),
                cache_hit_rate(all_results[workload.label]),
                fmt_int(mode_count(all_results[workload.label], "auto_cache", "fasim_ssw_profile_cache_unique_keys")),
                fmt_seconds(
                    mode_metric(
    ...
    clean = all(
        digest_matches_reference(all_results[workload.label], "auto_cache")
        and digest_matches_reference(all_results[workload.label], "auto_cache_validate")
        and (
            align_calls(all_results[workload.label], "auto_cache_validate") <= 0
            or cache_clean(all_results[workload.label], "auto_cache_validate")
        )
        for workload in workload_list
    )
    wins = [workload.label for workload in workload_list if delta_seconds(all_results[workload.label]) > 0.0]
    losses = [workload.label for workload in workload_list if delta_seconds(all_results[workload.label]) <= 0.0]
    lines.append("## Decision")
    lines.append("")
    if not clean:
        lines.append("Stop: at least one workload had digest, mismatch, or fallback telemetry.")
    elif wins and not losses:
        lines.append("The cache is exact-clean and faster across all characterized workloads.")
    elif wins:
        lines.append(
            "The cache is exact-clean and faster on "
            f"{', '.join(wins)}. Workloads without a median runtime win: {', '.join(losses)}."
        )
    else:
        lines.append("The cache is exact-clean, but no characterized workload showed a median runtime win.")
    lines.append("")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--work-dir",
        default=str(ROOT / ".tmp" / "fasim_ssw_profile_cache_characterization"),
    )
    parser.add_argument(
        "--output",
        default=str(ROOT / "docs" / "fasim_ssw_profile_cache_characterization.md"),
    )
