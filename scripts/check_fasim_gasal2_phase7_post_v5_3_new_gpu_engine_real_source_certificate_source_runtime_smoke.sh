...
WORK="${WORK:-"$ROOT/.tmp/check_fasim_gasal2_phase7_post_v5_3_new_gpu_engine_real_source_certificate_source_runtime_smoke"}"
...
run_case "$WORK/candidate" \
  FASIM_GASAL2_PHASE7_POST_V5_3_NEW_GPU_ENGINE_REAL_SOURCE_CERTIFICATE_SOURCE=1
...
prefix = (
    ...
    "real_source_certificate_source_"
)
...
if digest_for_file(baseline_output) != digest_for_file(candidate_output):
    raise SystemExit("real-source certificate source changed lite output digest")
if baseline_output.read_bytes() != candidate_output.read_bytes():
    raise SystemExit("real-source certificate source changed lite output bytes")
...
require_eq(base, "gate_first1_source_pass", "0")

require_eq(candidate, "requested", "1")
require_eq(candidate, "active", "1")
require_eq(candidate, "real_fasim_runtime_certificate_source", "1")
require_eq(candidate, "real_fasim_runtime_work_drop_path", "0")
require_eq(candidate, "runtime_certificate_is_synthetic", "0")
require_eq(candidate, "source_is_pre_drop", "1")
require_eq(candidate, "source_is_legacy_byte_cuda", "1")
require_eq(candidate, "candidate_uses_final_cpu_output_as_runtime_proof", "0")
require_positive_int(candidate, "source_task_count")
require_positive_int(candidate, "source_scoreinfo_count")
require_positive_int(candidate, "source_attempt_count")
require_positive_int(candidate, "reference_scoreinfo_count")
require_positive_int(candidate, "reference_attempt_count")
require_eq(candidate, "missing_certificate", "0")
...
require_eq(candidate, "fallback_accounting_clean", "1")
...
require_eq(candidate, "gate_first1_source_pass", "1")
require_eq(candidate, "gate_first1_pass", "0")

print("phase7_post_v5_3_new_gpu_engine_real_source_certificate_source_runtime_smoke=gate_first1_source_pass")
print("phase7_post_v5_3_new_gpu_engine_real_source_certificate_source_requested=1")
print("phase7_post_v5_3_new_gpu_engine_real_source_certificate_source_active=1")
print("phase7_post_v5_3_new_gpu_engine_real_source_certificate_source_real_fasim_runtime_certificate_source=1")
print("phase7_post_v5_3_new_gpu_engine_real_source_certificate_source_runtime_reduction_enabled=0")
print("phase7_post_v5_3_new_gpu_engine_real_source_certificate_source_gate_first1_source_pass=1")
