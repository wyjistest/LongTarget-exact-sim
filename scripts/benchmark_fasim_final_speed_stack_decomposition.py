...
def render_report(
    *,
    ...
    lines: List[str] = []
    lines.append("# Fasim Final Speed Stack Decomposition")
    lines.append("")
    lines.append(
        "This telemetry-only report decomposes the fastest current opt-in Fasim "
        "speed stack after AVX2 forward-only and SSW ProfileContext were "
        "characterized. It does not add optimization logic or change output, "
        "scoring, threshold, non-overlap, GPU AUTO, SIM-close, recovery, or "
        "validation behavior."
    )
    lines.append("")
    ...
    append_table(
        ...
        [
            ...
            row(
                ...
                scope="post-GPU",
                notes="CPU exact-column extension after compact GPU scoring",
            ),
            row(
                results,
    ...
    candidates = [
        ...
        ("exact-column extend", mode_metric(results, "final_stack", "fasim_gpu_dp_column_exact_column_extend_seconds")),
    ...
    elif winner == "exact-column extend":
        recommendation = "Revisit batched exact-column extend as a shadow, keeping CPU output authority."
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--work-dir",
        default=str(ROOT / ".tmp" / "fasim_final_speed_stack_decomposition"),
    )
    parser.add_argument(
        "--output",
        default=str(ROOT / "docs" / "fasim_final_speed_stack_decomposition.md"),
    )
