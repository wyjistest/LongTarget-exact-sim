...
WORK="${WORK:-"$ROOT/.tmp/check_fasim_gasal2_phase7_v3_narrow_certificate_runtime_smoke"}"
...
if value(base, "requested") != "0" or value(base, "active") != "0":
    raise SystemExit("v3 narrow certificate must be default-off")
if value(candidate, "requested") != "1":
    raise SystemExit("v3 narrow certificate was not requested")
if value(candidate, "active") != "1":
    raise SystemExit("v3 narrow certificate did not become active")
if value(candidate, "pre_scoreinfo_source") != "1":
    raise SystemExit("v3 narrow certificate must be pre-scoreInfo")
if value(candidate, "after_cpu_scoreinfo_source") != "0":
    raise SystemExit("v3 narrow certificate must not be after CPU scoreInfo")
...
if candidate_scoreinfos <= 0:
    raise SystemExit("v3 narrow certificate must emit candidate scoreInfo descriptors")
if candidate_attempts <= 0:
    raise SystemExit("v3 narrow certificate must emit candidate attempt descriptors")
if candidate_attempts >= 168_730_848:
    raise SystemExit("v3 narrow certificate must be narrower than all-column replay")

if value(candidate, "candidate_certificate_checked") != "1":
    raise SystemExit("v3 narrow certificate must mark certificate checked")
if int(value(candidate, "candidate_certificate_false_negatives")) <= 0:
    raise SystemExit("current narrow probe must fail closed with scoreInfo false negatives")
if int(value(candidate, "missing_required_attempts")) <= 0:
    raise SystemExit("current narrow probe must fail closed with missing required attempts")
if value(candidate, "cpu_scoreinfo_reduced") != "1":
    raise SystemExit("v3 narrow certificate candidate path must mark CPU scoreInfo reduction")
if int(value(candidate, "cpu_scoreinfo_calls")) != 0:
    raise SystemExit("v3 narrow certificate candidate path must not call CPU scoreInfo")
if int(value(candidate, "baseline_cpu_scoreinfo_calls")) <= 0:
    raise SystemExit("v3 narrow certificate must report a positive CPU baseline")

print("phase7_v3_narrow_certificate_runtime_smoke=bounded_probe_no_go_missing_certificate")
