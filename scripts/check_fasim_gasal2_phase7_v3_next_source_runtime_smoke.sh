...
WORK="${WORK:-"$ROOT/.tmp/check_fasim_gasal2_phase7_v3_next_source_runtime_smoke"}"
...
run_case "$WORK/candidate" \
  FASIM_GASAL2_PHASE7_V3_SEED_CERTIFICATE_SOURCE=1
...
if value(base, "requested") != "0" or value(base, "active") != "0":
    raise SystemExit("v3 next-source seed certificate must be default-off")
if value(candidate, "requested") != "1":
    raise SystemExit("v3 next-source seed certificate was not requested")
if value(candidate, "active") != "1":
    raise SystemExit("v3 next-source seed certificate did not become active")
if value(candidate, "pre_scoreinfo_source") != "1":
    raise SystemExit("v3 next-source seed certificate must be pre-scoreInfo")
if value(candidate, "after_cpu_scoreinfo_source") != "0":
    raise SystemExit("v3 next-source seed certificate must not run after CPU scoreInfo")

candidate_scoreinfos = int(value(candidate, "candidate_scoreinfos"))
candidate_attempts = int(value(candidate, "candidate_attempts"))
if candidate_scoreinfos <= 0:
    raise SystemExit("v3 next-source seed certificate must emit candidate scoreInfos")
if candidate_attempts <= 0:
    raise SystemExit("v3 next-source seed certificate must emit candidate attempts")
if candidate_attempts >= 168_730_848:
    raise SystemExit("v3 next-source seed certificate must stay below all-column replay scale")

if value(candidate, "candidate_certificate_checked") != "1":
    raise SystemExit("v3 next-source seed certificate must mark certificate checked")
if int(value(candidate, "candidate_certificate_false_negatives")) <= 0:
    raise SystemExit("current seed certificate smoke must fail closed with false negatives")
if int(value(candidate, "missing_required_attempts")) <= 0:
    raise SystemExit("current seed certificate smoke must fail closed with missing attempts")
if value(candidate, "cpu_scoreinfo_reduced") != "1":
    raise SystemExit("v3 next-source seed certificate must reduce CPU scoreInfo calls")
if int(value(candidate, "cpu_scoreinfo_calls")) != 0:
    raise SystemExit("v3 next-source seed certificate candidate path must not call CPU scoreInfo")
if int(value(candidate, "baseline_cpu_scoreinfo_calls")) <= 0:
    raise SystemExit("v3 next-source seed certificate must report a positive CPU baseline")

print("phase7_v3_next_source_runtime_smoke=seed_certificate_fail_closed")
...
print("phase7_v3_descriptor_source_candidate_certificate_false_negatives_gt_zero=1")
