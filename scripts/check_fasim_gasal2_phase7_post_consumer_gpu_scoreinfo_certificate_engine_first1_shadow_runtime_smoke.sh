...
WORK="${WORK:-"$ROOT/.tmp/check_fasim_gasal2_phase7_post_consumer_gpu_scoreinfo_certificate_engine_first1_shadow_runtime_smoke"}"
...
prefix = "benchmark.fasim_gasal2_phase7_post_consumer_gpu_scoreinfo_cert_engine_first1_shadow_"
...
if digest_for_file(baseline_output) != digest_for_file(candidate_output):
    raise SystemExit("post-consumer certificate-engine shadow changed lite output digest")
if baseline_output.read_bytes() != candidate_output.read_bytes():
    raise SystemExit("post-consumer certificate-engine shadow changed lite output bytes")
...
require_eq(candidate, "scoreinfo_cert_engine_first1_shadow", "1")
require_eq(candidate, "real_fasim_runtime_certificate_source", "1")
require_eq(candidate, "certificate_valid_before_work_drop", "0")
require_eq(candidate, "certificate_valid_before_d2h", "0")
...
require_positive_int(candidate, "gpu_scoreinfo_groups")
require_positive_int(candidate, "gpu_attempt_frontier_attempts")
require_positive_int(candidate, "gpu_selected_replay_attempts")
require_eq(candidate, "gpu_skipped_scoreinfo_groups", "0")
...
print("phase7_post_consumer_gpu_scoreinfo_certificate_engine_first1_shadow_runtime_smoke=fail_closed")
print("phase7_post_consumer_gpu_scoreinfo_certificate_engine_first1_shadow_requested=1")
print("phase7_post_consumer_gpu_scoreinfo_certificate_engine_first1_shadow_active=1")
