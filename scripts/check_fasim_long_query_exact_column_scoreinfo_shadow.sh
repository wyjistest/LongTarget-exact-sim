...
WORK="${WORK:-"$ROOT/.tmp/check_fasim_long_query_exact_column_scoreinfo_shadow"}"
...
if [[ "$baseline_digest" != "$candidate_digest" ]]; then
  echo "long-query exact-column scoreInfo shadow changed lite output digest" >&2
...
tasks="$(metric benchmark.fasim_long_query_exact_column_scoreinfo_shadow_tasks)"
...
fallback_batches="$(metric benchmark.fasim_long_query_exact_column_scoreinfo_shadow_fallback_batches)"
overflow_batches="$(metric benchmark.fasim_long_query_exact_column_scoreinfo_shadow_overflow_batches)"
...
for value_name in \
  requested active query_len tasks gpu_batches gpu_tasks fallback_batches \
  overflow_batches scoreinfo_mismatches error required_smem default_smem_limit \
  optin_smem_limit resource_fit; do
  if [[ -z "${!value_name}" ]]; then
    echo "missing exact-column scoreInfo shadow metric: $value_name" >&2
    exit 1
  fi
done
...
if [[ "$required_smem" -le "$default_smem_limit" ]]; then
  echo "expected MALAT1 exact-column required_smem to exceed default limit, got required=$required_smem default=$default_smem_limit" >&2
  exit 1
fi
if [[ "$required_smem" -gt "$optin_smem_limit" ]]; then
  echo "expected MALAT1 exact-column required_smem to fit opt-in limit, got required=$required_smem optin=$optin_smem_limit" >&2
  exit 1
fi
if [[ "$resource_fit" != "1" ]]; then
  echo "expected resource_fit=1 for MALAT1 opt-in resource boundary, got $resource_fit" >&2
  exit 1
fi
if [[ "$active" == "1" ]]; then
  if [[ "$gpu_batches" -le 0 || "$gpu_tasks" -le 0 ]]; then
    echo "active shadow should report GPU batches/tasks, got batches=$gpu_batches tasks=$gpu_tasks" >&2
    exit 1
  fi
  if [[ "$scoreinfo_mismatches" != "0" ]]; then
    echo "active shadow produced scoreInfo mismatches=$scoreinfo_mismatches" >&2
    exit 1
  fi
else
  if [[ "$fallback_batches" -le 0 && "$error" == "none" ]]; then
    echo "inactive shadow must report fallback or explanatory error" >&2
    exit 1
  fi
fi
...
printf 'gpu_tasks=%s\n' "$gpu_tasks"
printf 'fallback_batches=%s\n' "$fallback_batches"
printf 'overflow_batches=%s\n' "$overflow_batches"
