...
inline uint64_t countUniqueSimCandidateStateStartCoords(const vector<SimScanCudaCandidateState> &states)
{
  if(states.empty())
  {
    return 0;
  }
  vector<uint64_t> startCoords;
  startCoords.reserve(states.size());
  for(size_t stateIndex = 0; stateIndex < states.size(); ++stateIndex)
  {
    startCoords.push_back(simScanCudaCandidateStateStartCoord(states[stateIndex]));
  }
  sort(startCoords.begin(),startCoords.end());
  return static_cast<uint64_t>(unique(startCoords.begin(),startCoords.end()) -
                               startCoords.begin());
}
...
inline bool simSafeStoreMergeStructureShadowEnabledRuntime()
{
  static const bool enabled = []()
  {
    const char *env = getenv("LONGTARGET_SIM_CUDA_SAFE_STORE_MERGE_STRUCTURE_SHADOW");
    return env != NULL && env[0] != '\0' && strcmp(env,"0") != 0;
  }();
  return enabled;
}
...
inline bool simSafeWorksetDeferredIndexEnabledRuntime()
{
  static const bool enabled = []()
  {
    const char *env = getenv("LONGTARGET_SIM_CUDA_SAFE_WORKSET_DEFERRED_INDEX");
    return env != NULL && env[0] != '\0' && strcmp(env,"0") != 0;
  }();
  return enabled;
}

inline bool simSafeWorksetDeferredIndexValidateEnabledRuntime()
{
  static const bool enabled = []()
  {
    const char *env = getenv("LONGTARGET_SIM_CUDA_SAFE_WORKSET_DEFERRED_INDEX_VALIDATE");
    return env != NULL && env[0] != '\0' && strcmp(env,"0") != 0;
  }();
  return enabled;
}
...
inline bool simCudaInitialSafeStoreGpuPrecombineShadowEnabledRuntime()
{
  static const bool enabled = []()
  {
    const char *env = getenv("LONGTARGET_SIM_CUDA_INITIAL_SAFE_STORE_GPU_PRECOMBINE_SHADOW");
    return env != NULL && env[0] != '\0' && strcmp(env,"0") != 0;
  }();
  return enabled;
}
...
inline bool simCudaInitialSafeStoreGpuPrecombinePruneShadowEnabledRuntime()
{
  static const bool enabled = []()
  {
    const char *env = getenv("LONGTARGET_SIM_CUDA_INITIAL_SAFE_STORE_GPU_PRECOMBINE_PRUNE_SHADOW");
    return env != NULL && env[0] != '\0' && strcmp(env,"0") != 0;
  }();
  return enabled;
}

inline bool simCudaInitialSafeStoreGpuPrecombinePruneRequestedRuntime()
{
  static const bool enabled = []()
  {
    const char *env = getenv("LONGTARGET_SIM_CUDA_INITIAL_SAFE_STORE_GPU_PRECOMBINE_PRUNE");
    return env != NULL && env[0] != '\0' && strcmp(env,"0") != 0;
  }();
  return enabled;
}

inline bool simCudaInitialSafeStoreGpuPrecombinePruneValidateEnabledRuntime()
{
  static const bool enabled = []()
  {
    const char *env = getenv("LONGTARGET_SIM_CUDA_INITIAL_SAFE_STORE_GPU_PRECOMBINE_PRUNE_VALIDATE");
    return env != NULL && env[0] != '\0' && strcmp(env,"0") != 0;
  }();
  return enabled;
}
...
inline bool simCudaInitialSafeStorePruneIndexShadowEnabledRuntime()
{
  static const bool enabled = []()
  {
    const char *env = getenv("LONGTARGET_SIM_CUDA_INITIAL_SAFE_STORE_PRUNE_INDEX_SHADOW");
    return env != NULL && env[0] != '\0' && strcmp(env,"0") != 0;
  }();
  return enabled;
}
...
inline bool simCudaInitialSafeStoreDeviceMaintenanceEnabledRuntime()
{
  return simCudaInitialSafeStoreHandoffEnabledRuntime();
			  ...
			  if(env == NULL || env[0] == '\0')
			  {
				...
				inline bool simCudaInitialExactFrontierOneChunkBoundedShadowRequestedRuntime()
				{
				  const char *env = getenv("LONGTARGET_SIM_CUDA_INITIAL_EXACT_FRONTIER_ONE_CHUNK_BOUNDED_SHADOW");
				  return env != NULL && env[0] != '\0' && env[0] != '0';
				}

					inline bool simCudaInitialExactFrontierPerRequestShadowRequestedRuntime()
					{
					  const char *env = getenv("LONGTARGET_SIM_CUDA_INITIAL_EXACT_FRONTIER_PER_REQUEST_SHADOW");
					  return env != NULL && env[0] != '\0' && env[0] != '0';
					}

					inline bool simCudaInitialExactFrontierPerRequestShadowResidentSourceRequestedRuntime()
					{
					  const char *env =
					    getenv("LONGTARGET_SIM_CUDA_INITIAL_EXACT_FRONTIER_PER_REQUEST_SHADOW_RESIDENT_SOURCE");
					  return env != NULL && env[0] != '\0' && env[0] != '0';
					}
	...
	inline bool simCudaValidateEnabledRuntime()
	{
	  ...
	  else
	  {
		...
		enum SimSafeWorksetMergeBreakdownField
		{
		  SIM_SAFE_WORKSET_MERGE_MATERIALIZE_NANOSECONDS = 0,
		  SIM_SAFE_WORKSET_MERGE_CANDIDATE_ERASE_NANOSECONDS = 1,
		  SIM_SAFE_WORKSET_MERGE_SAFE_STORE_UPSERT_NANOSECONDS = 2,
		  SIM_SAFE_WORKSET_MERGE_CANDIDATE_APPLY_NANOSECONDS = 3,
		  SIM_SAFE_WORKSET_MERGE_SAFE_STORE_PRUNE_NANOSECONDS = 4,
		  SIM_SAFE_WORKSET_MERGE_SAFE_STORE_UPLOAD_NANOSECONDS = 5,
		  SIM_SAFE_WORKSET_MERGE_UNIQUE_START_KEY_COUNT = 6,
		  SIM_SAFE_WORKSET_MERGE_DUPLICATE_STATE_COUNT = 7,
		...
		{
		  uint64_t values[SIM_SAFE_WORKSET_MERGE_BREAKDOWN_FIELD_COUNT];

		  SimSafeWorksetMergeBreakdownStats()
		  {
		  ...
		  uint64_t get(SimSafeWorksetMergeBreakdownField field) const
		  {
		    return values[static_cast<size_t>(field)];
		  }

		  void add(SimSafeWorksetMergeBreakdownField field,uint64_t value)
		  {
		    values[static_cast<size_t>(field)] += value;
		  }
		};

		struct SimSafeWorksetMergeBreakdownAtomicStats
		{
		  std::atomic<uint64_t> values[SIM_SAFE_WORKSET_MERGE_BREAKDOWN_FIELD_COUNT];

		  SimSafeWorksetMergeBreakdownAtomicStats()
		  {
		    for(size_t fieldIndex = 0;
		        fieldIndex < SIM_SAFE_WORKSET_MERGE_BREAKDOWN_FIELD_COUNT;
		        ++fieldIndex)
		    {
		      values[fieldIndex].store(0,std::memory_order_relaxed);
		    }
		  }
		};

		inline SimSafeWorksetMergeBreakdownAtomicStats &simSafeWorksetMergeBreakdownAtomicStats()
		{
		  static SimSafeWorksetMergeBreakdownAtomicStats stats;
		  return stats;
		}

		struct SimSafeStoreMergeStructureShadowStats
		{
		  SimSafeStoreMergeStructureShadowStats():
		    calls(0),
		    nanoseconds(0),
		    digestMismatches(0),
		    sizeMismatches(0),
		...
		{
		  SimSafeStoreMergeStructureShadowAtomicStats()
		  {
		...
		{
		  static SimSafeStoreMergeStructureShadowAtomicStats stats;
		  return stats;
		}
			...
			struct SimSafeWorksetDeferredIndexStats
			{
			  SimSafeWorksetDeferredIndexStats():
			    active(0),
			    calls(0),
			    nanoseconds(0),
			    legacyValidateNanoseconds(0),
			    legacyIndexRebuilds(0),
			    actualIndexRebuilds(0),
			    indexRebuildsSaved(0),
			    sizeMismatches(0),
			    candidateMismatches(0),
			    orderMismatches(0),
			    digestMismatches(0),
			  ...
			  uint64_t calls;
			  uint64_t nanoseconds;
			  uint64_t legacyValidateNanoseconds;
			  uint64_t legacyIndexRebuilds;
			  uint64_t actualIndexRebuilds;
			  uint64_t indexRebuildsSaved;
			  uint64_t sizeMismatches;
			  uint64_t candidateMismatches;
			  uint64_t orderMismatches;
			  uint64_t digestMismatches;
			...
			struct SimSafeWorksetDeferredIndexAtomicStats
			{
			  SimSafeWorksetDeferredIndexAtomicStats()
			  {
			    active.store(0,std::memory_order_relaxed);
			    calls.store(0,std::memory_order_relaxed);
			    nanoseconds.store(0,std::memory_order_relaxed);
			    legacyValidateNanoseconds.store(0,std::memory_order_relaxed);
			    legacyIndexRebuilds.store(0,std::memory_order_relaxed);
			    actualIndexRebuilds.store(0,std::memory_order_relaxed);
			    indexRebuildsSaved.store(0,std::memory_order_relaxed);
			    sizeMismatches.store(0,std::memory_order_relaxed);
			    candidateMismatches.store(0,std::memory_order_relaxed);
			    orderMismatches.store(0,std::memory_order_relaxed);
			    digestMismatches.store(0,std::memory_order_relaxed);
			  ...
			  std::atomic<uint64_t> calls;
			  std::atomic<uint64_t> nanoseconds;
			  std::atomic<uint64_t> legacyValidateNanoseconds;
			  std::atomic<uint64_t> legacyIndexRebuilds;
			  std::atomic<uint64_t> actualIndexRebuilds;
			  std::atomic<uint64_t> indexRebuildsSaved;
			  std::atomic<uint64_t> sizeMismatches;
			  std::atomic<uint64_t> candidateMismatches;
			  std::atomic<uint64_t> orderMismatches;
			  std::atomic<uint64_t> digestMismatches;
			...
			inline SimSafeWorksetDeferredIndexAtomicStats &
			simSafeWorksetDeferredIndexAtomicStats()
			{
			  static SimSafeWorksetDeferredIndexAtomicStats stats;
			  return stats;
			}
		...
		struct SimInitialSafeStorePruneIndexShadowStats
		{
		  ...
		  uint64_t compactNanoseconds;
		  uint64_t indexRebuildNanoseconds;
		  uint64_t statesScanned;
		  uint64_t statesKept;
		  uint64_t statesRemoved;
		  uint64_t sizeMismatches;
		  uint64_t candidateMismatches;
		  uint64_t orderMismatches;
		  uint64_t digestMismatches;
		...
		struct SimInitialSafeStorePruneIndexShadowAtomicStats
		{
		  SimInitialSafeStorePruneIndexShadowAtomicStats()
		  {
		    calls.store(0,std::memory_order_relaxed);
		    nanoseconds.store(0,std::memory_order_relaxed);
		    scanNanoseconds.store(0,std::memory_order_relaxed);
		    compactNanoseconds.store(0,std::memory_order_relaxed);
		    indexRebuildNanoseconds.store(0,std::memory_order_relaxed);
		    statesScanned.store(0,std::memory_order_relaxed);
		    statesKept.store(0,std::memory_order_relaxed);
		    statesRemoved.store(0,std::memory_order_relaxed);
		    sizeMismatches.store(0,std::memory_order_relaxed);
		    candidateMismatches.store(0,std::memory_order_relaxed);
		    orderMismatches.store(0,std::memory_order_relaxed);
		    digestMismatches.store(0,std::memory_order_relaxed);
		  ...
		  std::atomic<uint64_t> calls;
		  std::atomic<uint64_t> nanoseconds;
		  std::atomic<uint64_t> scanNanoseconds;
		  std::atomic<uint64_t> compactNanoseconds;
		  std::atomic<uint64_t> indexRebuildNanoseconds;
		  std::atomic<uint64_t> statesScanned;
		  std::atomic<uint64_t> statesKept;
		  std::atomic<uint64_t> statesRemoved;
		  std::atomic<uint64_t> sizeMismatches;
		  std::atomic<uint64_t> candidateMismatches;
		  std::atomic<uint64_t> orderMismatches;
		  std::atomic<uint64_t> digestMismatches;
		...
		inline SimInitialSafeStorePruneIndexShadowAtomicStats &simInitialSafeStorePruneIndexShadowAtomicStats()
		{
		  static SimInitialSafeStorePruneIndexShadowAtomicStats stats;
		  return stats;
		}
		...
		struct SimInitialSafeStoreGpuPrecombineShadowStats
		{
		...
		struct SimInitialSafeStoreGpuPrecombineShadowAtomicStats
		{
		  SimInitialSafeStoreGpuPrecombineShadowAtomicStats()
		  {
		    calls.store(0,std::memory_order_relaxed);
		    nanoseconds.store(0,std::memory_order_relaxed);
		    inputSummaries.store(0,std::memory_order_relaxed);
		...
		inline SimInitialSafeStoreGpuPrecombineShadowAtomicStats &simInitialSafeStoreGpuPrecombineShadowAtomicStats()
		{
		  static SimInitialSafeStoreGpuPrecombineShadowAtomicStats stats;
		  return stats;
		}

		struct SimInitialSafeStoreGpuPrecombinePruneShadowStats
		{
		  SimInitialSafeStoreGpuPrecombinePruneShadowStats():
		    calls(0),
		    nanoseconds(0),
		    inputStates(0),
		    keptStates(0),
		  ...
		  uint64_t keptStates;
		  uint64_t removedStates;
		  uint64_t estD2HBytesBefore;
		  uint64_t estD2HBytesAfter;
		  uint64_t estD2HBytesSaved;
		  uint64_t sizeMismatches;
		  uint64_t candidateMismatches;
		  uint64_t orderMismatches;
		  uint64_t digestMismatches;
		...
		struct SimInitialSafeStoreGpuPrecombinePruneShadowAtomicStats
		{
		  SimInitialSafeStoreGpuPrecombinePruneShadowAtomicStats()
		  {
		    calls.store(0,std::memory_order_relaxed);
		    nanoseconds.store(0,std::memory_order_relaxed);
		    inputStates.store(0,std::memory_order_relaxed);
		    keptStates.store(0,std::memory_order_relaxed);
		    removedStates.store(0,std::memory_order_relaxed);
		    estD2HBytesBefore.store(0,std::memory_order_relaxed);
		    estD2HBytesAfter.store(0,std::memory_order_relaxed);
		    estD2HBytesSaved.store(0,std::memory_order_relaxed);
		    sizeMismatches.store(0,std::memory_order_relaxed);
		    candidateMismatches.store(0,std::memory_order_relaxed);
		    orderMismatches.store(0,std::memory_order_relaxed);
		    digestMismatches.store(0,std::memory_order_relaxed);
		  ...
		  std::atomic<uint64_t> keptStates;
		  std::atomic<uint64_t> removedStates;
		  std::atomic<uint64_t> estD2HBytesBefore;
		  std::atomic<uint64_t> estD2HBytesAfter;
		  std::atomic<uint64_t> estD2HBytesSaved;
		  std::atomic<uint64_t> sizeMismatches;
		  std::atomic<uint64_t> candidateMismatches;
		  std::atomic<uint64_t> orderMismatches;
		  std::atomic<uint64_t> digestMismatches;
		...
		inline SimInitialSafeStoreGpuPrecombinePruneShadowAtomicStats &simInitialSafeStoreGpuPrecombinePruneShadowAtomicStats()
		{
		  static SimInitialSafeStoreGpuPrecombinePruneShadowAtomicStats stats;
		  return stats;
		}

		struct SimInitialSafeStoreGpuPrecombinePruneStats
		{
		  SimInitialSafeStoreGpuPrecombinePruneStats():
		    active(0),
		    calls(0),
		    nanoseconds(0),
		    inputStates(0),
		...
		struct SimInitialSafeStoreGpuPrecombinePruneAtomicStats
		{
		  SimInitialSafeStoreGpuPrecombinePruneAtomicStats()
		  {
		    active.store(0,std::memory_order_relaxed);
		    calls.store(0,std::memory_order_relaxed);
		    nanoseconds.store(0,std::memory_order_relaxed);
		...
		inline SimInitialSafeStoreGpuPrecombinePruneAtomicStats &simInitialSafeStoreGpuPrecombinePruneAtomicStats()
		{
		  static SimInitialSafeStoreGpuPrecombinePruneAtomicStats stats;
		  return stats;
		}
		...
		inline void recordSimSafeWorksetMergeBreakdown(const SimSafeWorksetMergeBreakdownStats &delta)
		{
		  SimSafeWorksetMergeBreakdownAtomicStats &stats =
		    simSafeWorksetMergeBreakdownAtomicStats();
		  for(size_t fieldIndex = 0;
		      fieldIndex < SIM_SAFE_WORKSET_MERGE_BREAKDOWN_FIELD_COUNT;
		      ++fieldIndex)
		  {
		    stats.values[fieldIndex].fetch_add(delta.values[fieldIndex],
		                                       std::memory_order_relaxed);
		  }
		}

		inline void recordSimSafeStoreMergeStructureShadow(const SimSafeStoreMergeStructureShadowStats &delta)
		{
		  SimSafeStoreMergeStructureShadowAtomicStats &stats =
		    simSafeStoreMergeStructureShadowAtomicStats();
		  ...
		  stats.orderMismatches.fetch_add(delta.orderMismatches,std::memory_order_relaxed);
			...
			inline void recordSimSafeWorksetDeferredIndex(
			  const SimSafeWorksetDeferredIndexStats &delta)
			{
			  SimSafeWorksetDeferredIndexAtomicStats &stats =
			    simSafeWorksetDeferredIndexAtomicStats();
			  stats.active.fetch_add(delta.active,std::memory_order_relaxed);
			  stats.calls.fetch_add(delta.calls,std::memory_order_relaxed);
			  stats.nanoseconds.fetch_add(delta.nanoseconds,std::memory_order_relaxed);
			  stats.legacyValidateNanoseconds.fetch_add(delta.legacyValidateNanoseconds,
			                                            std::memory_order_relaxed);
			  stats.legacyIndexRebuilds.fetch_add(delta.legacyIndexRebuilds,std::memory_order_relaxed);
			  stats.actualIndexRebuilds.fetch_add(delta.actualIndexRebuilds,std::memory_order_relaxed);
			  stats.indexRebuildsSaved.fetch_add(delta.indexRebuildsSaved,std::memory_order_relaxed);
			  stats.sizeMismatches.fetch_add(delta.sizeMismatches,std::memory_order_relaxed);
			  stats.candidateMismatches.fetch_add(delta.candidateMismatches,std::memory_order_relaxed);
			  stats.orderMismatches.fetch_add(delta.orderMismatches,std::memory_order_relaxed);
			  stats.digestMismatches.fetch_add(delta.digestMismatches,std::memory_order_relaxed);
		...
		inline void recordSimInitialSafeStorePruneIndexShadow(const SimInitialSafeStorePruneIndexShadowStats &delta)
		{
		  ...
		  stats.calls.fetch_add(delta.calls,std::memory_order_relaxed);
		  stats.nanoseconds.fetch_add(delta.nanoseconds,std::memory_order_relaxed);
		  stats.scanNanoseconds.fetch_add(delta.scanNanoseconds,std::memory_order_relaxed);
		  stats.compactNanoseconds.fetch_add(delta.compactNanoseconds,std::memory_order_relaxed);
		  stats.indexRebuildNanoseconds.fetch_add(delta.indexRebuildNanoseconds,std::memory_order_relaxed);
		  stats.statesScanned.fetch_add(delta.statesScanned,std::memory_order_relaxed);
		  stats.statesKept.fetch_add(delta.statesKept,std::memory_order_relaxed);
		  stats.statesRemoved.fetch_add(delta.statesRemoved,std::memory_order_relaxed);
		  stats.sizeMismatches.fetch_add(delta.sizeMismatches,std::memory_order_relaxed);
		  stats.candidateMismatches.fetch_add(delta.candidateMismatches,std::memory_order_relaxed);
		  stats.orderMismatches.fetch_add(delta.orderMismatches,std::memory_order_relaxed);
		  stats.digestMismatches.fetch_add(delta.digestMismatches,std::memory_order_relaxed);
		...
		inline void recordSimInitialSafeStoreGpuPrecombineShadow(const SimInitialSafeStoreGpuPrecombineShadowStats &delta)
		{
		  SimInitialSafeStoreGpuPrecombineShadowAtomicStats &stats =
		    simInitialSafeStoreGpuPrecombineShadowAtomicStats();
		  stats.calls.fetch_add(delta.calls,std::memory_order_relaxed);
		  stats.nanoseconds.fetch_add(delta.nanoseconds,std::memory_order_relaxed);
		  stats.inputSummaries.fetch_add(delta.inputSummaries,std::memory_order_relaxed);
		...
		inline void recordSimInitialSafeStoreGpuPrecombinePruneShadow(const SimInitialSafeStoreGpuPrecombinePruneShadowStats &delta)
		{
		  SimInitialSafeStoreGpuPrecombinePruneShadowAtomicStats &stats =
		    simInitialSafeStoreGpuPrecombinePruneShadowAtomicStats();
		  stats.calls.fetch_add(delta.calls,std::memory_order_relaxed);
		  stats.nanoseconds.fetch_add(delta.nanoseconds,std::memory_order_relaxed);
		  stats.inputStates.fetch_add(delta.inputStates,std::memory_order_relaxed);
		  stats.keptStates.fetch_add(delta.keptStates,std::memory_order_relaxed);
		  stats.removedStates.fetch_add(delta.removedStates,std::memory_order_relaxed);
		  stats.estD2HBytesBefore.fetch_add(delta.estD2HBytesBefore,std::memory_order_relaxed);
		  stats.estD2HBytesAfter.fetch_add(delta.estD2HBytesAfter,std::memory_order_relaxed);
		  stats.estD2HBytesSaved.fetch_add(delta.estD2HBytesSaved,std::memory_order_relaxed);
		  stats.sizeMismatches.fetch_add(delta.sizeMismatches,std::memory_order_relaxed);
		  stats.candidateMismatches.fetch_add(delta.candidateMismatches,std::memory_order_relaxed);
		  stats.orderMismatches.fetch_add(delta.orderMismatches,std::memory_order_relaxed);
		  stats.digestMismatches.fetch_add(delta.digestMismatches,std::memory_order_relaxed);
		...
		inline void recordSimInitialSafeStoreGpuPrecombinePrune(const SimInitialSafeStoreGpuPrecombinePruneStats &delta)
		{
		  SimInitialSafeStoreGpuPrecombinePruneAtomicStats &stats =
		    simInitialSafeStoreGpuPrecombinePruneAtomicStats();
		  stats.active.fetch_add(delta.active,std::memory_order_relaxed);
		  stats.calls.fetch_add(delta.calls,std::memory_order_relaxed);
		  stats.nanoseconds.fetch_add(delta.nanoseconds,std::memory_order_relaxed);
		...
		inline SimSafeWorksetMergeBreakdownStats getSimSafeWorksetMergeBreakdownStats()
		{
		  SimSafeWorksetMergeBreakdownStats snapshot;
		  SimSafeWorksetMergeBreakdownAtomicStats &stats =
		    simSafeWorksetMergeBreakdownAtomicStats();
		  for(size_t fieldIndex = 0;
		      fieldIndex < SIM_SAFE_WORKSET_MERGE_BREAKDOWN_FIELD_COUNT;
		      ++fieldIndex)
		  {
		    snapshot.values[fieldIndex] =
		      stats.values[fieldIndex].load(std::memory_order_relaxed);
		  }
		  return snapshot;
		}

		inline SimSafeStoreMergeStructureShadowStats getSimSafeStoreMergeStructureShadowStats()
		{
		  SimSafeStoreMergeStructureShadowStats snapshot;
		  SimSafeStoreMergeStructureShadowAtomicStats &stats =
		    simSafeStoreMergeStructureShadowAtomicStats();
		  ...
		  return snapshot;
			...
			inline SimSafeWorksetDeferredIndexStats
			getSimSafeWorksetDeferredIndexStats()
			{
			  SimSafeWorksetDeferredIndexStats snapshot;
			  SimSafeWorksetDeferredIndexAtomicStats &stats =
			    simSafeWorksetDeferredIndexAtomicStats();
			  snapshot.active = stats.active.load(std::memory_order_relaxed);
			  snapshot.calls = stats.calls.load(std::memory_order_relaxed);
			  snapshot.nanoseconds = stats.nanoseconds.load(std::memory_order_relaxed);
			  snapshot.legacyValidateNanoseconds =
			    stats.legacyValidateNanoseconds.load(std::memory_order_relaxed);
			  snapshot.legacyIndexRebuilds = stats.legacyIndexRebuilds.load(std::memory_order_relaxed);
			  snapshot.actualIndexRebuilds = stats.actualIndexRebuilds.load(std::memory_order_relaxed);
			  snapshot.indexRebuildsSaved = stats.indexRebuildsSaved.load(std::memory_order_relaxed);
			  snapshot.sizeMismatches = stats.sizeMismatches.load(std::memory_order_relaxed);
			  snapshot.candidateMismatches = stats.candidateMismatches.load(std::memory_order_relaxed);
			  snapshot.orderMismatches = stats.orderMismatches.load(std::memory_order_relaxed);
			  snapshot.digestMismatches = stats.digestMismatches.load(std::memory_order_relaxed);
		...
		{
		  SimInitialSafeStorePruneIndexShadowStats snapshot;
		  SimInitialSafeStorePruneIndexShadowAtomicStats &stats =
		    simInitialSafeStorePruneIndexShadowAtomicStats();
		  snapshot.calls = stats.calls.load(std::memory_order_relaxed);
		  snapshot.nanoseconds = stats.nanoseconds.load(std::memory_order_relaxed);
		  snapshot.scanNanoseconds = stats.scanNanoseconds.load(std::memory_order_relaxed);
		  snapshot.compactNanoseconds = stats.compactNanoseconds.load(std::memory_order_relaxed);
		  snapshot.indexRebuildNanoseconds =
		    stats.indexRebuildNanoseconds.load(std::memory_order_relaxed);
		  snapshot.statesScanned = stats.statesScanned.load(std::memory_order_relaxed);
		  snapshot.statesKept = stats.statesKept.load(std::memory_order_relaxed);
		  snapshot.statesRemoved = stats.statesRemoved.load(std::memory_order_relaxed);
		  snapshot.sizeMismatches = stats.sizeMismatches.load(std::memory_order_relaxed);
		...
		inline SimInitialSafeStoreGpuPrecombineShadowStats getSimInitialSafeStoreGpuPrecombineShadowStats()
		{
		  SimInitialSafeStoreGpuPrecombineShadowStats snapshot;
		  SimInitialSafeStoreGpuPrecombineShadowAtomicStats &stats =
		    simInitialSafeStoreGpuPrecombineShadowAtomicStats();
		  snapshot.calls = stats.calls.load(std::memory_order_relaxed);
		  snapshot.nanoseconds = stats.nanoseconds.load(std::memory_order_relaxed);
		...
		inline SimInitialSafeStoreGpuPrecombinePruneShadowStats getSimInitialSafeStoreGpuPrecombinePruneShadowStats()
		{
		  SimInitialSafeStoreGpuPrecombinePruneShadowStats snapshot;
		  SimInitialSafeStoreGpuPrecombinePruneShadowAtomicStats &stats =
		    simInitialSafeStoreGpuPrecombinePruneShadowAtomicStats();
		  snapshot.calls = stats.calls.load(std::memory_order_relaxed);
		  snapshot.nanoseconds = stats.nanoseconds.load(std::memory_order_relaxed);
		  ...
		  snapshot.keptStates = stats.keptStates.load(std::memory_order_relaxed);
		  snapshot.removedStates = stats.removedStates.load(std::memory_order_relaxed);
		  snapshot.estD2HBytesBefore =
		    stats.estD2HBytesBefore.load(std::memory_order_relaxed);
		  snapshot.estD2HBytesAfter =
		    stats.estD2HBytesAfter.load(std::memory_order_relaxed);
		  snapshot.estD2HBytesSaved =
		    stats.estD2HBytesSaved.load(std::memory_order_relaxed);
		  snapshot.sizeMismatches = stats.sizeMismatches.load(std::memory_order_relaxed);
		  snapshot.candidateMismatches =
		    stats.candidateMismatches.load(std::memory_order_relaxed);
		  snapshot.orderMismatches = stats.orderMismatches.load(std::memory_order_relaxed);
		...
		inline SimInitialSafeStoreGpuPrecombinePruneStats getSimInitialSafeStoreGpuPrecombinePruneStats()
		{
		  SimInitialSafeStoreGpuPrecombinePruneStats snapshot;
		  SimInitialSafeStoreGpuPrecombinePruneAtomicStats &stats =
		    simInitialSafeStoreGpuPrecombinePruneAtomicStats();
		  snapshot.active = stats.active.load(std::memory_order_relaxed);
		  snapshot.calls = stats.calls.load(std::memory_order_relaxed);
		...
		inline std::atomic<uint64_t> &simInitialReduceChunkReplayedCount()
		{
		  ...
		  return count;
			...
			inline std::atomic<uint64_t> &simInitialExactFrontierCpuSafeStoreDigestAvailableCount()
			{
			  static std::atomic<uint64_t> count(0);
			  return count;
			}

			inline std::atomic<uint64_t> &simInitialExactFrontierCpuSafeStoreEpochAvailableCount()
			{
			  static std::atomic<uint64_t> count(0);
			  return count;
			}
			...
			inline std::atomic<uint64_t> &simInitialExactFrontierOneChunkBoundedShadowCallCount()
			{
			  static std::atomic<uint64_t> count(0);
			  return count;
			}

			inline std::atomic<uint64_t> &simInitialExactFrontierOneChunkBoundedShadowInputSummaryCount()
			{
			  static std::atomic<uint64_t> count(0);
			  return count;
			}

			inline std::atomic<uint64_t> &simInitialExactFrontierOneChunkBoundedShadowProcessedSummaryCount()
			{
			  static std::atomic<uint64_t> count(0);
			  return count;
			}

			inline std::atomic<uint64_t> &simInitialExactFrontierOneChunkBoundedShadowTruncatedCount()
			{
			  static std::atomic<uint64_t> count(0);
			  return count;
			}
			...
			inline std::atomic<uint64_t> &simInitialExactFrontierOneChunkBoundedShadowH2DBytes()
			{
			  static std::atomic<uint64_t> count(0);
			  return count;
			}

			inline std::atomic<uint64_t> &simInitialExactFrontierOneChunkBoundedShadowD2HBytes()
			{
			  static std::atomic<uint64_t> count(0);
			  return count;
			}

			inline std::atomic<uint64_t> &simInitialExactFrontierOneChunkBoundedShadowCandidateDigestMismatches()
			{
			  static std::atomic<uint64_t> count(0);
			  return count;
			}

			inline std::atomic<uint64_t> &simInitialExactFrontierOneChunkBoundedShadowMinCandidateMismatches()
			{
			  static std::atomic<uint64_t> count(0);
			  return count;
			}

			inline std::atomic<uint64_t> &simInitialExactFrontierOneChunkBoundedShadowFirstMaxTieMismatches()
			{
			  static std::atomic<uint64_t> count(0);
			  return count;
			}

			inline std::atomic<uint64_t> &simInitialExactFrontierOneChunkBoundedShadowSafeStoreDigestMismatches()
			{
			  static std::atomic<uint64_t> count(0);
			  return count;
			}

			inline std::atomic<uint64_t> &simInitialExactFrontierOneChunkBoundedShadowTotalMismatches()
			{
			  static std::atomic<uint64_t> count(0);
			  return count;
			}

			inline std::atomic<uint64_t> &simInitialExactFrontierPerRequestShadowRequestCount()
			{
			  static std::atomic<uint64_t> count(0);
			  return count;
			}
			...
			inline std::atomic<uint64_t> &simInitialExactFrontierPerRequestShadowProcessedSummaryCount()
			{
			  static std::atomic<uint64_t> count(0);
			  return count;
			}

			inline std::atomic<uint64_t> &simInitialExactFrontierPerRequestShadowH2DBytes()
			{
			  static std::atomic<uint64_t> count(0);
			  return count;
			}

			inline std::atomic<uint64_t> &simInitialExactFrontierPerRequestShadowD2HBytes()
			{
			  static std::atomic<uint64_t> count(0);
			  return count;
			}

				inline std::atomic<uint64_t> &simInitialExactFrontierPerRequestShadowNanoseconds()
				{
				  static std::atomic<uint64_t> count(0);
				  return count;
				}

				inline std::atomic<uint64_t> &simInitialExactFrontierPerRequestShadowKernelNanoseconds()
				{
				  static std::atomic<uint64_t> count(0);
				  return count;
				}

				inline std::atomic<uint64_t> &simInitialExactFrontierPerRequestShadowCompareNanoseconds()
				{
				  static std::atomic<uint64_t> count(0);
				  return count;
				}

				inline std::atomic<uint64_t> &simInitialExactFrontierPerRequestShadowResidentSourceActiveCount()
				{
				  static std::atomic<uint64_t> count(0);
				  return count;
				}

				inline std::atomic<uint64_t> &simInitialExactFrontierPerRequestShadowSummaryH2DElidedCount()
				{
				  static std::atomic<uint64_t> count(0);
				  return count;
				}

				inline std::atomic<uint64_t> &simInitialExactFrontierPerRequestShadowSummaryH2DBytesSaved()
				{
				  static std::atomic<uint64_t> count(0);
				  return count;
				}

				inline std::atomic<uint64_t> &simInitialExactFrontierPerRequestShadowFallbackCount()
				{
				  static std::atomic<uint64_t> count(0);
				  return count;
				}

				inline std::atomic<uint64_t> &simInitialExactFrontierPerRequestShadowCandidateDigestMismatches()
			{
			  static std::atomic<uint64_t> count(0);
			  return count;
			}

			inline std::atomic<uint64_t> &simInitialExactFrontierPerRequestShadowCandidateValueMismatches()
			{
			  static std::atomic<uint64_t> count(0);
			  return count;
			}

			inline std::atomic<uint64_t> &simInitialExactFrontierPerRequestShadowMinCandidateMismatches()
			{
			  static std::atomic<uint64_t> count(0);
			  return count;
			}

			inline std::atomic<uint64_t> &simInitialExactFrontierPerRequestShadowFirstMaxTieMismatches()
			{
			  static std::atomic<uint64_t> count(0);
			  return count;
			}
		...
		inline void recordSimDeviceKLoopNanoseconds(uint64_t nanoseconds)
		{
		  simDeviceKLoopNanoseconds().fetch_add(nanoseconds, std::memory_order_relaxed);
			                                                             ...
			                                                             bool firstMaxTie)
			{
			  ...
			  if(safeStoreDigest)
			  {
			    simInitialExactFrontierCpuSafeStoreDigestAvailableCount().fetch_add(
			      1,
			      std::memory_order_relaxed);
			  }
			  if(safeStoreEpoch)
			  ...
			  uint32_t mismatchMask)
			{
			  ...
			  simInitialExactFrontierOneChunkBoundedShadowNanoseconds().fetch_add(nanoseconds, std::memory_order_relaxed);
			  simInitialExactFrontierOneChunkBoundedShadowH2DBytes().fetch_add(h2dBytes, std::memory_order_relaxed);
			  simInitialExactFrontierOneChunkBoundedShadowD2HBytes().fetch_add(d2hBytes, std::memory_order_relaxed);
			  if((mismatchMask & (1u << 0)) != 0)
			    simInitialExactFrontierOneChunkBoundedShadowCandidateDigestMismatches().fetch_add(1, std::memory_order_relaxed);
			  if((mismatchMask & (1u << 2)) != 0)
			    simInitialExactFrontierOneChunkBoundedShadowMinCandidateMismatches().fetch_add(1, std::memory_order_relaxed);
			  if((mismatchMask & (1u << 3)) != 0)
			    simInitialExactFrontierOneChunkBoundedShadowFirstMaxTieMismatches().fetch_add(1, std::memory_order_relaxed);
			  if((mismatchMask & (1u << 4)) != 0)
			    simInitialExactFrontierOneChunkBoundedShadowSafeStoreDigestMismatches().fetch_add(1, std::memory_order_relaxed);
			  if(mismatchMask != 0)
			  {
			    simInitialExactFrontierOneChunkBoundedShadowTotalMismatches().fetch_add(1, std::memory_order_relaxed);
			  }
			}
				...
				inline void recordSimInitialExactFrontierPerRequestShadowSkipped()
				{
				  ...
				  uint64_t h2dBytes,
				  uint64_t d2hBytes,
				  bool residentSourceActive,
				  bool summaryH2DElided,
				  uint64_t summaryH2DBytesSaved,
				  bool fallback,
				  uint32_t mismatchMask)
				{
				  simInitialExactFrontierPerRequestShadowComparedCount().fetch_add(
				    1,
			    std::memory_order_relaxed);
			  simInitialExactFrontierPerRequestShadowProcessedSummaryCount().fetch_add(
			    processedSummaries,
			    std::memory_order_relaxed);
				  simInitialExactFrontierPerRequestShadowNanoseconds().fetch_add(
				    nanoseconds,
				    std::memory_order_relaxed);
				  simInitialExactFrontierPerRequestShadowKernelNanoseconds().fetch_add(
				    kernelNanoseconds,
				    std::memory_order_relaxed);
				  simInitialExactFrontierPerRequestShadowCompareNanoseconds().fetch_add(
				    compareNanoseconds,
				    std::memory_order_relaxed);
				  simInitialExactFrontierPerRequestShadowH2DBytes().fetch_add(
				    h2dBytes,
				    std::memory_order_relaxed);
				  simInitialExactFrontierPerRequestShadowD2HBytes().fetch_add(
				    d2hBytes,
				    std::memory_order_relaxed);
				  if(residentSourceActive)
				  {
				    simInitialExactFrontierPerRequestShadowResidentSourceActiveCount().fetch_add(
				      1,
				      std::memory_order_relaxed);
				  }
				  if(summaryH2DElided)
				  {
				    simInitialExactFrontierPerRequestShadowSummaryH2DElidedCount().fetch_add(
				      1,
				      std::memory_order_relaxed);
				  }
				  simInitialExactFrontierPerRequestShadowSummaryH2DBytesSaved().fetch_add(
				    summaryH2DBytesSaved,
				    std::memory_order_relaxed);
				  if(fallback)
				  {
				    simInitialExactFrontierPerRequestShadowFallbackCount().fetch_add(
				      1,
				      std::memory_order_relaxed);
				  }
				  if((mismatchMask & (1u << 0)) != 0)
			  {
			    simInitialExactFrontierPerRequestShadowCandidateDigestMismatches().fetch_add(
			      1,
			      std::memory_order_relaxed);
			  }
			  if((mismatchMask & (1u << 1)) != 0)
			  {
			    simInitialExactFrontierPerRequestShadowCandidateValueMismatches().fetch_add(
			      1,
			      std::memory_order_relaxed);
			  }
			  if((mismatchMask & (1u << 2)) != 0)
			  {
			    simInitialExactFrontierPerRequestShadowMinCandidateMismatches().fetch_add(
			      1,
			      std::memory_order_relaxed);
			  }
			  if((mismatchMask & (1u << 3)) != 0)
			  {
			    simInitialExactFrontierPerRequestShadowFirstMaxTieMismatches().fetch_add(
			      1,
			      std::memory_order_relaxed);
			  }
			  if((mismatchMask & (1u << 4)) != 0)
			  ...
			  if(mismatchMask != 0)
			  {
			    simInitialExactFrontierPerRequestShadowMismatchedRequestCount().fetch_add(
			      1,
			      std::memory_order_relaxed);
			    simInitialExactFrontierPerRequestShadowTotalMismatches().fetch_add(
			      1,
			      std::memory_order_relaxed);
			  }
			}
			  ...
			  packSeconds =
				                                                    ...
				                                                    uint64_t &summaryReplayedCount)
				{
				  ...
				  summarySkippedCount =
					...
					inline void getSimInitialExactFrontierOneChunkBoundedShadowStats(
					  uint64_t &callCount,
					  uint64_t &inputSummaries,
					  uint64_t &processedSummaries,
					  ...
					  uint64_t &h2dBytes,
					  uint64_t &d2hBytes,
					  uint64_t &candidateDigestMismatches,
					  uint64_t &minCandidateMismatches,
					  uint64_t &firstMaxTieMismatches,
					  uint64_t &safeStoreDigestMismatches,
					  uint64_t &totalMismatches)
					{
					  callCount = simInitialExactFrontierOneChunkBoundedShadowCallCount().load(std::memory_order_relaxed);
					  ...
					  candidateDigestMismatches = simInitialExactFrontierOneChunkBoundedShadowCandidateDigestMismatches().load(std::memory_order_relaxed);
					  minCandidateMismatches = simInitialExactFrontierOneChunkBoundedShadowMinCandidateMismatches().load(std::memory_order_relaxed);
					  firstMaxTieMismatches = simInitialExactFrontierOneChunkBoundedShadowFirstMaxTieMismatches().load(std::memory_order_relaxed);
					  safeStoreDigestMismatches = simInitialExactFrontierOneChunkBoundedShadowSafeStoreDigestMismatches().load(std::memory_order_relaxed);
					  ...
					  uint64_t &totalMismatches)
					{
					  ...
					  processedSummaries =
					    simInitialExactFrontierPerRequestShadowProcessedSummaryCount().load(std::memory_order_relaxed);
						  nanoseconds =
						    simInitialExactFrontierPerRequestShadowNanoseconds().load(std::memory_order_relaxed);
						  kernelNanoseconds =
						    simInitialExactFrontierPerRequestShadowKernelNanoseconds().load(std::memory_order_relaxed);
						  compareNanoseconds =
						    simInitialExactFrontierPerRequestShadowCompareNanoseconds().load(std::memory_order_relaxed);
						  h2dBytes =
						    simInitialExactFrontierPerRequestShadowH2DBytes().load(std::memory_order_relaxed);
						  d2hBytes =
						    simInitialExactFrontierPerRequestShadowD2HBytes().load(std::memory_order_relaxed);
						  residentSourceActive =
						    simInitialExactFrontierPerRequestShadowResidentSourceActiveCount().load(std::memory_order_relaxed);
						  summaryH2DElided =
						    simInitialExactFrontierPerRequestShadowSummaryH2DElidedCount().load(std::memory_order_relaxed);
						  summaryH2DBytesSaved =
						    simInitialExactFrontierPerRequestShadowSummaryH2DBytesSaved().load(std::memory_order_relaxed);
						  fallbacks =
						    simInitialExactFrontierPerRequestShadowFallbackCount().load(std::memory_order_relaxed);
						  candidateDigestMismatches =
						    simInitialExactFrontierPerRequestShadowCandidateDigestMismatches().load(std::memory_order_relaxed);
					  candidateValueMismatches =
					    simInitialExactFrontierPerRequestShadowCandidateValueMismatches().load(std::memory_order_relaxed);
...
inline SimInitialSafeStoreGpuPrecombineShadowStats
runSimInitialSafeStoreGpuPrecombineShadow(const vector<SimScanCudaInitialRunSummary> &summaries,
                                          const SimCandidateStateStore &realStore);
inline SimInitialSafeStoreGpuPrecombinePruneShadowStats
runSimInitialSafeStoreGpuPrecombinePruneShadow(
...
inline void pruneSimCandidateStateStoreShadow(SimCandidateStateStore &store,
                                              const SimKernelContext &context,
                                              uint64_t *scanNanoseconds = NULL,
                                              uint64_t *compactNanoseconds = NULL,
                                              uint64_t *indexRebuildNanoseconds = NULL)
{
  if(!store.valid)
  {
    return;
  ...
  keptStates.reserve(store.states.size());
  const std::chrono::steady_clock::time_point scanStart =
    scanNanoseconds != NULL ? std::chrono::steady_clock::now() : std::chrono::steady_clock::time_point();
  for(size_t stateIndex = 0; stateIndex < store.states.size(); ++stateIndex)
  {
    const SimScanCudaCandidateState &state = store.states[stateIndex];
    const uint64_t startCoord = simScanCudaCandidateStateStartCoord(state);
  ...
  if(scanNanoseconds != NULL)
  {
    *scanNanoseconds = simElapsedNanoseconds(scanStart);
  }
  const std::chrono::steady_clock::time_point compactStart =
    compactNanoseconds != NULL ? std::chrono::steady_clock::now() : std::chrono::steady_clock::time_point();
  store.states.swap(keptStates);
  if(compactNanoseconds != NULL)
  {
    *compactNanoseconds = simElapsedNanoseconds(compactStart);
  }
  const std::chrono::steady_clock::time_point indexRebuildStart =
    indexRebuildNanoseconds != NULL ? std::chrono::steady_clock::now() : std::chrono::steady_clock::time_point();
  rebuildSimCandidateStateStoreIndex(store);
  if(indexRebuildNanoseconds != NULL)
  {
    *indexRebuildNanoseconds = simElapsedNanoseconds(indexRebuildStart);
  }
}

inline uint64_t simCandidateStateStoreFingerprint(const SimCandidateStateStore &store)
{
  uint64_t hash = 1469598103934665603ull;
  hash = simRegionSchedulerShapeHashCombine(hash,store.valid ? 1ull : 0ull);
  if(!store.valid)
  {
    return hash;
  }
  ...
  vector<uint64_t> stateFingerprints;
  stateFingerprints.reserve(store.states.size());
  for(size_t stateIndex = 0; stateIndex < store.states.size(); ++stateIndex)
  {
    stateFingerprints.push_back(
      simRegionSchedulerShapeCandidateStateFingerprint(store.states[stateIndex]));
  }
  sort(stateFingerprints.begin(),stateFingerprints.end());
  hash = simRegionSchedulerShapeHashCombine(hash,
                                           static_cast<uint64_t>(stateFingerprints.size()));
...
runSimInitialSafeStoreGpuPrecombineShadow(const vector<SimScanCudaInitialRunSummary> &summaries,
                                          const SimCandidateStateStore &realStore)
{
  SimInitialSafeStoreGpuPrecombineShadowStats stats;
  stats.calls = 1;
  stats.inputSummaries = static_cast<uint64_t>(summaries.size());
  ...
  if(!gpuOk)
  {
    ...
    {
      fprintf(stderr,
              "SIM CUDA initial safe-store GPU precombine shadow failed: %s\n",
              gpuError.c_str());
    }
    return stats;
  }
  ...
  const bool sizeMatch =
    realStore.valid && gpuStates.size() == realStore.states.size();
  if(!sizeMatch)
  {
    stats.sizeMismatches = 1;
  }
  const bool orderMatch =
    sizeMatch &&
    simCudaCandidateStateVectorsEqualOrdered(gpuStates,realStore.states);
  if(!orderMatch)
  {
    stats.orderMismatches = 1;
  }
  const bool candidateMatch =
    sizeMatch &&
    simCudaCandidateStateVectorsEqualAsSet(gpuStates,realStore.states);
  if(!candidateMatch)
  {
    stats.candidateMismatches = 1;
  }
  if(simCandidateStateVectorSafeStoreFingerprint(gpuStates) !=
     simCandidateStateStoreFingerprint(realStore))
  {
    stats.digestMismatches = 1;
  }
...
runSimInitialSafeStoreGpuPrecombinePruneShadow(
  ...
  const SimCandidateStateStore &realPostPruneStore)
{
  SimInitialSafeStoreGpuPrecombinePruneShadowStats stats;
  stats.calls = 1;
  const SimCandidateStateStore &prePruneStore =
    prePruneContext.safeCandidateStateStore;
  stats.inputStates =
    static_cast<uint64_t>(prePruneStore.valid ? prePruneStore.states.size() : 0);
  stats.estD2HBytesBefore =
    stats.inputStates *
    static_cast<uint64_t>(sizeof(SimScanCudaCandidateState));

  vector<SimScanCudaCandidateState> finalCandidateStates;
  collectSimContextCandidateStates(prePruneContext,finalCandidateStates);
  vector<SimScanCudaCandidateState> gpuPrunedStates;
  double gpuSeconds = 0.0;
  uint64_t d2hBytes = 0;
  string gpuError;
  const bool gpuOk =
    prePruneStore.valid &&
    sim_scan_cuda_prune_initial_safe_store_gpu_precombine_shadow(
      prePruneStore.states,
      finalCandidateStates,
      static_cast<int>(prePruneContext.runningMin),
      &gpuPrunedStates,
      &gpuSeconds,
      &d2hBytes,
      &gpuError);
  stats.nanoseconds = simSecondsToNanoseconds(gpuSeconds);
  stats.keptStates = static_cast<uint64_t>(gpuPrunedStates.size());
  if(stats.inputStates > stats.keptStates)
  {
    stats.removedStates = stats.inputStates - stats.keptStates;
  }
  stats.estD2HBytesAfter = d2hBytes;
  if(stats.estD2HBytesBefore > stats.estD2HBytesAfter)
  {
    stats.estD2HBytesSaved =
      stats.estD2HBytesBefore - stats.estD2HBytesAfter;
  }
  if(!gpuOk)
  {
    stats.sizeMismatches = 1;
    ...
    {
      fprintf(stderr,
              "SIM CUDA initial safe-store GPU precombine prune shadow failed: %s\n",
              gpuError.c_str());
    }
    return stats;
  }
  ...
  const bool sizeMatch =
    realPostPruneStore.valid && gpuPrunedStates.size() == realPostPruneStore.states.size();
  if(!sizeMatch)
  {
    stats.sizeMismatches = 1;
  }
  const bool orderMatch =
    sizeMatch &&
    simCudaCandidateStateVectorsEqualOrdered(gpuPrunedStates,
                                             realPostPruneStore.states);
  if(!orderMatch)
  {
    stats.orderMismatches = 1;
  }
  const bool candidateMatch =
    sizeMatch &&
    simCudaCandidateStateVectorsEqualAsSet(gpuPrunedStates,
                                           realPostPruneStore.states);
  if(!candidateMatch)
  {
    stats.candidateMismatches = 1;
  }
  if(simCandidateStateVectorSafeStoreFingerprint(gpuPrunedStates) !=
     simCandidateStateStoreFingerprint(realPostPruneStore))
  {
    stats.digestMismatches = 1;
  }
  return stats;
...
inline bool simInitialSafeStoreGpuPrecombinePruneStoresMatch(
  const SimCandidateStateStore &gpuStore,
  const SimCandidateStateStore &legacyStore,
  ...
  uint64_t *indexRebuildNanosecondsOut)
{
  ...
  if(stats != NULL)
  {
    ...
    for(size_t stateIndex = 0; stateIndex < states.size(); ++stateIndex)
    {
      if(states[stateIndex].score > context.runningMin)
      {
        stats->pruneKeptAboveFloor += 1;
      }
      else
      {
        const uint64_t startCoord =
          simScanCudaCandidateStateStartCoord(states[stateIndex]);
        if(simContextContainsCandidateStartCoord(context,startCoord))
        {
          stats->pruneKeptFrontier += 1;
        }
      }
...
runSimInitialSafeStoreGpuPrecombine(
  const vector<SimScanCudaInitialRunSummary> &summaries,
  const SimKernelContext &preUpdateContext,
  bool validate,
  ...
  bool &outApplied,
  bool &outPruned,
  SimInitialSafeStoreGpuPrecombinePruneStats *outPruneStats)
{
  SimInitialSafeStoreGpuPrecombineStats stats;
  stats.calls = 1;
  stats.inputSummaries = static_cast<uint64_t>(summaries.size());
  const SimCandidateStateStore &preUpdateStore =
    preUpdateContext.safeCandidateStateStore;
  ...
  {
    *outPruneStats = SimInitialSafeStoreGpuPrecombinePruneStats();
  }
  ...
  if(pruneRequested && preUpdateStore.valid && !preUpdateStore.states.empty())
  {
    ...
    if(outPruneStats != NULL)
    {
      outPruneStats->calls = 1;
      outPruneStats->fallbacks = 1;
    }
    if(simCudaValidateEnabledRuntime())
    {
      fprintf(stderr,
              "SIM CUDA initial safe-store GPU precombine prune requires an empty pre-update store, falling back to legacy\n");
    }
    return stats;
  }
  ...
  if(pruneRequested)
  {
    ...
    collectSimContextCandidateStates(preUpdateContext,finalCandidateStates);
  }
  if(simCudaInitialSafeStoreGpuPrecombineResidentSourceRequestedRuntime())
  {
    ...
    {
      gpuOk =
        sim_scan_cuda_precombine_prune_initial_safe_store_resident(
          summaries.size(),
          finalCandidateStates,
          static_cast<int>(preUpdateContext.runningMin),
          &gpuStates,
          &gpuSeconds,
          &uniqueStates,
          &d2hBytes,
  ...
  if(!gpuOk)
  {
    ...
    {
      gpuOk =
        sim_scan_cuda_precombine_prune_initial_safe_store_shadow(
          summaries,
          finalCandidateStates,
          static_cast<int>(preUpdateContext.runningMin),
          &gpuStates,
  ...
  stats.d2hBytes = d2hBytes;
  stats.uniqueStates =
    pruneRequested ? uniqueStates : static_cast<uint64_t>(gpuStates.size());
  if(stats.inputSummaries > stats.uniqueStates)
  {
    stats.estSavedUpserts = stats.inputSummaries - stats.uniqueStates;
  }
  if(pruneRequested && outPruneStats != NULL)
  {
    outPruneStats->nanoseconds = stats.nanoseconds;
    outPruneStats->inputStates = stats.uniqueStates;
    outPruneStats->keptStates = static_cast<uint64_t>(gpuStates.size());
    outPruneStats->removedStates =
      outPruneStats->inputStates > outPruneStats->keptStates ?
      outPruneStats->inputStates - outPruneStats->keptStates : 0;
    outPruneStats->d2hBytes = d2hBytes;
    const uint64_t prePruneD2HBytes =
      outPruneStats->inputStates *
      static_cast<uint64_t>(sizeof(SimScanCudaCandidateState));
  ...
  if(pruneRequested && simCudaInitialSafeStoreGpuPrecombinePruneValidateEnabledRuntime())
  {
    const std::chrono::steady_clock::time_point validateStart =
      std::chrono::steady_clock::now();
    SimKernelContext legacyContext = preUpdateContext;
    SimInitialSafeStoreRebuildStats legacyStats;
    uint64_t legacyHostEpochBumps = 0;
    mergeSimCudaInitialRunSummariesIntoSafeStoreLocal(
      summaries,
      legacyContext.safeCandidateStateStore,
      &legacyStats,
      &legacyHostEpochBumps);
    pruneSimSafeCandidateStateStore(legacyContext,&legacyStats);
    const bool validateOk =
      outPruneStats != NULL &&
      simInitialSafeStoreGpuPrecombinePruneStoresMatch(
        outStore,
        legacyContext.safeCandidateStateStore,
        *outPruneStats);
    if(outPruneStats != NULL)
    {
      outPruneStats->validateNanoseconds = simElapsedNanoseconds(validateStart);
    }
    if(!validateOk)
    {
      stats.sizeMismatches = outPruneStats != NULL ? outPruneStats->sizeMismatches : 1;
      stats.candidateMismatches =
        outPruneStats != NULL ? outPruneStats->candidateMismatches : 1;
      stats.orderMismatches = outPruneStats != NULL ? outPruneStats->orderMismatches : 1;
      stats.digestMismatches = outPruneStats != NULL ? outPruneStats->digestMismatches : 1;
      stats.fallbacks = 1;
      if(outPruneStats != NULL)
      {
        outPruneStats->fallbacks = 1;
      ...
      {
        fprintf(stderr,
                "SIM CUDA initial safe-store GPU precombine prune validation mismatch: "
                "digest=%llu size=%llu candidate=%llu order=%llu\n",
  ...
  if(pruneRequested && outPruneStats != NULL)
  {
    outPruneStats->active = 1;
  }
  outApplied = true;
  outPruned = pruneRequested;
  return stats;
}
...
runSimInitialSafeStorePruneIndexShadow(const SimKernelContext &prePruneContext,
                                       const SimCandidateStateStore &realPostPruneStore,
                                       const SimInitialSafeStoreRebuildStats &realPruneStats)
{
  const std::chrono::steady_clock::time_point shadowStart =
    std::chrono::steady_clock::now();

  SimCandidateStateStore shadowStore = prePruneContext.safeCandidateStateStore;
  const uint64_t scannedStates =
    static_cast<uint64_t>(shadowStore.valid ? shadowStore.states.size() : 0);
  uint64_t scanNanoseconds = 0;
  uint64_t compactNanoseconds = 0;
  uint64_t indexRebuildNanoseconds = 0;
  pruneSimCandidateStateStoreShadow(shadowStore,
                                    prePruneContext,
                                    &scanNanoseconds,
                                    &compactNanoseconds,
                                    &indexRebuildNanoseconds);

  SimInitialSafeStorePruneIndexShadowStats stats;
  stats.calls = 1;
  stats.scanNanoseconds = scanNanoseconds;
  stats.compactNanoseconds = compactNanoseconds;
  stats.indexRebuildNanoseconds = indexRebuildNanoseconds;
  stats.nanoseconds = simElapsedNanoseconds(shadowStart);
  stats.statesScanned = scannedStates;
  stats.statesKept = static_cast<uint64_t>(shadowStore.states.size());
  if(stats.statesScanned > stats.statesKept)
  {
    stats.statesRemoved = stats.statesScanned - stats.statesKept;
  }
  else
  {
    stats.statesRemoved = realPruneStats.pruneRemovedStates;
  }

  const bool sizeMatch = shadowStore.valid == realPostPruneStore.valid &&
                         shadowStore.states.size() == realPostPruneStore.states.size();
  if(!sizeMatch)
  {
    stats.sizeMismatches = 1;
  }
  const bool orderMatch =
    sizeMatch &&
    simCudaCandidateStateVectorsEqualOrdered(shadowStore.states,realPostPruneStore.states);
  if(!orderMatch)
  {
    stats.orderMismatches = 1;
  }
  const bool candidateMatch =
    sizeMatch &&
    simCudaCandidateStateVectorsEqualAsSet(shadowStore.states,realPostPruneStore.states);
  if(!candidateMatch)
  {
    stats.candidateMismatches = 1;
  }
...
inline void recordSimInitialSafeStorePruneIndexShadowIfEnabled(
  const SimKernelContext *prePruneContext,
  const SimCandidateStateStore &realPostPruneStore,
  const SimInitialSafeStoreRebuildStats &realPruneStats)
{
  if(prePruneContext == NULL ||
     !simCudaInitialSafeStorePruneIndexShadowEnabledRuntime())
  {
    return;
  }
  const SimInitialSafeStorePruneIndexShadowStats shadowStats =
    runSimInitialSafeStorePruneIndexShadow(*prePruneContext,
                                           realPostPruneStore,
                                           realPruneStats);
  recordSimInitialSafeStorePruneIndexShadow(shadowStats);
  ...
  {
    fprintf(stderr,
            "SIM CUDA initial safe-store prune/index shadow mismatch: "
            "digest=%llu size=%llu candidate=%llu order=%llu\n",
...
inline void recordSimInitialSafeStoreGpuPrecombinePruneShadowIfEnabled(
  const SimKernelContext *prePruneContext,
  const SimCandidateStateStore &realPostPruneStore)
{
  if(prePruneContext == NULL ||
     !simCudaInitialSafeStoreGpuPrecombinePruneShadowEnabledRuntime())
  {
    return;
  }
  const SimInitialSafeStoreGpuPrecombinePruneShadowStats shadowStats =
    runSimInitialSafeStoreGpuPrecombinePruneShadow(*prePruneContext,
                                                   realPostPruneStore);
  recordSimInitialSafeStoreGpuPrecombinePruneShadow(shadowStats);
  if((shadowStats.digestMismatches != 0 ||
      shadowStats.sizeMismatches != 0 ||
      shadowStats.candidateMismatches != 0 ||
      shadowStats.orderMismatches != 0) &&
  ...
  {
    fprintf(stderr,
            "SIM CUDA initial safe-store GPU precombine prune shadow mismatch: "
            "digest=%llu size=%llu candidate=%llu order=%llu\n",
            static_cast<unsigned long long>(shadowStats.digestMismatches),
            static_cast<unsigned long long>(shadowStats.sizeMismatches),
            static_cast<unsigned long long>(shadowStats.candidateMismatches),
...
runSimSafeStoreMergeStructureShadow(const SimCandidateStateStore &preMergeStore,
                                    ...
                                    uint64_t currentEraseScannedStates,
                                    uint64_t currentPruneScannedStates,
                                    uint64_t currentPruneRemovedStates)
{
  const std::chrono::steady_clock::time_point shadowStart =
    std::chrono::steady_clock::now();
  ...
  if(preMergeStore.valid)
  {
    ...
    {
      upsertSimCandidateStateStoreState(updatedAffectedStates[stateIndex],shadowStore);
    }
    pruneSimCandidateStateStoreShadow(shadowStore,postMergeContext);
  }

  SimSafeStoreMergeStructureShadowStats stats;
  stats.calls = 1;
  stats.nanoseconds = simElapsedNanoseconds(shadowStart);
  stats.estCurrentScannedStates = currentEraseScannedStates + currentPruneScannedStates;
  stats.estCompactScannedStates =
    static_cast<uint64_t>(preMergeStore.valid ? preMergeStore.states.size() : 0) +
    static_cast<uint64_t>(updatedAffectedStates.size());
...
struct SimSafeWorksetDeferredIndexBuildStats
{
  SimSafeWorksetDeferredIndexBuildStats():
    eraseScanLookupNanoseconds(0),
    eraseCompactNanoseconds(0),
    upsertNanoseconds(0),
    pruneScanNanoseconds(0),
    pruneCompactNanoseconds(0),
    indexRebuildNanoseconds(0),
    storeSizeBefore(0),
    storeSizeAfterErase(0),
    storeSizeAfterUpsert(0),
    storeSizeAfterPrune(0),
    erasedStates(0),
    pruneScannedStates(0),
    pruneRemovedStates(0),
    indexEntriesBefore(0),
    eraseIndexEntriesAfter(0),
    pruneIndexEntriesBefore(0),
    pruneIndexEntriesAfter(0),
    actualIndexRebuilds(0)
  {
  }

  uint64_t eraseScanLookupNanoseconds;
  uint64_t eraseCompactNanoseconds;
  uint64_t upsertNanoseconds;
  uint64_t pruneScanNanoseconds;
  uint64_t pruneCompactNanoseconds;
  uint64_t indexRebuildNanoseconds;
  uint64_t storeSizeBefore;
  uint64_t storeSizeAfterErase;
  uint64_t storeSizeAfterUpsert;
  uint64_t storeSizeAfterPrune;
  uint64_t erasedStates;
  uint64_t pruneScannedStates;
  uint64_t pruneRemovedStates;
  uint64_t indexEntriesBefore;
  uint64_t eraseIndexEntriesAfter;
  uint64_t pruneIndexEntriesBefore;
  uint64_t pruneIndexEntriesAfter;
  uint64_t actualIndexRebuilds;
};

inline SimCandidateStateStore buildSimSafeWorksetDeferredIndexStore(
  const SimCandidateStateStore &preMergeStore,
  const vector<uint64_t> &uniqueAffected,
  const vector<SimScanCudaCandidateState> &updatedAffectedStates,
  const SimKernelContext &postMergeContext,
  SimSafeWorksetDeferredIndexBuildStats *buildStats)
{
  SimCandidateStateStore store;
  store.valid = preMergeStore.valid;
  store.hostEpoch = preMergeStore.hostEpoch;
  if(!preMergeStore.valid)
  {
    return store;
  }
  ...
  if(buildStats != NULL)
  {
    buildStats->storeSizeBefore =
      static_cast<uint64_t>(preMergeStore.states.size());
    buildStats->indexEntriesBefore =
      static_cast<uint64_t>(preMergeStore.startCoordToIndex.size());
  }

  store.states.reserve(preMergeStore.states.size() + updatedAffectedStates.size());
  bool returnedStatesLimitedToAffectedStarts = true;
  for(size_t stateIndex = 0; stateIndex < updatedAffectedStates.size(); ++stateIndex)
  {
    const uint64_t startCoord =
      simScanCudaCandidateStateStartCoord(updatedAffectedStates[stateIndex]);
    if(!binary_search(uniqueAffected.begin(),uniqueAffected.end(),startCoord))
    {
      returnedStatesLimitedToAffectedStarts = false;
      break;
    }
  }
  unordered_map<uint64_t,size_t> startCoordToIndex;
  if(!returnedStatesLimitedToAffectedStarts)
  {
    startCoordToIndex.reserve(preMergeStore.states.size() + updatedAffectedStates.size());
  }

  const std::chrono::steady_clock::time_point eraseScanStart =
    buildStats != NULL ? std::chrono::steady_clock::now() : std::chrono::steady_clock::time_point();
  for(size_t stateIndex = 0; stateIndex < preMergeStore.states.size(); ++stateIndex)
  {
    const SimScanCudaCandidateState &state = preMergeStore.states[stateIndex];
    const uint64_t startCoord = simScanCudaCandidateStateStartCoord(state);
    ...
    if(!returnedStatesLimitedToAffectedStarts)
    {
      startCoordToIndex[startCoord] = store.states.size();
    }
    store.states.push_back(state);
  }
  if(buildStats != NULL)
  {
    buildStats->eraseScanLookupNanoseconds = simElapsedNanoseconds(eraseScanStart);
    buildStats->storeSizeAfterErase =
      static_cast<uint64_t>(store.states.size());
    buildStats->eraseIndexEntriesAfter =
      static_cast<uint64_t>(store.states.size());
    if(buildStats->storeSizeBefore > buildStats->storeSizeAfterErase)
    {
      buildStats->erasedStates =
        buildStats->storeSizeBefore - buildStats->storeSizeAfterErase;
    }
  }

  const std::chrono::steady_clock::time_point upsertStart =
    buildStats != NULL ? std::chrono::steady_clock::now() : std::chrono::steady_clock::time_point();
  unordered_map<uint64_t,size_t> returnedStartCoordToIndex;
  if(returnedStatesLimitedToAffectedStarts)
  {
    returnedStartCoordToIndex.reserve(updatedAffectedStates.size());
  }
  for(size_t stateIndex = 0; stateIndex < updatedAffectedStates.size(); ++stateIndex)
  {
    const SimScanCudaCandidateState &state = updatedAffectedStates[stateIndex];
    const uint64_t startCoord = simScanCudaCandidateStateStartCoord(state);
    unordered_map<uint64_t,size_t> &upsertIndex =
      returnedStatesLimitedToAffectedStarts ? returnedStartCoordToIndex : startCoordToIndex;
    unordered_map<uint64_t,size_t>::iterator found = upsertIndex.find(startCoord);
    if(found == upsertIndex.end())
    {
      upsertIndex[startCoord] = store.states.size();
      store.states.push_back(state);
    }
    else
    {
  ...
  if(buildStats != NULL)
  {
    buildStats->upsertNanoseconds = simElapsedNanoseconds(upsertStart);
    buildStats->storeSizeAfterUpsert =
      static_cast<uint64_t>(store.states.size());
    buildStats->pruneIndexEntriesBefore =
      static_cast<uint64_t>(store.states.size());
  }
  ...
  keptStates.reserve(store.states.size());
  const long floor = postMergeContext.runningMin;
  const std::chrono::steady_clock::time_point pruneScanStart =
    buildStats != NULL ? std::chrono::steady_clock::now() : std::chrono::steady_clock::time_point();
  for(size_t stateIndex = 0; stateIndex < store.states.size(); ++stateIndex)
  {
    const SimScanCudaCandidateState &state = store.states[stateIndex];
  ...
  if(buildStats != NULL)
  {
    buildStats->pruneScanNanoseconds = simElapsedNanoseconds(pruneScanStart);
    buildStats->pruneScannedStates =
      static_cast<uint64_t>(store.states.size());
  }

  const std::chrono::steady_clock::time_point pruneCompactStart =
    buildStats != NULL ? std::chrono::steady_clock::now() : std::chrono::steady_clock::time_point();
  store.states.swap(keptStates);
  if(buildStats != NULL)
  {
    buildStats->pruneCompactNanoseconds = simElapsedNanoseconds(pruneCompactStart);
    buildStats->storeSizeAfterPrune =
      static_cast<uint64_t>(store.states.size());
    if(buildStats->pruneScannedStates > buildStats->storeSizeAfterPrune)
    {
      buildStats->pruneRemovedStates =
        buildStats->pruneScannedStates - buildStats->storeSizeAfterPrune;
    }
  }
  ...
  if(buildStats != NULL)
  {
    buildStats->indexRebuildNanoseconds = simElapsedNanoseconds(indexRebuildStart);
    buildStats->pruneIndexEntriesAfter =
      static_cast<uint64_t>(store.startCoordToIndex.size());
    buildStats->actualIndexRebuilds = 1;
  }

  return store;
}

inline void addSimSafeWorksetDeferredIndexMismatches(
  const SimCandidateStateStore &candidateStore,
  const SimCandidateStateStore &referenceStore,
  uint64_t candidateKeptStates,
  uint64_t candidateRemovedStates,
  uint64_t referenceKeptStates,
  uint64_t referenceRemovedStates,
  SimSafeWorksetDeferredIndexStats &stats)
{
  const bool sizeMatch = candidateStore.valid == referenceStore.valid &&
                         candidateStore.states.size() == referenceStore.states.size();
  if(!sizeMatch)
  {
    stats.sizeMismatches = 1;
  }
  const bool orderMatch =
    sizeMatch &&
    simCudaCandidateStateVectorsEqualOrdered(candidateStore.states,
                                             referenceStore.states);
  if(!orderMatch)
  {
    stats.orderMismatches = 1;
  ...
  const bool candidateMatch =
    sizeMatch &&
    simCudaCandidateStateVectorsEqualAsSet(candidateStore.states,
                                           referenceStore.states);
  if(!candidateMatch)
  {
    stats.candidateMismatches = 1;
  }
  if(simCandidateStateStoreFingerprint(candidateStore) !=
     simCandidateStateStoreFingerprint(referenceStore))
  {
    stats.digestMismatches = 1;
  }
  if(candidateStore.startCoordToIndex.size() != referenceStore.startCoordToIndex.size())
  {
    stats.indexEntryMismatches = 1;
  }
  if(candidateKeptStates != referenceKeptStates)
  {
    stats.keptCountMismatches = 1;
  }
  if(candidateRemovedStates != referenceRemovedStates)
  {
    stats.removedCountMismatches = 1;
  }
}

inline bool simSafeWorksetDeferredIndexStatsHaveMismatch(
  const SimSafeWorksetDeferredIndexStats &stats)
{
  return stats.sizeMismatches != 0 ||
         stats.candidateMismatches != 0 ||
         stats.orderMismatches != 0 ||
         stats.digestMismatches != 0 ||
         stats.indexEntryMismatches != 0 ||
         stats.keptCountMismatches != 0 ||
         stats.removedCountMismatches != 0;
}
...
inline void runSimCandidateLoop(const SimRequest &request,
	...
	inline void runSimInitialExactFrontierOneChunkBoundedShadowIfRequested(
	  const vector<SimScanCudaInitialRunSummary> &summaries,
	  bool benchmarkEnabled)
	{
	  if(!benchmarkEnabled ||
	  ...
	  if(!sim_scan_cuda_reduce_initial_run_summaries_for_test(
	       ...
	       &shadowError))
	  {
	    if(simCudaValidateEnabledRuntime() && !shadowError.empty())
	    {
	      fprintf(stderr,
	              "SIM CUDA bounded one-chunk exact frontier shadow failed: %s\n",
	              shadowError.c_str());
	    }
	    return;
	  }
	...
	inline void runSimInitialExactFrontierPerRequestShadowIfRequested(
	  const vector<SimScanCudaInitialRunSummary> &summaries,
	  bool benchmarkEnabled)
	{
	  if(!benchmarkEnabled ||
	     !simCudaInitialExactFrontierPerRequestShadowRequestedRuntime())
	  {
	    return;
	  }
		  const uint64_t requestIndex =
		  ...
		  SimScanCudaInitialReduceReplayStats replayStats;
		  string shadowError;
		  const bool residentSourceRequested =
		    simCudaInitialExactFrontierPerRequestShadowResidentSourceRequestedRuntime();
		  bool residentSourceActive = false;
		  bool residentSourceFallback = false;
		  uint64_t h2dBytes = 0;
		  uint64_t summaryH2DBytesSaved = 0;
		  const uint64_t summaryBytes =
		    static_cast<uint64_t>(summaries.size()) *
		    static_cast<uint64_t>(sizeof(SimScanCudaInitialRunSummary));
		  const std::chrono::steady_clock::time_point shadowStart =
		    std::chrono::steady_clock::now();
		  const std::chrono::steady_clock::time_point kernelStart =
		    std::chrono::steady_clock::now();
		  bool gpuReplayOk = false;
		  if(residentSourceRequested)
		  {
		    gpuReplayOk =
		      sim_scan_cuda_reduce_initial_run_summaries_resident_for_test(
		        summaries.size(),
		        &gpuStates,
		        &gpuRunningMin,
		        &replayStats,
		    ...
		    if(gpuReplayOk)
		    {
		      residentSourceActive = true;
		      summaryH2DBytesSaved += summaryBytes;
		    }
		    else
		    {
		      residentSourceFallback = true;
		      if(simCudaValidateEnabledRuntime() && !shadowError.empty())
		      {
		        fprintf(stderr,
		                "SIM CUDA per-request exact frontier resident shadow replay unavailable, falling back to host H2D: %s\n",
		                shadowError.c_str());
		      }
		    }
		  }
		  if(!gpuReplayOk)
		  {
		    shadowError.clear();
		    gpuReplayOk =
		      sim_scan_cuda_reduce_initial_run_summaries_for_test(
		        summaries,
		        &gpuStates,
		        &gpuRunningMin,
		        &replayStats,
		        &shadowError);
		    h2dBytes += summaryBytes;
		  }
		  if(!gpuReplayOk)
		  {
		    if(simCudaValidateEnabledRuntime() && !shadowError.empty())
		    {
	      fprintf(stderr,
	              "SIM CUDA per-request exact frontier shadow failed: %s\n",
	              shadowError.c_str());
	    }
	    return;
	  }
	  ...
	  double safeStoreBuildSeconds = 0.0;
		  ...
		  bool safeStoreBuildOk = false;
		  if(residentSourceActive)
		  {
		    safeStoreBuildOk =
		      sim_scan_cuda_build_persistent_safe_candidate_state_store_from_resident_initial_run_summaries_for_shadow(
		        summaries.size(),
		        gpuStates,
		        gpuRunningMin,
		        &gpuSafeStore,
		    ...
		    else
		    {
		      ...
		      {
		        fprintf(stderr,
		                "SIM CUDA per-request exact frontier resident safe-store shadow unavailable, falling back to host H2D: %s\n",
		  ...
		  if(!safeStoreBuildOk)
		  {
		    safeStoreError.clear();
		    safeStoreBuildOk =
		      sim_scan_cuda_build_persistent_safe_candidate_state_store_from_initial_run_summaries(
		        summaries,
		        gpuStates,
		        gpuRunningMin,
		        &gpuSafeStore,
	    ...
	    if(simCudaValidateEnabledRuntime() && !safeStoreError.empty())
	    {
	      fprintf(stderr,
		    ...
		    return;
			  ...
			  const uint64_t kernelNanoseconds = simElapsedNanoseconds(kernelStart);
			  const std::chrono::steady_clock::time_point compareStart =
			    std::chrono::steady_clock::now();
			  vector<SimScanCudaCandidateState> cpuStates;
			  int cpuRunningMin = 0;
	    ...
	    compareSimInitialExactFrontierOneChunkSnapshots(
	      ...
	      makeSimInitialExactFrontierOneChunkSnapshotForBoundedShadow(
	        ...
	        gpuRunningMin,
		        true,
		        gpuSafeStoreDigest));
		  const uint64_t compareNanoseconds = simElapsedNanoseconds(compareStart);
		  const uint64_t shadowNanoseconds = simElapsedNanoseconds(shadowStart);
	  ...
	  if(!frontierMatch || !safeStoreMatch)
	  {
		  ...
		  bool hostSafeStoreAlreadyPruned)
		{
		  recordSimInitialExactFrontierOneChunkInputAdapterIfRequested(
		    summaries,
		    benchmarkEnabled);
		  runSimInitialExactFrontierOneChunkBoundedShadowIfRequested(
		    summaries,
		    benchmarkEnabled);
		  runSimInitialExactFrontierPerRequestShadowIfRequested(
		    summaries,
		    benchmarkEnabled);
		  const bool maintainSafeStore =
		    simLocateCudaModeRuntime() == SIM_LOCATE_CUDA_MODE_SAFE_WORKSET ||
		  ...
		  if(maintainSafeStore)
		  {
		    ...
		    if(!maintainedSafeStoreOnDevice)
		    {
		      ...
		      if(!hostSafeStoreAlreadyUpdated)
		      {
		        ...
		        bool gpuPrecombinePrunedApplied = false;
		        if(simCudaInitialSafeStoreGpuPrecombineRequestedRuntime())
		        {
		          ...
		          bool gpuPrecombinePruned = false;
		          SimInitialSafeStoreGpuPrecombinePruneStats gpuPrecombinePruneStats;
		          const SimInitialSafeStoreGpuPrecombineStats gpuPrecombineStats =
		            runSimInitialSafeStoreGpuPrecombine(
		              ...
		              gpuPrecombineOk,
		              gpuPrecombinePruned,
		              &gpuPrecombinePruneStats);
		          recordSimInitialSafeStoreGpuPrecombine(gpuPrecombineStats);
		          recordSimInitialSafeStoreGpuPrecombinePrune(gpuPrecombinePruneStats);
		          if(gpuPrecombineOk)
		          {
		            ...
		            gpuPrecombinePrunedApplied = gpuPrecombinePruned;
		          }
		        }
		        ...
		        if(benchmarkEnabled &&
		           simCudaInitialSafeStoreGpuPrecombineShadowEnabledRuntime())
		        {
		          const SimInitialSafeStoreGpuPrecombineShadowStats gpuShadowStats =
		            runSimInitialSafeStoreGpuPrecombineShadow(
		              summaries,
		              context.safeCandidateStateStore);
		          recordSimInitialSafeStoreGpuPrecombineShadow(gpuShadowStats);
		          if((gpuShadowStats.digestMismatches != 0 ||
		              gpuShadowStats.sizeMismatches != 0 ||
		              gpuShadowStats.candidateMismatches != 0 ||
		              gpuShadowStats.orderMismatches != 0) &&
		             simCudaValidateEnabledRuntime())
		          {
		            fprintf(stderr,
		                    "SIM CUDA initial safe-store GPU precombine shadow mismatch: "
		                    "digest=%llu size=%llu candidate=%llu order=%llu\n",
		                    static_cast<unsigned long long>(gpuShadowStats.digestMismatches),
		                    static_cast<unsigned long long>(gpuShadowStats.sizeMismatches),
		                    static_cast<unsigned long long>(gpuShadowStats.candidateMismatches),
		                    static_cast<unsigned long long>(gpuShadowStats.orderMismatches));
		          }
		        }
		        if(gpuPrecombinePrunedApplied)
		        {
		          hostSafeStoreAlreadyPruned = true;
		        }
		      }
		      if(!hostSafeStoreAlreadyPruned)
		      {
		        const bool runPruneIndexShadow =
		          benchmarkEnabled &&
		          simCudaInitialSafeStorePruneIndexShadowEnabledRuntime();
		        const bool runGpuPrecombinePruneShadow =
		          benchmarkEnabled &&
		          simCudaInitialSafeStoreGpuPrecombinePruneShadowEnabledRuntime();
		        unique_ptr<SimKernelContext> prePruneShadowContext;
		        if(runPruneIndexShadow || runGpuPrecombinePruneShadow)
		        {
		          prePruneShadowContext.reset(new SimKernelContext(context));
		        }
		        ...
		        recordSimInitialSafeStorePruneIndexShadowIfEnabled(
		          prePruneShadowContext.get(),
		          context.safeCandidateStateStore,
		          safeStoreRebuildStats);
		        recordSimInitialSafeStoreGpuPrecombinePruneShadowIfEnabled(
		          prePruneShadowContext.get(),
		          context.safeCandidateStateStore);
		      }
		      if(benchmarkEnabled)
                                            ...
                                            SimSafeWorksetFallbackReason *fallbackReason)
{
  ...
  if(useResidencyMaintenance)
  {
    ...
    const std::chrono::steady_clock::time_point cpuMergeStart =
      benchmarkEnabled ? std::chrono::steady_clock::now() : std::chrono::steady_clock::time_point();
    const std::chrono::steady_clock::time_point candidateApplyStart =
      benchmarkEnabled ? std::chrono::steady_clock::now() : std::chrono::steady_clock::time_point();
    ...
    if(benchmarkEnabled)
    {
      ...
      if(recordTelemetry)
      {
        recordSimSafeWorksetMerge(residencyResult.updatedStateCount,
                                  cpuMergeNanoseconds);
        const uint64_t frontierStateCount =
          static_cast<uint64_t>(residencyResult.frontierStates.size());
        SimSafeWorksetMergeBreakdownStats mergeBreakdown;
        mergeBreakdown.add(SIM_SAFE_WORKSET_MERGE_CANDIDATE_APPLY_NANOSECONDS,
                           candidateApplyNanoseconds);
        const uint64_t uniqueStartKeyCount =
          countUniqueSimCandidateStateStartCoords(residencyResult.frontierStates);
        mergeBreakdown.add(SIM_SAFE_WORKSET_MERGE_UNIQUE_START_KEY_COUNT,
                           uniqueStartKeyCount);
        mergeBreakdown.add(SIM_SAFE_WORKSET_MERGE_DUPLICATE_STATE_COUNT,
                           frontierStateCount - uniqueStartKeyCount);
        mergeBreakdown.add(SIM_SAFE_WORKSET_MERGE_CANDIDATE_UPDATE_COUNT,
                           frontierStateCount);
        mergeBreakdown.add(SIM_SAFE_WORKSET_MERGE_RESIDENCY_UPDATE_COUNT,
                           residencyResult.updatedStateCount);
        recordSimSafeWorksetMergeBreakdown(mergeBreakdown);
      }
    }
  ...
  const bool safeWorksetDeferredIndexRequested =
    simSafeWorksetDeferredIndexEnabledRuntime();
  const bool safeWorksetDeferredIndexValidateEnabled =
    safeWorksetDeferredIndexRequested &&
    simSafeWorksetDeferredIndexValidateEnabledRuntime();
  bool safeWorksetDeferredIndexActive = false;
  vector<SimScanCudaCandidateState> updatedAffectedStates;
  updatedAffectedStates.swap(aggregatedResult.candidateStates);
  if(benchmarkEnabled)
  {
    mergeBreakdown.add(SIM_SAFE_WORKSET_MERGE_MATERIALIZE_NANOSECONDS,
  ...
  if(benchmarkEnabled)
  {
    mergeBreakdown.add(SIM_SAFE_WORKSET_MERGE_CANDIDATE_ERASE_NANOSECONDS,
                       simElapsedNanoseconds(mergeStageStart));
    mergeStageStart = std::chrono::steady_clock::now();
  }
  if(context.safeCandidateStateStore.valid)
  {
    ...
    mergeBreakdown.add(SIM_SAFE_WORKSET_MERGE_SAFE_STORE_SIZE_BEFORE_COUNT,
                       safeStoreSizeBefore);
    safeWorksetDeferredIndexActive = safeWorksetDeferredIndexRequested;
    if(safeStoreMergeStructureShadowEnabled ||
       safeWorksetDeferredIndexShadowEnabled ||
       safeWorksetDeferredIndexActive)
    {
      safeStoreMergeShadowPreMergeStore = context.safeCandidateStateStore;
    }
    if(!safeWorksetDeferredIndexActive)
    {
  ...
  if(context.safeCandidateStateStore.valid && !safeWorksetDeferredIndexActive)
  {
  ...
  if(safeWorksetDeferredIndexActive && safeStoreMergeShadowPreMergeStore.valid)
  {
    const std::chrono::steady_clock::time_point deferredStart =
      std::chrono::steady_clock::now();
    SimSafeWorksetDeferredIndexStats deferredStats;
    deferredStats.active = 1;
    deferredStats.calls = 1;
    deferredStats.legacyIndexRebuilds = 2;

    SimSafeWorksetDeferredIndexBuildStats buildStats;
    SimCandidateStateStore deferredStore =
      buildSimSafeWorksetDeferredIndexStore(safeStoreMergeShadowPreMergeStore,
                                            uniqueAffected,
                                            updatedAffectedStates,
                                            context,
                                            &buildStats);
    deferredStats.actualIndexRebuilds = buildStats.actualIndexRebuilds;
    if(deferredStats.legacyIndexRebuilds > deferredStats.actualIndexRebuilds)
    {
      deferredStats.indexRebuildsSaved =
        deferredStats.legacyIndexRebuilds - deferredStats.actualIndexRebuilds;
    }

    SimCandidateStateStore legacyValidationStore;
    uint64_t legacyValidationKeptStates = buildStats.storeSizeAfterPrune;
    uint64_t legacyValidationRemovedStates = buildStats.pruneRemovedStates;
    if(safeWorksetDeferredIndexValidateEnabled)
    {
      const std::chrono::steady_clock::time_point validateStart =
        std::chrono::steady_clock::now();
      SimKernelContext legacyContext(context);
      legacyContext.safeCandidateStateStore = safeStoreMergeShadowPreMergeStore;
      eraseSimSafeCandidateStateStoreSortedUniqueStartCoords(
          uniqueAffected,
          legacyContext,
          NULL);
      ...
      const uint64_t legacyPrePruneSize =
        static_cast<uint64_t>(legacyContext.safeCandidateStateStore.states.size());
      pruneSimSafeCandidateStateStore(legacyContext,NULL,NULL);
      legacyValidationStore = legacyContext.safeCandidateStateStore;
      legacyValidationKeptStates =
        static_cast<uint64_t>(legacyValidationStore.states.size());
      legacyValidationRemovedStates =
        legacyPrePruneSize > legacyValidationKeptStates
          ? legacyPrePruneSize - legacyValidationKeptStates
          : 0;
      deferredStats.legacyValidateNanoseconds = simElapsedNanoseconds(validateStart);
      addSimSafeWorksetDeferredIndexMismatches(deferredStore,
                                               legacyValidationStore,
                                               buildStats.storeSizeAfterPrune,
                                               buildStats.pruneRemovedStates,
                                               legacyValidationKeptStates,
                                               legacyValidationRemovedStates,
    ...
    if(simSafeWorksetDeferredIndexStatsHaveMismatch(deferredStats))
    {
      deferredStats.fallbacks = 1;
      context.safeCandidateStateStore = std::move(legacyValidationStore);
    }
    else
    {
      context.safeCandidateStateStore = std::move(deferredStore);
      bumpSimCandidateStateStoreHostEpoch(context.safeCandidateStateStore);
      bumpSimCandidateStateStoreHostEpoch(context.safeCandidateStateStore);
    }
    if(recordTelemetry)
    ...
    safeStoreEraseStats.scannedStates = buildStats.storeSizeBefore;
    safeStoreEraseStats.erasedStates = buildStats.erasedStates;
    safeStoreEraseStats.indexEntriesBefore = buildStats.indexEntriesBefore;
    safeStoreEraseStats.indexEntriesAfter = buildStats.eraseIndexEntriesAfter;
    safeStoreEraseStats.scanLookupNanoseconds = buildStats.eraseScanLookupNanoseconds;
    safeStoreEraseStats.compactNanoseconds = buildStats.eraseCompactNanoseconds;
    safeWorksetPruneStructureStats.scanNanoseconds = buildStats.pruneScanNanoseconds;
    safeWorksetPruneStructureStats.compactNanoseconds = buildStats.pruneCompactNanoseconds;
    safeWorksetPruneStructureStats.indexRebuildNanoseconds = buildStats.indexRebuildNanoseconds;
    safeWorksetPruneStructureStats.indexEntriesBefore = buildStats.pruneIndexEntriesBefore;
    safeWorksetPruneStructureStats.indexEntriesAfter = buildStats.pruneIndexEntriesAfter;
    safeWorksetPruneStats.pruneIndexRebuildNanoseconds = buildStats.indexRebuildNanoseconds;
    safeStoreSizeAfterPrune = buildStats.storeSizeAfterPrune;
    safeStorePruneScannedStates = buildStats.pruneScannedStates;
    safeStorePruneRemovedStates = buildStats.pruneRemovedStates;

    mergeBreakdown.add(SIM_SAFE_WORKSET_MERGE_SAFE_STORE_ERASE_NANOSECONDS,
                       buildStats.eraseScanLookupNanoseconds + buildStats.eraseCompactNanoseconds);
    mergeBreakdown.add(SIM_SAFE_WORKSET_MERGE_SAFE_STORE_ERASE_SCAN_LOOKUP_NANOSECONDS,
                       buildStats.eraseScanLookupNanoseconds);
    mergeBreakdown.add(SIM_SAFE_WORKSET_MERGE_SAFE_STORE_ERASE_COMPACT_NANOSECONDS,
                       buildStats.eraseCompactNanoseconds);
    mergeBreakdown.add(SIM_SAFE_WORKSET_MERGE_SAFE_STORE_ERASE_INDEX_REBUILD_NANOSECONDS,0);
    mergeBreakdown.add(SIM_SAFE_WORKSET_MERGE_SAFE_STORE_ERASE_SCANNED_STATE_COUNT,
                       buildStats.storeSizeBefore);
    mergeBreakdown.add(SIM_SAFE_WORKSET_MERGE_SAFE_STORE_ERASED_STATE_COUNT,
                       buildStats.erasedStates);
    ...
    const uint64_t tombstoneEstMarkOps =
      static_cast<uint64_t>(uniqueAffected.size());
    const uint64_t tombstoneEstScanStatesSaved =
      buildStats.storeSizeBefore > tombstoneEstMarkOps
        ? buildStats.storeSizeBefore - tombstoneEstMarkOps
        : 0;
    mergeBreakdown.add(SIM_SAFE_WORKSET_MERGE_SAFE_STORE_TOMBSTONE_EST_MARK_OP_COUNT,
                       tombstoneEstMarkOps);
    ...
    mergeBreakdown.add(SIM_SAFE_WORKSET_MERGE_PRUNE_KEPT_STATE_COUNT,
                       safeStoreSizeAfterPrune);
    mergeBreakdown.add(SIM_SAFE_WORKSET_MERGE_SAFE_STORE_PRUNE_INDEX_REBUILD_NANOSECONDS,
                       buildStats.indexRebuildNanoseconds);
    mergeBreakdown.add(SIM_SAFE_WORKSET_MERGE_SAFE_STORE_PRUNE_SCAN_NANOSECONDS,
                       buildStats.pruneScanNanoseconds);
    mergeBreakdown.add(SIM_SAFE_WORKSET_MERGE_SAFE_STORE_PRUNE_COMPACT_NANOSECONDS,
                       buildStats.pruneCompactNanoseconds);
    mergeBreakdown.add(SIM_SAFE_WORKSET_MERGE_SAFE_STORE_PRUNE_INDEX_ENTRIES_BEFORE_COUNT,
                       buildStats.pruneIndexEntriesBefore);
    mergeBreakdown.add(SIM_SAFE_WORKSET_MERGE_SAFE_STORE_PRUNE_INDEX_ENTRIES_AFTER_COUNT,
                       buildStats.pruneIndexEntriesAfter);
    safeStoreEraseNanoseconds =
      buildStats.eraseScanLookupNanoseconds + buildStats.eraseCompactNanoseconds;
    safeStoreUpsertLoopNanoseconds = buildStats.upsertNanoseconds;
    deferredStats.nanoseconds = simElapsedNanoseconds(deferredStart);
    if(recordTelemetry)
    {
      recordSimSafeWorksetDeferredIndex(deferredStats);
      if(simSafeWorksetDeferredIndexStatsHaveMismatch(deferredStats) &&
         simCudaValidateEnabledRuntime())
      {
        fprintf(stderr,
                "SIM CUDA safe-workset deferred-index mismatch: "
                "digest=%llu size=%llu candidate=%llu order=%llu index=%llu kept=%llu removed=%llu\n",
                static_cast<unsigned long long>(deferredStats.digestMismatches),
                static_cast<unsigned long long>(deferredStats.sizeMismatches),
                static_cast<unsigned long long>(deferredStats.candidateMismatches),
                static_cast<unsigned long long>(deferredStats.orderMismatches),
                static_cast<unsigned long long>(deferredStats.indexEntryMismatches),
                static_cast<unsigned long long>(deferredStats.keptCountMismatches),
                static_cast<unsigned long long>(deferredStats.removedCountMismatches));
      }
    }
  }
  if(safeStoreMergeStructureShadowEnabled && safeStoreMergeShadowPreMergeStore.valid)
  {
    const SimSafeStoreMergeStructureShadowStats shadowStats =
      runSimSafeStoreMergeStructureShadow(safeStoreMergeShadowPreMergeStore,
                                          uniqueAffected,
                                          updatedAffectedStates,
                                          context,
                                          safeStoreSizeBefore,
                                          safeStorePruneScannedStates,
                                          safeStorePruneRemovedStates);
    recordSimSafeStoreMergeStructureShadow(shadowStats);
    if((shadowStats.digestMismatches != 0 ||
        shadowStats.sizeMismatches != 0 ||
        shadowStats.candidateMismatches != 0) &&
       recordTelemetry &&
    ...
    {
      fprintf(stderr,
              "SIM CUDA safe-store merge structure shadow mismatch: "
              "digest=%llu size=%llu candidate=%llu\n",
              static_cast<unsigned long long>(shadowStats.digestMismatches),
              static_cast<unsigned long long>(shadowStats.sizeMismatches),
              static_cast<unsigned long long>(shadowStats.candidateMismatches));
    }
  ...
  if(benchmarkEnabled)
  {
    mergeBreakdown.add(SIM_SAFE_WORKSET_MERGE_SAFE_STORE_PRUNE_NANOSECONDS,
                       simElapsedNanoseconds(mergeStageStart));
    mergeStageStart = std::chrono::steady_clock::now();
  }
  const bool hadGpuSafeCandidateStateStore = context.gpuSafeCandidateStateStore.valid;
  if(hadGpuSafeCandidateStateStore)
  {
    string gpuStoreError;
  ...
  if(benchmarkEnabled && hadGpuSafeCandidateStateStore)
  {
    mergeBreakdown.add(SIM_SAFE_WORKSET_MERGE_SAFE_STORE_UPLOAD_NANOSECONDS,
                       simElapsedNanoseconds(mergeStageStart));
  }
  ...
  if(benchmarkEnabled)
  {
    ...
    if(recordTelemetry)
    {
      const uint64_t uniqueReturnedStartKeyCount =
        countUniqueSimCandidateStateStartCoords(updatedAffectedStates);
      mergeBreakdown.add(SIM_SAFE_WORKSET_MERGE_UNIQUE_START_KEY_COUNT,
                         uniqueReturnedStartKeyCount);
      mergeBreakdown.add(SIM_SAFE_WORKSET_MERGE_DUPLICATE_STATE_COUNT,
                         returnedStateCount - uniqueReturnedStartKeyCount);
      recordSimSafeWorksetMerge(returnedStateCount,cpuMergeNanoseconds);
      recordSimSafeWorksetMergeBreakdown(mergeBreakdown);
    }
  }
  else if(recordTelemetry)
  {
    recordSimSafeWorksetMerge(returnedStateCount,0);
    recordSimSafeWorksetMergeBreakdown(mergeBreakdown);
  }
  if(recordTelemetry)
  {
