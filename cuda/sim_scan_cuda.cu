...
__global__ void sim_scan_copy_single_summary_to_candidate_state_kernel(
  const SimScanCudaInitialRunSummary *summaries,
  SimScanCudaCandidateState *candidateStates)
{
  if(threadIdx.x != 0 || blockIdx.x != 0)
  {
    return;
  }
  initSimScanCudaCandidateStateFromInitialRunSummary(summaries[0],candidateStates[0]);
}
                                                  ...
                                                  SimScanCudaBatchResult *batchResult)
{
  ...
  batchResult->regionPackedAggregationNoBlockedMetadataUploadSkips +=
    requestBatchResult.regionPackedAggregationNoBlockedMetadataUploadSkips;
                                                                  ...
                                                                  string *errorOut)
{
  ...
  *outDiagSeconds =
  ...
  if(uniqueCount < 0 || static_cast<size_t>(uniqueCount) > hashCapacity)
  {
    if(errorOut != NULL)
    {
      *errorOut = "SIM CUDA proposal online unique count overflow";
    }
    return false;
  }
  if(uniqueCount > 0 &&
     !ensure_sim_scan_cuda_buffer(&context->outputCandidateStatesDevice,
                                  &context->outputCandidateStatesCapacity,
                                  static_cast<size_t>(uniqueCount),
                                  errorOut))
  {
    return false;
  }
  if(outOutputBufferOverensureSkips != NULL &&
     static_cast<size_t>(uniqueCount) < hashCapacity)
  {
    *outOutputBufferOverensureSkips = 1;
  }
    ...
    *outRunSummaryCount = static_cast<uint64_t>(totalRunSummaries);
    if(totalRunSummaries > 0)
    {
      ...
      else
      {
        ...
        if(filterStartCoordCount == 0 && batchResult != NULL)
        {
          batchResult->regionSingleRequestNoFilterReduceKeyBufferEnsureSkips += 1;
        }
          ...
          else
          {
            if(reducedCandidateCount == 1)
            {
              if(batchResult != NULL)
              {
                ...
                batchResult->regionSingleRequestSingleCandidateOutputBufferEnsureSkips += 1;
              }
            }
            else
            {
  ...
  uint64_t *outZeroRunEventCountSkips = NULL)
{
  ...
  if(outNoBlockedMetadataUploadSkips != NULL)
  {
    *outNoBlockedMetadataUploadSkips = 0;
  }
  ...
  const uint64_t *blockedWordsDevice = NULL;
  const int *blockedBasesDevice = NULL;
  const int *blockedStartsDevice = NULL;
  if(totalBlockedWords > 0)
  {
    if(!ensure_sim_scan_cuda_buffer(&context->rowIntervalOffsetsDevice,
       ...
       !ensure_sim_scan_cuda_buffer(&context->batchRunningMinsDevice,
                                    ...
                                    static_cast<size_t>(batchSize),
                                    errorOut) ||
       !ensure_sim_scan_cuda_buffer(&context->blockedWordsDevice,
    ...
    blockedBasesDevice = context->rowIntervalOffsetsDevice;
    blockedStartsDevice = context->batchRunningMinsDevice;
    blockedWordsDevice = context->blockedWordsDevice;
  }
  else if(outNoBlockedMetadataUploadSkips != NULL)
  {
    *outNoBlockedMetadataUploadSkips = 1;
  }
  ...
  for(int batchIndex = 0; batchIndex < batchSize; ++batchIndex)
  {
    ...
    if(runCount < 0 || runCount > maxEventsAllowed)
    {
      if(errorOut != NULL)
      {
  ...
  for(int batchIndex = 0; batchIndex < batchSize; ++batchIndex)
  {
    ...
    const int runCount = (*outRunCounts)[static_cast<size_t>(batchIndex)];
    if(eventCount < 0 || eventCount > maxEventsAllowed || runCount > eventCount)
    {
      if(errorOut != NULL)
      {
                                                                                    ...
                                                                                    uint64_t *outNoFilterSliceReduceKeyBufferEnsureSkips = NULL)
{
  ...
  if(summaryCount == 1)
  {
    ...
    {
      const int outputCandidateCount = 1;
      cudaError_t countStatus =
        cudaMemcpy(context->batchCandidateCountsDevice + requestIndex,
                   &outputCandidateCount,
                   sizeof(int),
                   cudaMemcpyHostToDevice);
      if(countStatus != cudaSuccess)
    ...
    {
      *outNoFilterReservedCopySkips += 1;
    }
