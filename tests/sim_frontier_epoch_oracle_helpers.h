#ifndef LONGTARGET_TESTS_SIM_FRONTIER_EPOCH_ORACLE_HELPERS_H
#define LONGTARGET_TESTS_SIM_FRONTIER_EPOCH_ORACLE_HELPERS_H
...
namespace sim_frontier_epoch_test
{
...
static void collect_summary_epoch_ids(const FrontierOracleResult &oracle,
                                      size_t summaryCount,
                                      std::vector<uint64_t> &outSummaryEpochIds)
{
    outSummaryEpochIds.assign(summaryCount, 0);
    for (size_t traceIndex = 0; traceIndex < oracle.trace.size(); ++traceIndex)
    {
        const FrontierTraceEvent &event = oracle.trace[traceIndex];
        if (event.summaryOrdinal < outSummaryEpochIds.size())
        {
            outSummaryEpochIds[event.summaryOrdinal] = event.epochId;
        }
    }
}

static void collect_live_epoch_ids(const FrontierOracleResult &oracle,
                                   std::vector<uint64_t> &outLiveEpochIds)
{
    outLiveEpochIds.clear();
    for (size_t epochIndex = 0; epochIndex < oracle.epochs.size(); ++epochIndex)
    {
        if (oracle.epochs[epochIndex].live)
        {
            outLiveEpochIds.push_back(oracle.epochs[epochIndex].epochId);
        }
    }
    std::sort(outLiveEpochIds.begin(), outLiveEpochIds.end());
}
...
static TestCase make_reinsert_after_eviction_case()
{
    TestCase test;
    test.name = "reinsert_after_eviction";
...
static std::vector<TestCase> make_frontier_epoch_test_cases()
{
    std::vector<TestCase> tests;
    tests.push_back(make_contiguous_same_start_case());
    tests.push_back(make_reinsert_after_eviction_case());
                         ...
                         std::vector<std::string> &mismatches)
{
    ...
    {
        add_mismatch(mismatches, test.name, "safe_store_pruned", "store is invalid");
        ok = false;
    }
    if (!oracle.chunkable)
    {
