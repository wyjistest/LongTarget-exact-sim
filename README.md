...
- `benchmark.sim_initial_exact_frontier_shadow_has_ordered_digest_check` / `benchmark.sim_initial_exact_frontier_shadow_has_unordered_digest_check` / `benchmark.sim_initial_exact_frontier_shadow_has_min_candidate_check` / `benchmark.sim_initial_exact_frontier_contract_cpu_ordered_digest_available` / `benchmark.sim_initial_exact_frontier_contract_cpu_unordered_digest_available` / `benchmark.sim_initial_exact_frontier_contract_cpu_min_candidate_available`: exact frontier replay contract gap-fill fields. The `has_*_check` flags stay `0` until an actual CPU-vs-shadow comparison exists; the `contract_cpu_*_available` flags report whether the authoritative CPU final frontier can provide ordered digest, unordered digest, and min-candidate baseline state for future shadow comparison. These are availability fields, not mismatch counters.
