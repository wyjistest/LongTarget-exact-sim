...
- `benchmark.sim_region_packed_zero_run_candidate_buffer_ensure_skips`: aggregated region packed-path batches with no run summaries that skip candidate output buffer allocation checks
- `benchmark.sim_region_single_request_direct_reduce_zero_candidate_compact_buffer_ensure_skips`: direct-reduce single-request zero-candidate cases that skip compact-output buffer allocation checks
...
- `benchmark.sim_initial_true_batch_single_request_matrix_buffer_skips`: initial true-batch wrapper telemetry for single-request batch DP matrix buffer allocation checks skipped by reusing the scalar H score/coord buffers
- `benchmark.sim_initial_true_batch_single_request_diag_buffer_skips`: initial true-batch wrapper telemetry for single-request diagonal scratch buffer allocation checks skipped by reusing scalar diagonal scratch buffers
- `benchmark.sim_initial_true_batch_single_request_metadata_buffer_skips`: initial true-batch wrapper telemetry for single-request summary-only row/count/offset metadata buffer allocation checks skipped by reusing scalar metadata buffers
...
- `benchmark.sim_initial_true_batch_single_request_run_base_buffer_ensure_skips`: initial true-batch wrapper telemetry for single-request run-base buffer allocation checks skipped by using implicit base 0
- `benchmark.sim_initial_true_batch_single_request_run_base_materialize_skips`: initial true-batch wrapper telemetry for single-request run-base zero materialization elided by compacting/reducing summaries with implicit base 0
- `benchmark.sim_initial_true_batch_single_request_all_candidate_count_skips`: initial true-batch wrapper telemetry for single-request all-candidate count kernels skipped when unpruned reduced candidates are already counted on host
- `benchmark.sim_initial_true_batch_single_request_all_candidate_count_buffer_ensure_skips`: initial true-batch wrapper telemetry for single-request all-candidate count buffer allocation checks skipped when the implicit host count path does not need the device count buffer
- `benchmark.sim_initial_true_batch_single_request_all_candidate_base_buffer_ensure_skips`: initial true-batch wrapper telemetry for single-request all-candidate output-base buffer allocation checks skipped by compacting with implicit output base 0
- `benchmark.sim_initial_true_batch_single_request_all_candidate_base_upload_skips`: initial true-batch wrapper telemetry for single-request all-candidate output-base H2D uploads skipped by compacting with implicit output base 0
...
- `benchmark.sim_initial_hash_reduce_single_request_count_kernel_skips`: initial hash-reduce telemetry for single-request grouped-count kernels skipped by reading the compact cursor count instead
...
- `benchmark.sim_initial_true_batch_single_request_proposal_v3_state_count_upload_skips`: initial true-batch proposal V3 telemetry for single-request state-count H2D uploads skipped by using the scalar selector path
- `benchmark.sim_initial_true_batch_single_request_proposal_v3_selected_buffer_ensure_skips`: initial true-batch proposal V3 telemetry for single-request selected-state/count buffer allocation checks skipped by using the scalar selector path
...
- `benchmark.sim_initial_true_batch_single_request_proposal_v3_selected_compact_skips`: initial true-batch proposal V3 telemetry for single-request selected-state compact kernels skipped by copying the already contiguous selected slice
...
- `benchmark.sim_initial_segmented_single_request_all_candidate_count_kernel_skips`: segmented reducer telemetry for single-request safe-store compact count kernels skipped by reading the compact cursor count instead
