...
When Phase 3b recommends `design_device_side_ordered_candidate_maintenance_shadow`, Phase 3c is still design-only: `scripts/summarize_longtarget_device_ordered_maintenance_shadow_design.py --ordered-maintenance-budget-decision <ordered_candidate_maintenance_budget_decision.json> --output-dir <dir>` emits `device_ordered_maintenance_shadow_design.json` and `device_ordered_maintenance_shadow_design.md`. The artifact records `input_contract=row_run_summaries_ordered`, `ordering_contract=preserve_original_summary_order`, forbidden transforms such as `unordered_key_summary`, `victim_slot_reorder`, `lazy_generation_index`, and `floor_update_skip`, plus the required host-vs-shadow digest matrix. Active Phase 3c can recommend `implement_opt_in_device_ordered_maintenance_shadow`, but `runtime_prototype_allowed=false` and `default_path_changes_allowed=false`; the next implementation must be opt-in validation telemetry only, not a runtime replacement.

Phase 3d begins with the ordered-maintenance shadow validation contract, not a device/runtime implementation. The host path can now build a deterministic `SimOrderedMaintenanceHostDigest` over ordered row-run summary replay, final candidate state, candidate-index visibility, replacement sequence, running-min/floor sequence, and safe-store state. Benchmark telemetry always exposes the disabled/default shadow schema (`benchmark.sim_device_ordered_maintenance_shadow_*` plus host/shadow digest hashes); actual host digest recording is only activated by explicit shadow env flags (`LONGTARGET_SIM_DEVICE_ORDERED_MAINTENANCE_SHADOW=1`, optionally with `LONGTARGET_SIM_DEVICE_ORDERED_MAINTENANCE_SHADOW_VALIDATE=1`). `runtime_prototype_allowed=false` and `default_path_changes_allowed=false` remain in force.

Phase 3d.1 adds an opt-in independent CPU shadow replay. With `LONGTARGET_SIM_DEVICE_ORDERED_MAINTENANCE_SHADOW=1`, the benchmark copies the pre-merge context, replays the ordered row-run summaries through a shadow-only CPU path, compares host and shadow digests, and emits both `benchmark.sim_device_ordered_maintenance_shadow_*` and `benchmark.sim_ordered_maintenance_shadow_*` aliases. `LONGTARGET_SIM_DEVICE_ORDERED_MAINTENANCE_SHADOW_VALIDATE=1` turns a digest mismatch into an explicit failure. `scripts/summarize_longtarget_device_ordered_maintenance_shadow_validation.py --shadow-telemetry <projection-or-benchmark-json> --output-dir <dir>` emits `device_ordered_maintenance_shadow_validation_cases.tsv`, summary/decision JSON, and markdown; its only active next actions are `debug_shadow_mismatch`, `expand_shadow_coverage`, `profile_shadow_cost`, `collect_shadow_validation_telemetry`, or `enable_shadow_validation`. It still keeps `runtime_prototype_allowed=false`.

After Phase 3d.1 validation passes, `scripts/summarize_longtarget_device_ordered_maintenance_shadow_cost.py --shadow-validation-decision <device_ordered_maintenance_shadow_validation_decision.json> --cost-telemetry <baseline-or-shadow-json> --output-dir <dir>` summarizes baseline, `shadow_no_validate`, and `shadow_validate` cost evidence. It emits `device_ordered_maintenance_shadow_cost_cases.tsv`, summary/decision JSON, and markdown. The gate only permits `design_device_shadow_kernel`, `profile_shadow_cost_breakdown`, `debug_shadow_mismatch`, `expand_shadow_coverage`, `collect_shadow_cost_telemetry`, or `stop_device_ordered_shadow`; it keeps `runtime_prototype_allowed=false` and `default_path_changes_allowed=false`.

Phase 3d.2 opens the device-shadow lane only at the backend-interface level. `LONGTARGET_SIM_DEVICE_ORDERED_MAINTENANCE_SHADOW_BACKEND=cpu|device` selects the shadow backend; `cpu` is the existing independent CPU replay, while `device` currently records the authoritative host digest and emits `shadow_backend=device` with `shadow_status=not_supported`. No device result is connected to output, and the validation summarizer treats `device/not_supported` as incomplete coverage with `recommended_next_action=expand_device_shadow_coverage`. `runtime_prototype_allowed=false` and `default_path_changes_allowed=false` remain in force.

The first Phase 3d.2 device-shadow regression remains synthetic, but now covers the complete synthetic digest matrix: candidate-state, replacement-sequence, running-min/floor change-sequence, candidate-index visibility, and safe-store/handoff digest subsets. `make check-sim-ordered-maintenance-device-shadow` reuses the ordered CUDA initial-replay kernel and checks device final candidate state, runningMin, runningMinSlot, slot key/score/generation hashes, summary ordinal hash, replacement sequence hash/count, runningMin change hash/count, runningMinSlot change hash/count, floor change hash/count, candidate-index visibility sequence hash, candidate-index hit/miss/insert/erase/check counts, safe-store state hash/count, and candidate-state handoff hash/count against the host and CPU-shadow digests on small ordered sequences. It intentionally does not promote `shadow_backend=device` to `passed`, because real 5-case and 6-workload device shadow validation artifacts are still required before benchmark device validation can run.
