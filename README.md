...
The first Phase 3d.2 device-shadow regression remains synthetic, but now covers both candidate-state and replacement-sequence digest subsets: `make check-sim-ordered-maintenance-device-shadow` reuses the ordered CUDA initial-replay kernel and checks device final candidate state, runningMin, slot key/score/generation hashes, summary ordinal hash, handoff hash, replacement sequence hash, and replacement count against the host and CPU-shadow digests on small ordered sequences. It intentionally does not promote `shadow_backend=device` to `passed`, because running-min slot sequence, candidate-index visibility, and safe-store digest coverage are still required before real device shadow validation can run.
