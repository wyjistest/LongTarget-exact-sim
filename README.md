...
Phase 3d.2 opens the device-shadow lane only at the backend-interface level. `LONGTARGET_SIM_DEVICE_ORDERED_MAINTENANCE_SHADOW_BACKEND=cpu|device` selects the shadow backend; `cpu` is the existing independent CPU replay, while `device` currently records the authoritative host digest and emits `shadow_backend=device` with `shadow_status=not_supported`. No device kernel is implemented, no device result is connected to output, and the validation summarizer treats `device/not_supported` as incomplete coverage with `recommended_next_action=expand_device_shadow_coverage`. `runtime_prototype_allowed=false` and `default_path_changes_allowed=false` remain in force.

The first Phase 3d.2 device-shadow regression is synthetic and candidate-state-only: `make check-sim-ordered-maintenance-device-shadow` reuses the ordered CUDA initial-replay kernel and checks device final candidate state, runningMin, slot key/score/generation hashes, summary ordinal hash, and handoff hash against the host and CPU-shadow digests on small ordered sequences. It intentionally does not promote `shadow_backend=device` to `passed`, because replacement-sequence, candidate-index visibility, and safe-store digest coverage are still required before real device shadow validation can run.
