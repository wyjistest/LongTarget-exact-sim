...
- `LONGTARGET_SIM_CUDA_INITIAL_EXACT_FRONTIER_SHADOW_GATE=1`: default-off shadow contract gate stub for future GPU exact ordered frontier replay validation. It is not a replay implementation, never selects the experimental reducer or `gpu_real` authority by itself, and keeps CPU replay authoritative. Until a contract-compliant shadow backend and required mismatch counters exist, opt-in telemetry reports `active=0` with an explicit disabled reason.
...
- `LONGTARGET_SIM_CUDA_REGION_DEFERRED_COUNTS_VALIDATE=1`: default-off diagnostic validation for the single-request direct region reducer; compares the packed deferred-count snapshot against scalar event/run/candidate count copies, keeps current dispatch/output authoritative, and adds extra D2H/sync cost so validation timings are not production timings
...
- `LONGTARGET_SIM_CUDA_INITIAL_SUMMARY_HOST_COPY_ELISION=1`: experimental summary handoff measurement path that materializes single-request initial summaries directly into the result vector and elides the intermediate host copy; unsupported reduce/proposal cases keep the default path (default: off)
...
- `LONGTARGET_SIM_CUDA_SAFE_WINDOW_LARGE_GEOMETRY_SHADOW=1`: estimator-only diagnostic for large-window / high-inflation safe-window calls; records current coarsened cells, raw-geometry shadow cell estimates, and estimated saved cells without executing raw geometry or changing planner authority/region dispatch (default: off)
- `LONGTARGET_SIM_CUDA_SAFE_WINDOW_GEOMETRY_TELEMETRY=1`: record additional safe-window geometry counters for raw planned cells, max window/band cells, coarsening inflation, coarse distribution buckets, top inflated calls, and sparse_v2 selection/rejection outcomes (default: off; benchmark keys are still emitted as zero when not recorded)
...
- `benchmark.sim_safe_workset_merge_materialize_seconds` / `benchmark.sim_safe_workset_merge_candidate_erase_seconds` / `benchmark.sim_safe_workset_merge_safe_store_upsert_seconds` / `benchmark.sim_safe_workset_merge_candidate_apply_seconds` / `benchmark.sim_safe_workset_merge_safe_store_prune_seconds` / `benchmark.sim_safe_workset_merge_safe_store_prune_index_rebuild_seconds` / `benchmark.sim_safe_workset_merge_safe_store_upload_seconds`: telemetry-only breakdown of the existing exact-safe host merge; these fields do not skip merge work, change candidate update order, change safe-store refresh/upload behavior, or alter dispatch authority
- `benchmark.sim_safe_workset_merge_safe_store_erase_seconds` / `benchmark.sim_safe_workset_merge_safe_store_upsert_loop_seconds`: telemetry-only split of `sim_safe_workset_merge_safe_store_upsert_seconds` into affected-start safe-store erase and returned-state upsert-loop time; the original combined field remains for comparison
- `benchmark.sim_safe_workset_merge_safe_store_erase_scan_lookup_seconds` / `benchmark.sim_safe_workset_merge_safe_store_erase_compact_seconds` / `benchmark.sim_safe_workset_merge_safe_store_erase_index_rebuild_seconds` / `benchmark.sim_safe_workset_merge_safe_store_erase_scanned_states` / `benchmark.sim_safe_workset_merge_safe_store_erased_states` / `benchmark.sim_safe_workset_merge_safe_store_erase_index_entries_before` / `benchmark.sim_safe_workset_merge_safe_store_erase_index_entries_after`: telemetry-only structure split of affected-start safe-store erase; the scan/lookup bucket is the current full-store scan plus sorted affected-start lookup/copy loop, not a new erase path
- `benchmark.sim_safe_workset_merge_safe_store_prune_scan_seconds` / `benchmark.sim_safe_workset_merge_safe_store_prune_compact_seconds` / `benchmark.sim_safe_workset_merge_safe_store_prune_index_entries_before` / `benchmark.sim_safe_workset_merge_safe_store_prune_index_entries_after`: telemetry-only structure split of safe-workset safe-store prune around the existing predicate scan, vector swap, and index rebuild
- `benchmark.sim_safe_workset_merge_safe_store_tombstone_est_mark_ops` / `benchmark.sim_safe_workset_merge_safe_store_tombstone_est_erase_scan_states_saved` / `benchmark.sim_safe_workset_merge_safe_store_tombstone_est_erase_index_entries_saved`: telemetry-only first-order estimates for a hypothetical tombstone/generation erase strategy; no tombstone path is implemented or used
- `benchmark.sim_safe_workset_merge_unique_start_keys` / `benchmark.sim_safe_workset_merge_duplicate_states` / `benchmark.sim_safe_workset_merge_candidate_updates` / `benchmark.sim_safe_workset_merge_safe_store_updates` / `benchmark.sim_safe_workset_merge_residency_updates`: telemetry-only classification of safe-workset merge inputs and update paths; duplicate/start-key counts are diagnostic only and no returned state is skipped
- `benchmark.sim_safe_workset_merge_affected_start_keys` / `benchmark.sim_safe_workset_merge_safe_store_size_before` / `benchmark.sim_safe_workset_merge_safe_store_size_after_erase` / `benchmark.sim_safe_workset_merge_safe_store_size_after_upsert` / `benchmark.sim_safe_workset_merge_safe_store_size_after_prune` / `benchmark.sim_safe_workset_merge_prune_scanned_states` / `benchmark.sim_safe_workset_merge_prune_removed_states` / `benchmark.sim_safe_workset_merge_prune_kept_states`: telemetry-only safe-store merge structure counters used to characterize erase/upsert/prune work; they do not change safe-store ordering or pruning semantics
- `LONGTARGET_SIM_CUDA_SAFE_STORE_MERGE_STRUCTURE_SHADOW=1` enables an opt-in diagnostic shadow that rebuilds a compact safe-store candidate on a host-side snapshot after the existing exact merge remains authoritative. `benchmark.sim_safe_store_merge_shadow_enabled` / `benchmark.sim_safe_store_merge_shadow_calls` / `benchmark.sim_safe_store_merge_shadow_seconds` / `benchmark.sim_safe_store_merge_shadow_digest_mismatches` / `benchmark.sim_safe_store_merge_shadow_size_mismatches` / `benchmark.sim_safe_store_merge_shadow_candidate_mismatches` / `benchmark.sim_safe_store_merge_shadow_order_mismatches` / `benchmark.sim_safe_store_merge_shadow_est_current_scanned_states` / `benchmark.sim_safe_store_merge_shadow_est_compact_scanned_states` / `benchmark.sim_safe_store_merge_shadow_est_saved_scans` / `benchmark.sim_safe_store_merge_shadow_est_tombstone_mark_ops` / `benchmark.sim_safe_store_merge_shadow_est_tombstone_erase_scan_states_saved` / `benchmark.sim_safe_store_merge_shadow_est_tombstone_erase_index_entries_saved` / `benchmark.sim_safe_store_merge_shadow_prune_removed_ratio` report compare results and scan-work estimates only; the shadow store is never used for dispatch, upload, validation, or later locate/region work
- `LONGTARGET_SIM_CUDA_SAFE_WORKSET_DEFERRED_INDEX_SHADOW=1` enables a default-off diagnostic shadow that tests a safe-workset safe-store merge shape with affected-start erase, returned-state upsert, and prune all applied to a side store before rebuilding the safe-store index once at the end. `benchmark.sim_safe_workset_deferred_index_shadow_enabled` / `benchmark.sim_safe_workset_deferred_index_shadow_calls` / `benchmark.sim_safe_workset_deferred_index_shadow_seconds` / `benchmark.sim_safe_workset_deferred_index_shadow_input_states` / `benchmark.sim_safe_workset_deferred_index_shadow_returned_states` / `benchmark.sim_safe_workset_deferred_index_shadow_affected_start_keys` / `benchmark.sim_safe_workset_deferred_index_shadow_legacy_index_rebuilds` / `benchmark.sim_safe_workset_deferred_index_shadow_shadow_index_rebuilds` / `benchmark.sim_safe_workset_deferred_index_shadow_est_index_rebuilds_saved` / `benchmark.sim_safe_workset_deferred_index_shadow_index_rebuild_seconds` / `benchmark.sim_safe_workset_deferred_index_shadow_est_seconds_saved` / `benchmark.sim_safe_workset_deferred_index_shadow_kept_states` / `benchmark.sim_safe_workset_deferred_index_shadow_removed_states` / `benchmark.sim_safe_workset_deferred_index_shadow_index_entries` / mismatch counters report compare results and index-rebuild estimates only; the shadow store is never used as production authority
- `LONGTARGET_SIM_CUDA_SAFE_WORKSET_DEFERRED_INDEX=1` enables a default-off real safe-workset safe-store merge path that filters affected starts, upserts returned states, applies prune, and rebuilds the safe-store index once at final commit instead of rebuilding after erase and again after prune. `LONGTARGET_SIM_CUDA_SAFE_WORKSET_DEFERRED_INDEX_VALIDATE=1` builds the legacy two-rebuild result on a side store, compares size/candidate/order/digest/index-entry/kept/removed counts, and falls back to legacy on mismatch. `benchmark.sim_safe_workset_deferred_index_requested` / `benchmark.sim_safe_workset_deferred_index_active` / `benchmark.sim_safe_workset_deferred_index_validate_enabled` / `benchmark.sim_safe_workset_deferred_index_calls` / `benchmark.sim_safe_workset_deferred_index_seconds` / `benchmark.sim_safe_workset_deferred_index_legacy_validate_seconds` / `benchmark.sim_safe_workset_deferred_index_legacy_index_rebuilds` / `benchmark.sim_safe_workset_deferred_index_actual_index_rebuilds` / `benchmark.sim_safe_workset_deferred_index_index_rebuilds_saved` / mismatch counters / `benchmark.sim_safe_workset_deferred_index_fallbacks` report the opt-in. CPU host safe-store remains authoritative and region dispatch, safe-window planner, initial safe-store GPU best path, clean frontier gate, `gpu_real`, `ordered_segmented_v3`, and `EXACT_FRONTIER_REPLAY` are unchanged
...
- `benchmark.sim_safe_window_large_geometry_shadow_*`: default-off estimator-only diagnostic for `LONGTARGET_SIM_CUDA_SAFE_WINDOW_LARGE_GEOMETRY_SHADOW=1`; records selected large/high-inflation calls, raw/current cell totals, estimated saved cells, and explicit thresholds, but does not execute raw geometry or prove exactness
...
- `benchmark.sim_region_deferred_count_validate_enabled` / `benchmark.sim_region_deferred_count_validate_calls` / `benchmark.sim_region_deferred_count_total_mismatches`: default-off deferred-count validation state, coverage, and mismatch telemetry; validation compares packed snapshot counts to scalar count copies and reports event/run/candidate mismatch, fallback, scalar-copy, and snapshot-copy counters
...
- `benchmark.sim_initial_summary_host_copy_elision_enabled` / `benchmark.sim_initial_summary_d2h_copy_seconds` / `benchmark.sim_initial_summary_unpack_seconds` / `benchmark.sim_initial_summary_result_materialize_seconds` / `benchmark.sim_initial_summary_host_copy_elided_bytes`: initial summary handoff copy/materialization telemetry. The host-copy elision flag and elided-byte counter remain opt-in, while `sim_initial_summary_result_materialize_seconds` is also useful on the default path for separating result-vector materialization from D2H copy and CPU frontier replay.
...
- `benchmark.sim_initial_exact_frontier_shadow_gate_requested` / `benchmark.sim_initial_exact_frontier_shadow_gate_active` / `benchmark.sim_initial_exact_frontier_shadow_gate_authority` / `benchmark.sim_initial_exact_frontier_shadow_gate_disabled_reason` / `benchmark.sim_initial_exact_frontier_shadow_gate_calls` / `benchmark.sim_initial_exact_frontier_shadow_gate_supported` / `benchmark.sim_initial_exact_frontier_shadow_gate_missing_contract_counters`: clean shadow-only contract gate telemetry for future GPU exact ordered frontier replay work. Default logs report `requested=0`, `active=0`, `authority=cpu`, `calls=0`, and `disabled_reason=env_off`; opt-in logs keep `active=0` and `authority=cpu` with `disabled_reason=missing_contract_counters` until first-max/tie, min-candidate, candidate digest, safe-store digest/epoch, and chunk-boundary comparisons are available.
- `benchmark.sim_initial_exact_frontier_shadow_backend` / `benchmark.sim_initial_exact_frontier_shadow_backend_supported` / `benchmark.sim_initial_exact_frontier_shadow_backend_disabled_reason` / `benchmark.sim_initial_exact_frontier_shadow_one_chunk_supported` / `benchmark.sim_initial_exact_frontier_shadow_one_chunk_calls` / `benchmark.sim_initial_exact_frontier_shadow_one_chunk_final_compare_supported` / `benchmark.sim_initial_exact_frontier_shadow_one_chunk_missing_backend`: one-chunk exact frontier shadow backend stub telemetry. With the clean gate off, logs report `backend=none` and `disabled_reason=env_off`; with the gate requested, logs identify the future `one_chunk_stub` backend but keep it unsupported with `disabled_reason=one_chunk_backend_not_implemented`, zero calls, no final-state comparison support, CPU authority, and inactive gate state.
- `LONGTARGET_SIM_CUDA_INITIAL_EXACT_FRONTIER_ONE_CHUNK_INPUT_ADAPTER=1`: default-off one-chunk exact frontier input-adapter dry-run. It inspects the already materialized ordered initial run summaries and reports `benchmark.sim_initial_exact_frontier_shadow_one_chunk_input_adapter_*`, summary count/bytes, ordered input digest availability/value, field coverage, and unsupported record count. It does not run GPU replay, call the final comparator, emit mismatch counters, or change the clean shadow gate/replay backend state.
- `LONGTARGET_SIM_CUDA_INITIAL_EXACT_FRONTIER_ONE_CHUNK_BOUNDED_SHADOW=1` with `LONGTARGET_SIM_CUDA_INITIAL_EXACT_FRONTIER_ONE_CHUNK_BOUNDED_MAX_SUMMARIES=N`: default-off bounded GPU shadow for production-shaped ordered summaries. Optional `LONGTARGET_SIM_CUDA_INITIAL_EXACT_FRONTIER_ONE_CHUNK_BOUNDED_RANGE_MODE=prefix|tail|middle|request|offset`, `LONGTARGET_SIM_CUDA_INITIAL_EXACT_FRONTIER_ONE_CHUNK_BOUNDED_REQUEST_INDEX`, and `LONGTARGET_SIM_CUDA_INITIAL_EXACT_FRONTIER_ONE_CHUNK_BOUNDED_START_OFFSET` select deterministic ranges; tail/middle/offset are standalone subrange replays, not true intermediate production state. The conservative cap remains 65,536 summaries; `LONGTARGET_SIM_CUDA_INITIAL_EXACT_FRONTIER_ONE_CHUNK_BOUNDED_ALLOW_LARGE_PREFIX=1` raises the cap to 1,048,576 only for selected `request` mode. `LONGTARGET_SIM_CUDA_INITIAL_EXACT_FRONTIER_ONE_CHUNK_BOUNDED_ALLOW_FULL_REQUEST=1` raises the cap to 2,097,152 only when `RANGE_MODE=request` and an explicit `REQUEST_INDEX` are set; it does not affect prefix, tail, middle, offset, or all-request coverage. The result is never fed to output, safe-store, locate, region, planner authority, or the clean gate.
- `LONGTARGET_SIM_CUDA_INITIAL_EXACT_FRONTIER_PER_REQUEST_SHADOW=1`: default-off per-request exact frontier diagnostic shadow. Each request/window replays its ordered initial summaries independently on GPU, compares the GPU candidate and pruned safe-store digest snapshot to the CPU replay snapshot with CPU authority, and reports `benchmark.sim_initial_exact_frontier_per_request_shadow_*` counts, bytes, seconds, candidate mismatch counters, and `safe_store_digest_mismatches`. Optional `LONGTARGET_SIM_CUDA_INITIAL_EXACT_FRONTIER_PER_REQUEST_SHADOW_REQUEST_INDEX`, `LONGTARGET_SIM_CUDA_INITIAL_EXACT_FRONTIER_PER_REQUEST_SHADOW_MAX_REQUESTS`, and `LONGTARGET_SIM_CUDA_INITIAL_EXACT_FRONTIER_PER_REQUEST_SHADOW_REQUEST_LIST` select a deterministic request subset for cheaper smoke runs. `LONGTARGET_SIM_CUDA_INITIAL_EXACT_FRONTIER_PER_REQUEST_SHADOW_RESIDENT_SOURCE=1` asks the diagnostic to consume the GPU-resident initial summary buffer when available and reports resident-source requested/active/supported, disabled reason, input source, elided summary H2D bytes, kernel seconds, compare seconds, and fallbacks. It does not perform continuous all-request replay, does not feed GPU state into output, safe-store, locate, region, or planner authority, and reports `contract_level=candidate_safe_store`, `has_safe_store_contract=1`, `has_epoch_contract=0`, and `full_contract_disabled_reason=missing_epoch_contract` because GPU `telemetryEpoch` is not a host epoch; see `docs/cuda_exact_frontier_per_request_shadow_sampling.md`, `docs/cuda_exact_frontier_per_request_shadow_contract_status.md`, and `docs/cuda_exact_frontier_per_request_shadow_epoch_contract_gap.md`.
- `benchmark.sim_initial_exact_frontier_shadow_has_ordered_digest_check` / `benchmark.sim_initial_exact_frontier_shadow_has_unordered_digest_check` / `benchmark.sim_initial_exact_frontier_shadow_has_min_candidate_check` / `benchmark.sim_initial_exact_frontier_shadow_has_safe_store_digest_check` / `benchmark.sim_initial_exact_frontier_shadow_has_safe_store_epoch_check` / `benchmark.sim_initial_exact_frontier_shadow_has_first_max_check` / `benchmark.sim_initial_exact_frontier_shadow_has_tie_check` / `benchmark.sim_initial_exact_frontier_shadow_has_first_max_tie_check` / `benchmark.sim_initial_exact_frontier_contract_cpu_ordered_digest_available` / `benchmark.sim_initial_exact_frontier_contract_cpu_unordered_digest_available` / `benchmark.sim_initial_exact_frontier_contract_cpu_min_candidate_available` / `benchmark.sim_initial_exact_frontier_contract_cpu_safe_store_digest_available` / `benchmark.sim_initial_exact_frontier_contract_cpu_safe_store_epoch_available` / `benchmark.sim_initial_exact_frontier_contract_cpu_safe_store_epoch` / `benchmark.sim_initial_exact_frontier_contract_cpu_safe_store_epoch_bumps` / `benchmark.sim_initial_exact_frontier_contract_cpu_first_max_available` / `benchmark.sim_initial_exact_frontier_contract_cpu_tie_available` / `benchmark.sim_initial_exact_frontier_contract_cpu_first_max_tie_available`: exact frontier replay contract gap-fill fields. The `has_*_check` flags stay `0` until an actual CPU-vs-shadow comparison exists; the `contract_cpu_*_available` flags report whether the authoritative CPU final frontier or safe-store can provide ordered digest, unordered digest, min-candidate, safe-store digest, safe-store epoch, and first-max/tie baseline state for future shadow comparison. CPU safe-store digest availability requires a valid host safe-store at the final baseline point; CPU safe-store epoch availability requires the host store's committed-state generation. The epoch fields are CPU baseline telemetry only, not mismatch counters.
- `benchmark.sim_safe_store_host_epoch` / `benchmark.sim_safe_store_host_epoch_bumps`: host safe-store committed-state generation telemetry. The epoch advances on logical host safe-store commits such as reset/clear, post-update, post-prune, or replacement; it is separate from GPU `telemetryEpoch`, upload generation, stale invalidation, and digest comparison.
...
- `benchmark.sim_initial_scan_diag_seconds` / `benchmark.sim_initial_scan_online_reduce_seconds` / `benchmark.sim_initial_scan_wait_seconds`: CUDA-side initial scan subphase telemetry for diagnostic setup, optional online reduction, and device synchronization/wait work reported by the CUDA scanner
- `benchmark.sim_initial_scan_count_copy_seconds` / `benchmark.sim_initial_scan_base_upload_seconds`: scalar/count-copy and base-upload timing inside the CUDA initial scan handoff path; these are telemetry-only fields for critical-path attribution and do not change scan dispatch or replay authority
...
- `benchmark.sim_initial_context_apply_candidate_seconds` / `benchmark.sim_initial_context_apply_floor_seconds` / `benchmark.sim_initial_context_apply_frontier_seconds` / `benchmark.sim_initial_context_apply_safe_store_handoff_seconds` / `benchmark.sim_initial_context_apply_candidate_erase_seconds` / `benchmark.sim_initial_context_apply_candidate_insert_seconds` / `benchmark.sim_initial_context_apply_candidate_sort_seconds` / `benchmark.sim_initial_context_apply_running_min_updates` / `benchmark.sim_initial_context_apply_candidate_updates` / `benchmark.sim_initial_context_apply_frontier_updates` / `benchmark.sim_initial_context_apply_safe_store_handoffs` / `benchmark.sim_initial_context_apply_noop_events`: telemetry-only breakdown of initial context apply work. Current timers wrap the default summary-replay and chunk-skip replay local blocks when benchmark telemetry is enabled; zero-valued frontier/handoff/erase/insert/sort fields mean the covered path did not use a locally separated block
- `benchmark.sim_initial_candidate_replay_summaries` / `benchmark.sim_initial_candidate_replay_processed` / `benchmark.sim_initial_candidate_replay_accepted` / `benchmark.sim_initial_candidate_replay_rejected_below_floor` / `benchmark.sim_initial_candidate_replay_insertions` / `benchmark.sim_initial_candidate_replay_replacements` / `benchmark.sim_initial_candidate_replay_erasures` / `benchmark.sim_initial_candidate_replay_tie_updates` / `benchmark.sim_initial_candidate_replay_first_max_updates` / `benchmark.sim_initial_candidate_replay_final_candidates` / `benchmark.sim_initial_candidate_replay_survival_ratio`: telemetry-only structure counters for the initial context-apply candidate replay loop when benchmark telemetry is enabled. The counters classify the existing summary replay without skipping summaries, changing candidate order, changing floor/runningMin semantics, changing first-max/tie behavior, or changing safe-store update/prune/upload behavior
- `benchmark.sim_initial_candidate_churn_container_high_water` / `benchmark.sim_initial_candidate_churn_container_final_size` / `benchmark.sim_initial_candidate_churn_cumulative_container_size` / `benchmark.sim_initial_candidate_churn_replacement_chains` / `benchmark.sim_initial_candidate_churn_max_replacement_chain` / `benchmark.sim_initial_candidate_churn_overwritten_updates` / `benchmark.sim_initial_candidate_churn_final_survivor_updates` / `benchmark.sim_initial_candidate_churn_overwritten_ratio` / `benchmark.sim_initial_candidate_churn_first_max_updates` / `benchmark.sim_initial_candidate_churn_tie_updates` / `benchmark.sim_initial_candidate_churn_order_sensitive_updates` / `benchmark.sim_initial_candidate_churn_heap_builds` / `benchmark.sim_initial_candidate_churn_heap_updates` / `benchmark.sim_initial_candidate_churn_index_rebuilds`: telemetry-only candidate container churn counters for the covered initial replay path. Replacement chains and survivor counts are tracked by fixed candidate slot, so they estimate overwritten slot updates rather than global start-key ancestry. The fields are emitted with benchmark telemetry and do not add skip/filter behavior
- `LONGTARGET_SIM_CUDA_INITIAL_CANDIDATE_CONTAINER_SHADOW=1` enables a default-off estimator-only diagnostic for an alternative initial candidate container structure. `benchmark.sim_initial_candidate_container_shadow_enabled` / `benchmark.sim_initial_candidate_container_shadow_calls` / `benchmark.sim_initial_candidate_container_shadow_seconds` / `benchmark.sim_initial_candidate_container_shadow_state_mismatches` / `benchmark.sim_initial_candidate_container_shadow_size_mismatches` / `benchmark.sim_initial_candidate_container_shadow_digest_mismatches` / `benchmark.sim_initial_candidate_container_shadow_order_mismatches` / `benchmark.sim_initial_candidate_container_shadow_floor_mismatches` / `benchmark.sim_initial_candidate_container_shadow_min_candidate_mismatches` / `benchmark.sim_initial_candidate_container_shadow_events` / `benchmark.sim_initial_candidate_container_shadow_active_candidates` / `benchmark.sim_initial_candidate_container_shadow_stale_entries` / `benchmark.sim_initial_candidate_container_shadow_lazy_pops` / `benchmark.sim_initial_candidate_container_shadow_est_saved_erasures` / `benchmark.sim_initial_candidate_container_shadow_est_saved_index_rebuilds` / `benchmark.sim_initial_candidate_container_shadow_high_water_entries` / `benchmark.sim_initial_candidate_container_shadow_compaction_estimate` report final-state self-checks and lazy/versioned-container saved-work estimates only. The shadow does not add a real lazy/tombstone candidate path, does not change replay order, and does not feed output, safe-store, floor authority, locate, or region work
- `benchmark.sim_initial_safe_store_update_calls` / `benchmark.sim_initial_safe_store_update_summaries` / `benchmark.sim_initial_safe_store_update_inserted_states` / `benchmark.sim_initial_safe_store_update_merged_summaries` / `benchmark.sim_initial_safe_store_update_store_size_before` / `benchmark.sim_initial_safe_store_update_store_size_after`: telemetry-only shape counters for the initial CPU safe-store update loop; they decompose the existing host merge without changing replay order or safe-store contents
- `benchmark.sim_initial_safe_store_prune_calls` / `benchmark.sim_initial_safe_store_prune_scanned_states` / `benchmark.sim_initial_safe_store_prune_kept_states` / `benchmark.sim_initial_safe_store_prune_removed_states` / `benchmark.sim_initial_safe_store_prune_kept_above_floor` / `benchmark.sim_initial_safe_store_prune_kept_frontier` / `benchmark.sim_initial_safe_store_prune_index_rebuild_seconds`: telemetry-only counters for initial safe-store prune scan/rebuild work; kept-above-floor and kept-frontier are mutually exclusive keep reasons
- `LONGTARGET_SIM_CUDA_INITIAL_SAFE_STORE_PRUNE_INDEX_SHADOW=1` enables a default-off diagnostic shadow for initial safe-store prune/index structure. `benchmark.sim_initial_safe_store_prune_index_shadow_enabled` / `benchmark.sim_initial_safe_store_prune_index_shadow_calls` / `benchmark.sim_initial_safe_store_prune_index_shadow_seconds` / `benchmark.sim_initial_safe_store_prune_index_shadow_scan_seconds` / `benchmark.sim_initial_safe_store_prune_index_shadow_compact_seconds` / `benchmark.sim_initial_safe_store_prune_index_shadow_index_rebuild_seconds` / `benchmark.sim_initial_safe_store_prune_index_shadow_states_scanned` / `benchmark.sim_initial_safe_store_prune_index_shadow_states_kept` / `benchmark.sim_initial_safe_store_prune_index_shadow_states_removed` / `benchmark.sim_initial_safe_store_prune_index_shadow_removed_ratio` / `benchmark.sim_initial_safe_store_prune_index_shadow_size_mismatches` / `benchmark.sim_initial_safe_store_prune_index_shadow_candidate_mismatches` / `benchmark.sim_initial_safe_store_prune_index_shadow_order_mismatches` / `benchmark.sim_initial_safe_store_prune_index_shadow_digest_mismatches` report side-copy prune/index cost and exactness only; the shadow store is never used for output, upload, dispatch, validation authority, or later locate/region work
- `LONGTARGET_SIM_CUDA_INITIAL_SAFE_STORE_PRECOMBINE_SHADOW=1` enables a default-off diagnostic shadow that precombines duplicate-start initial run summaries into one candidate state per start, then compares that shadow store against the existing authoritative post-update safe store. `benchmark.sim_initial_safe_store_precombine_shadow_enabled` / `benchmark.sim_initial_safe_store_precombine_shadow_calls` / `benchmark.sim_initial_safe_store_precombine_shadow_seconds` / `benchmark.sim_initial_safe_store_precombine_shadow_build_seconds` / `benchmark.sim_initial_safe_store_precombine_shadow_alloc_seconds` / `benchmark.sim_initial_safe_store_precombine_shadow_group_build_seconds` / `benchmark.sim_initial_safe_store_precombine_shadow_compare_seconds` / `benchmark.sim_initial_safe_store_precombine_shadow_order_compare_seconds` / `benchmark.sim_initial_safe_store_precombine_shadow_digest_seconds` / `benchmark.sim_initial_safe_store_precombine_shadow_size_mismatches` / `benchmark.sim_initial_safe_store_precombine_shadow_candidate_mismatches` / `benchmark.sim_initial_safe_store_precombine_shadow_order_mismatches` / `benchmark.sim_initial_safe_store_precombine_shadow_digest_mismatches` / `benchmark.sim_initial_safe_store_precombine_shadow_input_summaries` / `benchmark.sim_initial_safe_store_precombine_shadow_unique_states` / `benchmark.sim_initial_safe_store_precombine_shadow_duplicate_summaries` / `benchmark.sim_initial_safe_store_precombine_shadow_est_saved_upserts` report exactness, duplicate-work estimates, and low-overhead shadow-only cost breakdowns; the precombined store is never used for output, pruning, upload, dispatch, validation, or later locate/region work
- `LONGTARGET_SIM_CUDA_INITIAL_SAFE_STORE_GPU_PRECOMBINE_SHADOW=1` enables a default-off diagnostic CUDA precombine shadow for the initial safe-store update stage. It uploads GPU-produced initial summaries to a CUDA helper, reduces them to one unique-start candidate state per start, preserves first-seen safe-store order, downloads only the unique states, and compares them with the CPU-authoritative post-update safe store. `benchmark.sim_initial_safe_store_gpu_precombine_shadow_enabled` / `benchmark.sim_initial_safe_store_gpu_precombine_shadow_calls` / `benchmark.sim_initial_safe_store_gpu_precombine_shadow_seconds` / `benchmark.sim_initial_safe_store_gpu_precombine_shadow_input_summaries` / `benchmark.sim_initial_safe_store_gpu_precombine_shadow_unique_states` / `benchmark.sim_initial_safe_store_gpu_precombine_shadow_est_saved_upserts` / `benchmark.sim_initial_safe_store_gpu_precombine_shadow_h2d_bytes` / `benchmark.sim_initial_safe_store_gpu_precombine_shadow_d2h_bytes` / `benchmark.sim_initial_safe_store_gpu_precombine_shadow_size_mismatches` / `benchmark.sim_initial_safe_store_gpu_precombine_shadow_candidate_mismatches` / `benchmark.sim_initial_safe_store_gpu_precombine_shadow_order_mismatches` / `benchmark.sim_initial_safe_store_gpu_precombine_shadow_digest_mismatches` report diagnostic coverage only. The GPU-precombined store is never used for output, safe-store authority, pruning, upload, locate, region, or clean-gate decisions; see `docs/cuda_initial_safe_store_gpu_precombine_shadow.md`.
- `LONGTARGET_SIM_CUDA_INITIAL_SAFE_STORE_GPU_PRECOMBINE=1` enables a default-off real opt-in that uses CUDA unique-start precombine output as the initial host safe-store update input, then leaves the existing CPU prune/upload/locate/region path authoritative. `LONGTARGET_SIM_CUDA_INITIAL_SAFE_STORE_GPU_PRECOMBINE_VALIDATE=1` additionally builds the legacy CPU update result on a side store and compares size, candidate content, order, and digest before accepting the GPU-precombined input; validation mismatches record fallback telemetry and keep the legacy path. `LONGTARGET_SIM_CUDA_INITIAL_SAFE_STORE_GPU_PRECOMBINE_RESIDENT_SOURCE=1` asks the opt-in path to reuse the GPU-resident initial summary buffer when available, falling back to the host-H2D source otherwise. `benchmark.sim_initial_safe_store_gpu_precombine_requested` / `benchmark.sim_initial_safe_store_gpu_precombine_active` / `benchmark.sim_initial_safe_store_gpu_precombine_validate_enabled` / `benchmark.sim_initial_safe_store_gpu_precombine_calls` / `benchmark.sim_initial_safe_store_gpu_precombine_seconds` / `benchmark.sim_initial_safe_store_gpu_precombine_input_summaries` / `benchmark.sim_initial_safe_store_gpu_precombine_unique_states` / `benchmark.sim_initial_safe_store_gpu_precombine_est_saved_upserts` / `benchmark.sim_initial_safe_store_gpu_precombine_h2d_bytes` / `benchmark.sim_initial_safe_store_gpu_precombine_d2h_bytes` / `benchmark.sim_initial_safe_store_gpu_precombine_validate_seconds` / `benchmark.sim_initial_safe_store_gpu_precombine_size_mismatches` / `benchmark.sim_initial_safe_store_gpu_precombine_candidate_mismatches` / `benchmark.sim_initial_safe_store_gpu_precombine_order_mismatches` / `benchmark.sim_initial_safe_store_gpu_precombine_digest_mismatches` / `benchmark.sim_initial_safe_store_gpu_precombine_fallbacks` / `benchmark.sim_initial_safe_store_gpu_precombine_resident_source_requested` / `benchmark.sim_initial_safe_store_gpu_precombine_resident_source_active` / `benchmark.sim_initial_safe_store_gpu_precombine_resident_source_supported` / `benchmark.sim_initial_safe_store_gpu_precombine_resident_source_disabled_reason` / `benchmark.sim_initial_safe_store_gpu_precombine_summary_h2d_elided` / `benchmark.sim_initial_safe_store_gpu_precombine_summary_h2d_bytes_saved` / `benchmark.sim_initial_safe_store_gpu_precombine_input_source` report the opt-in path. The opt-in remains default-off and does not change candidate replay, prune, upload, locate, region, exact-frontier clean gate, `gpu_real`, `ordered_segmented_v3`, or `EXACT_FRONTIER_REPLAY`; see `docs/cuda_initial_safe_store_gpu_precombine.md`.
- `LONGTARGET_SIM_CUDA_INITIAL_SAFE_STORE_GPU_PRECOMBINE_PRUNE_SHADOW=1` enables a default-off diagnostic CUDA prune shadow after the initial safe-store GPU precombine path. It applies the same prune predicate on device (`score > runningMin` or final-frontier start coordinate), downloads the kept states, and compares size, candidate content, order, and digest against the CPU-authoritative post-prune safe store. `benchmark.sim_initial_safe_store_gpu_precombine_prune_shadow_enabled` / `benchmark.sim_initial_safe_store_gpu_precombine_prune_shadow_calls` / `benchmark.sim_initial_safe_store_gpu_precombine_prune_shadow_seconds` / `benchmark.sim_initial_safe_store_gpu_precombine_prune_shadow_input_states` / `benchmark.sim_initial_safe_store_gpu_precombine_prune_shadow_kept_states` / `benchmark.sim_initial_safe_store_gpu_precombine_prune_shadow_removed_states` / `benchmark.sim_initial_safe_store_gpu_precombine_prune_shadow_removed_ratio` / `benchmark.sim_initial_safe_store_gpu_precombine_prune_shadow_est_d2h_bytes_before` / `benchmark.sim_initial_safe_store_gpu_precombine_prune_shadow_est_d2h_bytes_after` / `benchmark.sim_initial_safe_store_gpu_precombine_prune_shadow_est_d2h_bytes_saved` / `benchmark.sim_initial_safe_store_gpu_precombine_prune_shadow_size_mismatches` / `benchmark.sim_initial_safe_store_gpu_precombine_prune_shadow_candidate_mismatches` / `benchmark.sim_initial_safe_store_gpu_precombine_prune_shadow_order_mismatches` / `benchmark.sim_initial_safe_store_gpu_precombine_prune_shadow_digest_mismatches` report diagnostic coverage only. The GPU-pruned states are never used for production safe-store, upload, locate, region, output, or clean-gate decisions; see `docs/cuda_initial_safe_store_gpu_precombine_prune_shadow.md`.
- `LONGTARGET_SIM_CUDA_INITIAL_SAFE_STORE_GPU_PRECOMBINE_PRUNE=1` extends the default-off real safe-store GPU precombine opt-in so the CUDA helper prunes unique-start states on device and downloads only kept states for the initial host safe-store. `LONGTARGET_SIM_CUDA_INITIAL_SAFE_STORE_GPU_PRECOMBINE_PRUNE_VALIDATE=1` compares the GPU-precombined/pruned host store against a legacy CPU post-prune side store before accepting it; mismatches record telemetry and fall back to legacy. `benchmark.sim_initial_safe_store_gpu_precombine_prune_requested` / `benchmark.sim_initial_safe_store_gpu_precombine_prune_active` / `benchmark.sim_initial_safe_store_gpu_precombine_prune_validate_enabled` / `benchmark.sim_initial_safe_store_gpu_precombine_prune_calls` / `benchmark.sim_initial_safe_store_gpu_precombine_prune_seconds` / `benchmark.sim_initial_safe_store_gpu_precombine_prune_input_states` / `benchmark.sim_initial_safe_store_gpu_precombine_prune_kept_states` / `benchmark.sim_initial_safe_store_gpu_precombine_prune_removed_states` / `benchmark.sim_initial_safe_store_gpu_precombine_prune_removed_ratio` / `benchmark.sim_initial_safe_store_gpu_precombine_prune_d2h_bytes` / `benchmark.sim_initial_safe_store_gpu_precombine_prune_d2h_bytes_saved` / `benchmark.sim_initial_safe_store_gpu_precombine_prune_validate_seconds` / `benchmark.sim_initial_safe_store_gpu_precombine_prune_size_mismatches` / `benchmark.sim_initial_safe_store_gpu_precombine_prune_candidate_mismatches` / `benchmark.sim_initial_safe_store_gpu_precombine_prune_order_mismatches` / `benchmark.sim_initial_safe_store_gpu_precombine_prune_digest_mismatches` / `benchmark.sim_initial_safe_store_gpu_precombine_prune_fallbacks` report the opt-in. CPU remains the default authority, upload/locate/region continue through the existing host safe-store handoff, and the path does not touch candidate replay, clean gate, `gpu_real`, `ordered_segmented_v3`, or `EXACT_FRONTIER_REPLAY`; see `docs/cuda_initial_safe_store_gpu_precombine.md`.
- `LONGTARGET_SIM_CUDA_INITIAL_SAFE_STORE_GPU_PRECOMBINE_PRUNE_FAST_MATERIALIZE=1` adds a default-off host-only materialization opt-in for GPU-pruned kept states. It keeps the same GPU precombine/prune output and D2H shape, but materializes the host safe-store vector and start-index map together with reserved capacity; `LONGTARGET_SIM_CUDA_INITIAL_SAFE_STORE_GPU_PRECOMBINE_PRUNE_FAST_MATERIALIZE_VALIDATE=1` compares against the previous materializer and falls back on mismatch. `benchmark.sim_initial_safe_store_fast_materialize_requested` / `benchmark.sim_initial_safe_store_fast_materialize_active` / `benchmark.sim_initial_safe_store_fast_materialize_seconds` / `benchmark.sim_initial_safe_store_fast_materialize_index_build_seconds` / `benchmark.sim_initial_safe_store_fast_materialize_validate_seconds` / `benchmark.sim_initial_safe_store_fast_materialize_size_mismatches` / `benchmark.sim_initial_safe_store_fast_materialize_candidate_mismatches` / `benchmark.sim_initial_safe_store_fast_materialize_order_mismatches` / `benchmark.sim_initial_safe_store_fast_materialize_digest_mismatches` / `benchmark.sim_initial_safe_store_fast_materialize_fallbacks` report the opt-in. This is not a packed-D2H path and does not change CPU upload/locate/region authority.
- `LONGTARGET_SIM_CUDA_INITIAL_SAFE_STORE_GPU_PRECOMBINE_PRUNE_PACKED_D2H=1` adds a default-off packed kept-state D2H measurement on top of the resident GPU precombine prune opt-in. It packs only post-prune kept states to 20-byte records, downloads and unpacks that payload, then compares size, candidate values, order, and digest against the existing unpacked kept-state D2H result. `benchmark.sim_initial_safe_store_gpu_precombine_prune_packed_d2h_requested` / `benchmark.sim_initial_safe_store_gpu_precombine_prune_packed_d2h_active` / `benchmark.sim_initial_safe_store_gpu_precombine_prune_packed_d2h_supported` / `benchmark.sim_initial_safe_store_gpu_precombine_prune_packed_d2h_disabled_reason` / `benchmark.sim_initial_safe_store_gpu_precombine_prune_packed_d2h_bytes` / `benchmark.sim_initial_safe_store_gpu_precombine_prune_unpacked_d2h_bytes` / `benchmark.sim_initial_safe_store_gpu_precombine_prune_packed_d2h_bytes_saved` / `benchmark.sim_initial_safe_store_gpu_precombine_prune_pack_seconds` / `benchmark.sim_initial_safe_store_gpu_precombine_prune_unpack_seconds` / `benchmark.sim_initial_safe_store_gpu_precombine_prune_packed_fallbacks` / `benchmark.sim_initial_safe_store_gpu_precombine_prune_packed_size_mismatches` / `benchmark.sim_initial_safe_store_gpu_precombine_prune_packed_candidate_mismatches` / `benchmark.sim_initial_safe_store_gpu_precombine_prune_packed_order_mismatches` / `benchmark.sim_initial_safe_store_gpu_precombine_prune_packed_digest_mismatches` report measurement coverage. The host safe-store remains CPU-authoritative through the existing kept-state materialization, and this path does not touch candidate replay, clean gate, `gpu_real`, `ordered_segmented_v3`, or `EXACT_FRONTIER_REPLAY`; see `docs/cuda_initial_safe_store_gpu_precombine.md`.
- `LONGTARGET_SIM_CUDA_INITIAL_SAFE_STORE_GPU_PRECOMBINE_PRUNE_PACKED_D2H_REAL=1` makes the packed kept-state payload the default-off real materialization source for the resident GPU precombine prune opt-in. It downloads packed kept states only, unpacks them on host, and materializes the host safe-store from that vector; `LONGTARGET_SIM_CUDA_INITIAL_SAFE_STORE_GPU_PRECOMBINE_PRUNE_PACKED_D2H_VALIDATE=1` adds an unpacked kept-state side download for size/candidate/order/digest validation and falls back on mismatch. `benchmark.sim_initial_safe_store_gpu_precombine_prune_packed_d2h_real_requested` / `benchmark.sim_initial_safe_store_gpu_precombine_prune_packed_d2h_real_active` / `benchmark.sim_initial_safe_store_gpu_precombine_prune_packed_d2h_validate_enabled` / `benchmark.sim_initial_safe_store_gpu_precombine_prune_unpacked_d2h_bytes_elided` / `benchmark.sim_initial_safe_store_gpu_precombine_prune_packed_unpack_seconds` / `benchmark.sim_initial_safe_store_gpu_precombine_prune_packed_materialize_seconds` / `benchmark.sim_initial_safe_store_gpu_precombine_prune_packed_index_rebuild_seconds` / `benchmark.sim_initial_safe_store_gpu_precombine_prune_packed_host_apply_seconds` / `benchmark.sim_initial_safe_store_gpu_precombine_prune_packed_validate_seconds` report the real opt-in and host cost breakdown. CPU upload/locate/region remain authoritative, candidate replay is unchanged, and the path does not activate the clean gate or route through `gpu_real`, `ordered_segmented_v3`, or `EXACT_FRONTIER_REPLAY`; see `docs/cuda_initial_safe_store_gpu_precombine.md`.
...
- `benchmark.sim_initial_scan_tail_seconds`: residual initial-scan tail timing that is not otherwise attributed to GPU, D2H, CPU merge, count-copy, base-upload, or sync-wait fields; use it as a gap indicator, not as an optimization target by itself
