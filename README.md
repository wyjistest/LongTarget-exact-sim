...
- `LONGTARGET_SIM_CUDA_INITIAL_SAFE_STORE_HANDOFF=1`: experimental partial safe-store handoff for the default summary-handoff initial scan; CPU still applies the exact frontier/context, while CUDA builds/prunes the all-state safe-store and keeps the persistent GPU handle for `safe_workset` locate (default: off; compatibility alias: `LONGTARGET_ENABLE_SIM_CUDA_INITIAL_SAFE_STORE_DEVICE_MAINTENANCE=1`)
