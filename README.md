...
- `LONGTARGET_SIM_CUDA_INITIAL_EXACT_FRONTIER_ONE_CHUNK_BOUNDED_SHADOW=1` with `LONGTARGET_SIM_CUDA_INITIAL_EXACT_FRONTIER_ONE_CHUNK_BOUNDED_MAX_SUMMARIES=N`: default-off bounded prefix GPU shadow for production-shaped ordered summaries. It compares only the first `N` summaries per request/window against a CPU prefix reference, reports `benchmark.sim_initial_exact_frontier_one_chunk_bounded_shadow_*`, and never feeds the result to output, safe-store, locate, region, planner authority, or the clean gate.
