...
To aggregate multiple real workload refreshes, run `scripts/summarize_longtarget_sim_pipeline_budget_rollup.py --sim-pipeline-budget-decision <workload-a/sim_pipeline_budget_decision.json> --sim-pipeline-budget-decision <workload-b/sim_pipeline_budget_decision.json> --output-dir <dir>`. The roll-up emits `sim_pipeline_budget_rollup.json`, `sim_pipeline_budget_rollup_decision.json`, and `sim_pipeline_budget_rollup.md`. If any workload still lacks SIM substage telemetry, the roll-up preserves `selection_status=insufficient_sim_substage_telemetry` and `recommended_next_action=collect_sim_substage_telemetry`; if ready workloads disagree or mix selected and no-stable decisions, it reports `selection_status=workload_dependent_subcomponent` and `recommended_next_action=expand_or_stratify_sim_pipeline_budget`. A stable subcomponent is authoritative only when real workload decisions agree, and `runtime_prototype_allowed` remains `false`.
