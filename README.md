...
- `LONGTARGET_SIM_CUDA_INITIAL_EXACT_FRONTIER_PER_REQUEST_SHADOW=1`: default-off per-request exact frontier diagnostic shadow. Each request/window replays its ordered initial summaries independently on GPU, compares the GPU candidate snapshot to the CPU replay snapshot with CPU authority, and reports `benchmark.sim_initial_exact_frontier_per_request_shadow_*` counts, bytes, seconds, and candidate mismatch counters. It does not perform continuous all-request replay, does not feed GPU state into output, safe-store, locate, region, or planner authority, and reports `has_safe_store_contract=0` / `has_epoch_contract=0`.
