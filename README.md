...
The first Phase 3d.2 device-shadow regression remains synthetic, but now covers candidate-state, replacement-sequence, and running-min/floor change-sequence digest subsets: `make check-sim-ordered-maintenance-device-shadow` reuses the ordered CUDA initial-replay kernel and checks device final candidate state, runningMin, runningMinSlot, slot key/score/generation hashes, summary ordinal hash, handoff hash, replacement sequence hash/count, runningMin change hash/count, runningMinSlot change hash/count, and floor change hash/count against the host and CPU-shadow digests on small ordered sequences. It intentionally does not promote `shadow_backend=device` to `passed`, because candidate-index visibility and safe-store digest coverage are still required before real device shadow validation can run.
